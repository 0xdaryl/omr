...
A `TR::Logger` is an abstract base class that declares a logging API. A family
...
A Logger that outputs directly to a file using `TR::IO` output functions.
...
A circular Logger constrains its output to be within a fixed boundary. It is
essentially a wrapper around another Logger supplied when the circular Logger
is created that constrains its output.

Note this is not a perfect circular logger where the maximum output size will
never be exceeded. Instead, it makes a best effort to keep the logging output
within a certain threshold. This is identical behaviour to the original
circular output throttle that existed before Loggers.
...
object to catch diagnostic output. The default Logger is the `TR::NullLogger`
...
`TR::AssertingLogger` as part of their testing to catch unguarded use of
logging functions.
...
```
   trprintf(myTraceOption, log, "My output: %d\n", myInt);
```
