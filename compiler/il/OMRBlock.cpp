      ...
      if (lastNode->getOpCode().isBranch() && lastNode->getBranchDestination() == origTo->getEntry())
         ...
         if (lastNode->getOpCode().hasBranchChildren())
            ...
            if (doTrace)
               {
               if(origToEntry)
                  log->trprintf("jumpwithmultipletargets: origToEntry->getNode = %p\n",origToEntry->getNode());
               if(newToEntry)
                  log->trprintf("jumpwithmultipletargets: newToEntry->getNode = %p\n",newToEntry->getNode());
               }
