   ...
   if (noFPRA)
      ...
      if (resultCanStayInFloatRegister(cg->getCurrentEvaluationTreeTop()->getNode(), node))
         ...
         if (cg->comp()->getOption(TR_TraceCG))
            cg->comp()->getLogger()->trprintf("Result of node %p can stay in FP reg.\n", node);
         trgReg = floatTrgReg;
         }
      else
         {
   ...
   if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
      ...
      if (noFPRA)
         ...
         if (resultCanStayInFloatRegister(cg->getCurrentEvaluationTreeTop()->getNode(), node))
            {
            if (cg->comp()->getOption(TR_TraceCG))
               cg->comp()->getLogger()->trprintf("Result of node %p can stay in FP reg (not exec.).\n", node);
            }
...
TR::Register *OMR::ARM::TreeEvaluator::dconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (noFPRA)
      ...
      if (resultCanStayInFloatRegister(cg->getCurrentEvaluationTreeTop()->getNode(), node))
         {
         if (comp->getOption(TR_TraceCG))
            comp->getLogger()->trprintf("Result of node %p can stay in FP reg.\n", node);
         trgReg = doubleTrgReg;
         }
      else
         {
...
//#define STAYINFP
   ...
   if (noFPRA)
      ...
      if (resultCanStayInFloatRegister(cg->getCurrentEvaluationTreeTop()->getNode(), node))
         {
         if (cg->comp()->getOption(TR_TraceCG))
            cg->comp()->getLogger()->trprintf("Result of node %p can stay in FP reg in fload.\n", node);
...
TR::Register *OMR::ARM::TreeEvaluator::dloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (noFPRA)
      ...
      if (resultCanStayInFloatRegister(cg->getCurrentEvaluationTreeTop()->getNode(), node))
         {
         if (cg->comp()->getOption(TR_TraceCG))
            cg->comp()->getLogger()->trprintf("Result of node %p can stay in FP reg in dload.\n", node);
         trgReg = doubleTrgReg;
         }
      else
         {
