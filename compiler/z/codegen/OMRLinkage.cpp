...
OMR::Z::Linkage::saveArguments(void * cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol> *parameterList
      ...
      for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext())
         ...
         if (type.isFloatingPoint())
            ...
            if (type.getDataType() == TR::Double
                ...
                count = (self()->comp()->target().is64Bit()) ? 1 : 2;
#ifdef J9_PROJECT_SPECIFIC
            else if (type.isLongDouble())
                count = (self()->comp()->target().is64Bit()) ? 2 : 4;
#endif
            else
                count = 1;
            ...
            {
            numIntArgs += (type.isInt64() && self()->comp()->target().is32Bit()) ? 2 : 1;
            }
         }
      }
   ...
   for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext())
      ...
      if (lri >= 0)
         ...
         if (type.isFloatingPoint()) // float/double/long double
         ...
         else if(self()->comp()->target().isLinux() && dtype == TR::Aggregate)
           {
           ...
           if(self()->comp()->target().is32Bit() && paramCursor->getSize() > 4)
             {
            ...
            lastFreeIntArgIndex = lri + 1;
            if (fullLong && self()->comp()->target().is32Bit())
               {
               lastFreeIntArgIndex++;
               }
            }
         ...
         if (ai>=0 && !InPreProlog)
            ...
            if (regNum != ai || (dtype == TR::Int64 && self()->comp()->target().is32Bit()))
               {
               ...
               if (freeScratchable.isSet(ai))
                  ...
                  if (dtype == TR::Float
                     ...
                     {
                     if (dtype == TR::Int64 && self()->comp()->target().is32Bit())
                        {
...
OMR::Z::Linkage::copyArgRegister(TR::Node * callNode, TR::Node * child, TR::Register * argRegister)
   ...
   if (self()->comp()->target().is32Bit() && child->getDataType() == TR::Int64 && !argRegister->getRegisterPair())
      {
...
OMR::Z::Linkage::pushArg(TR::Node * callNode, TR::Node * child, int32_t numIntegerArgs, int32_t numFloatArgs, int32_t * stackOffsetPtr,TR::RegisterDependencyConditions * dependencies, TR::Register * argRegister)
   ...
   int8_t regSize = (self()->comp()->target().is64Bit())? 8:4;
   int8_t argSize = 0;
   ...
   if (self()->comp()->target().is32Bit() && argType.isInt64())
...
#ifdef J9_PROJECT_SPECIFIC
         if (argType == TR::DecimalFloat && self()->comp()->target().is64Bit()) // special case for DecFloat on 64bit
   ...
   for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step)
      ...
      switch (child->getDataType())
         ...
         case TR::DecimalDouble:
            ...
            if (self()->isSkipGPRsForFloatParms())
               {
               numIntegerArgs += (self()->comp()->target().is64Bit()) ? 1 : 2;
               }
            break;
#ifdef J9_PROJECT_SPECIFIC
         case TR::DecimalLongDouble:
            ...
            if (self()->isSkipGPRsForFloatParms())
               ...
               if (numIntegerArgs < self()->getNumIntegerArgumentRegisters())
                  {
                  numIntegerArgs += (self()->comp()->target().is64Bit()) ? 2 : 4;
                  }
               }
            break;
   ...
   if (depVirReg)
      ...
      *virtualRegPtr = depVirReg;
      ...
      if (isDummy)
         ...
         if (self()->comp()->target().is64Bit() && regNum <= TR::RealRegister::LastGPR)
            {
...
OMR::Z::Linkage::restorePreservedRegs(TR::RealRegister::RegNum firstUsedReg,
   ...
   {
   bool break64 = self()->comp()->target().is64Bit() && self()->comp()->getOption(TR_Enable39064Epilogue);
   ...
   else if ((self()->comp()->target().is64Bit() && !break64) ||
