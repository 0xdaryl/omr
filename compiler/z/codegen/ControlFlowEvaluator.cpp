...
TR::Register *OMR::Z::TreeEvaluator::xmaxxminHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (isFloatingPointOp) {
        ...
        generateRXEInstruction(cg, tdcOpcode, node, operand1, generateS390MemoryReference(0x00F, cg),
            0); // can't use load and test here since it would set the quiet bit
        ...
        if (node->getOpCode().isMax()) {
            ...
            generateRXEInstruction(cg, tdcOpcode, node, operand1, generateS390MemoryReference(0x800, cg),
                0); // operand1 is +0 ?
            generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK4, node,
                cFlowRegionEnd); // it is +0
            ...
            generateRXEInstruction(cg, tdcOpcode, node, operand1, generateS390MemoryReference(0x400, cg),
                0); // operand1 is -0 ?
            generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK4, node, cFlowRegionEnd);
        }
    }
...
TR::Register *OMR::Z::TreeEvaluator::fpMinMaxVectorHelper(TR::Node *node, TR::CodeGenerator *cg)
{
     ...
     * For Min: (a < b) || (a == NaN)
     */
    generateVRRcInstruction(cg, TR::InstOpCode::VO, node, argumentSelectorReg, argumentSelectorReg, compareResultReg, 0,
        0, 0);
...
static TR::Register *imaximinHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    TR::InstOpCode::S390BranchCondition branchCond
        = node->getOpCode().isMax() ? TR::InstOpCode::COND_BHR : TR::InstOpCode::COND_BLR;
...
static TR::Register *lmaxlminHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    TR::InstOpCode::S390BranchCondition branchCond
        = node->getOpCode().isMax() ? TR::InstOpCode::COND_BHR : TR::InstOpCode::COND_BLR;
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
...
TR::Register *OMR::Z::TreeEvaluator::fmaxHelper(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *OMR::Z::TreeEvaluator::dmaxHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    TR::Register *result = NULL;
    if (cg->getSupportsVectorRegisters()) {
...
TR::Register *OMR::Z::TreeEvaluator::fminHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    TR::Register *result = NULL;
    if (cg->getSupportsVectorRegisters()
        && cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_S390_VECTOR_FACILITY_ENHANCEMENT_1)) {
...
TR::Register *OMR::Z::TreeEvaluator::dminHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    TR::Register *result = NULL;
    if (cg->getSupportsVectorRegisters()) {
