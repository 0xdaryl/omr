...
OMR::Z::CodeGenerator::checkIsUnneededAloadi(TR::Node *parent, TR::Node *node, TR::TreeTop *tt)
   ...
   if (node->isUnneededAloadi())
      ...
      if (parent->getOpCodeValue() == TR::ifacmpne
         ...
         {
         trprintf(self()->comp()->getOption(TR_TraceCG), self()->comp()->log(), "parent %p appears to be a nullcheck over node: %p\n", parent, node);
         }
...
OMR::Z::CodeGenerator::StopUsingEscapedMemRefsRegisters(int32_t topOfMemRefStackBeforeEvaluation)
   ...
   while (_stackOfMemoryReferencesCreatedDuringEvaluation.topIndex() > topOfMemRefStackBeforeEvaluation)
      ...
      trprintf(self()->comp()->getOption(TR_TraceCG), self()->comp()->log(),
...
OMR::Z::CodeGenerator::processUnusedNodeDuringEvaluation(TR::Node *node)
   ...
   if (node)
      ...
      if (node->getOpaquePseudoRegister())
         ...
         trprintf(trace, log, "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s - handle extra ref to addr child (ref is node based %s - %s %p)\n",
...
OMR::Z::CodeGenerator::ssrClobberEvaluate(TR::Node * node, TR::MemoryReference *sourceMR) // sourceMR can be NULL
   ...
   trprintf(trace, log, "\tneedsClobberEval : %s (%p), storageRef #%d (%p) (isBCD = %s, isReadOnlyTemp = %s, tempRefCount = %d)\n",
   ...
   if (node->getReferenceCount() > 1)
      ...
      trprintf(trace, log, "\t\t-->needsClobber=true due to node->refCount %d\n", node->getReferenceCount());
      }
   else if (srcStorageReference->getOwningRegisterCount() > 1)
      {
      needsClobberDueToRegCount = true;
      trprintf(trace, log, "\t\t-->needsClobber=true due to srcStorageReference #%d (%s) owningRegCount %d\n",
      ...
      trprintf(trace, log, "\t\t-->needsClobber=true due to reuse of readOnlyTemp %s with refCount %d > 1\n",
      ...
      {
      trprintf(trace, log, "\t\t-->needsClobber=false, srcStorageReference ref #%d (%s) owningRegCount=%d)\n",
   ...
   if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse)
      ...
      trprintf(trace, log, "ssrClobberEvaluate: node %s (%p) with srcReg %s (srcRegSize %d, srcReg->validSymSize %d, srcReg->liveSymSize %d) and srcStorageReference #%d with %s (symSize %d)\n",
      ...
      trprintf(trace, log, "\tgot copyStorageReference #%d with %s for clobberEvaluate\n",i
      ...
      if (srcStorageReference->isReadOnlyTemporary() && srcRegister->getRightAlignedIgnoredBytes() > 0)
         ...
         trprintf(trace, log, "\tremove %d ignoredBytes from sourceMR->_offset (%d->%d) for readOnlyTemp #%d (%s)\n",
               regIgnoredBytes, sourceMR->getOffset(), sourceMR->getOffset()+regIgnoredBytes, srcStorageReference->getReferenceNumber(),
               self()->getDebug()->getName(srcStorageReference->getSymbol()));
         sourceMR->addToOffset(regIgnoredBytes);
         }

      trprintf(trace, log, "\tgen MVC or memcpy sequence for copy with size %d\n", byteLength);
      ...
      trprintf(trace, log, "\tupdate srcRegister %s with copy ref #%d (%s)\n",
      ...
      if (trace && alreadyClearedDigits)
         ...
         log->printf("\t\t* setting surviving leftAlignedZeroDigits %d on srcRegister %s with new copy ref #%d (digitsInSrc - digitsToClear = %d - %d = %d)\n",
         ...
         for (TR::Node *listNode = listIt.getFirst(); listNode; listNode = listIt.getNext())
            ...
            if (listReg != srcRegister)
               ...
               if (listReg->getStorageReference() != srcStorageReference)
                  ...
                  //
                  trprintf(trace, log, "\ty^y : skip update on reg %s node %s (%p), storageRef has already changed -- was #%d (%s), now #%d (%s)\n",
                  ...
                  {
                  trprintf(trace, log, "\ty^y : update reg %s with ref #%d (%s) on node %s (%p) refCount %d from _nodesToUpdateOnClobber with copy ref #%d (%s) (skip if refCount == 0)\n",
                        self()->getDebug()->getName(listReg),
                        listReg->getStorageReference()->getReferenceNumber(), self()->getDebug()->getName(listReg->getStorageReference()->getSymbol()),
...
bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node *node)
   ...
   if (node->getOpCode().isPackedRightShift())
      ...
      if (roundAmount == 0 && shiftAmount != 0 && ((shiftAmount&0x1)==0))
         {
         trprintf(trace, log, "\tnodeRequiresATemporary = true for packedShiftRight %s (%p)\n", node->getOpCode().getName(), node);
         return true;
         }
      }
      ...
      trprintf(trace, log, "\tnodeRequiresATemporary = true for packedDivide %s (%p)\n", node->getOpCode().getName(), node);
      return true;
      }
      ...
      {
      trprintf(trace, log, "\tnodeRequiresATemporary = true for zonedSepToZoned %s (%p)\n", node->getOpCode().getName(), node);
      return true;
      }
   else
#endif
      ...
      {
      trprintf(trace, log, "\tnodeRequiresATemporary = true for aggrShiftRight %s (%p)\n", node->getOpCode().getName(), node);
      return true;
      }
   return false;
   }
...
OMR::Z::CodeGenerator::isOutOf32BitPositiveRange(int64_t value, bool trace)
   ...
   if (value < 0 || value > INT_MAX)
      {
      trprintf(trace, self()->comp()->log(), "\tisOutOf32BitPositiveRange = true : value %lld < 0 (or > INT_MAX)\n", value);
      return true;
      }
   return false;
   }
...
OMR::Z::CodeGenerator::possiblyConflictingNode(TR::Node *node)
   ...
   if (node->getReferenceCount() > 1)
      ...
      if (node->getRegister() == NULL && node->getOpCode().canHaveStorageReferenceHint())
         {
         trprintf(trace, log, "\t\t\tpossiblyConflicting=true %s (%p) : reg==NULL and storageRefHint_Case, refCount %d\n",
               node->getOpCode().getName(), node, node->getReferenceCount());
         possiblyConflicting = true;
         }
      else if (node->getType().isBCD())
         {
         trprintf(trace, log, "\t\t\tpossiblyConflicting=true %s (%p) : BCDOrAggrType_Case, reg=%s, refCount %d\n",
...
OMR::Z::CodeGenerator::collectConflictingAddressNodes(TR::Node *parent, TR::Node *node, TR::list<TR::Node*> *conflictingAddressNodes)
   ...
   trprintf(trace, log, "\t\texamining node %s (%p) : register %s\n",
   ...
   if (self()->possiblyConflictingNode(node))
      {
      trprintf(trace, log, "\t\t\tadd %s (%p) to conflicting nodes list (refCount %d)\n", node->getOpCode().getName(), node, node->getReferenceCount());
...
OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node *node1, TR::Node *node2)
   ...
   if (node1->getSize() != node2->getSize())
      {
      trprintf(trace, log, "\t\tloadOrStoreAddressesMatch = false (sizes differ) : node1 %s (%p) size = %d and node2 %s (%p) size = %d\n",
   ...
   trprintf(trace, log, "\t\tloadOrStoreAddressesMatch = %s : node1 %s (%p) and node2 %s (%p)\n",
