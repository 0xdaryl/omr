    ...
    if (node->isUnneededAloadi()) {
        if (parent->getOpCodeValue() == TR::ifacmpne || parent->getOpCodeValue() == TR::ificmpeq
            ...
            if (self()->comp()->getOption(TR_TraceCG))
                self()->comp()->getLogger()->printf("parent %p appears to be a nullcheck over node: %p\n", parent,
                    node);
    ...
    if (self()->comp()->target().cpu.isZ() && (node->getOpCode().isLoadVar() || node->getOpCode().isStore())
        ...
        if (add1 && add2 && const1
            && performTransformation(self()->comp(), "%sBase/index/displacement form addressing prep for node [%p]\n",
                OPT_DETAILS, node)) {
            // comp()->getLogger()->printf("&&& Found pattern root=%llx add1=%llx add2=%llx const1=%llx sub=%llx
...
void OMR::Z::CodeGenerator::doBinaryEncoding()
{
    ...
    if (self()->comp()->getOption(TR_TraceCG))
        self()->comp()->getDebug()->dumpMethodInstrs(self()->comp()->getLogger(), "Post Prologue/epilogue Instructions",
            false);
    ...
    if (node) {
        ...
        if (node->getOpaquePseudoRegister()) {
            ...
            if (self()->traceBCDCodeGen())
                comp->getLogger()->printf("\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s "
        ...
        if (node->safeToDoRecursiveDecrement()) {
            ...
            if (node->getOpaquePseudoRegister() && !alreadyProcessedUnusedStorageRef) {
                ...
                TR_StorageReference *ref = reg->getStorageReference();
                if (self()->traceBCDCodeGen())
                    comp->getLogger()->printf("\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg "
                                              "%s - handle extra ref to addr child (ref is node based %s - %s %p)\n",
    ...
    TR::MemoryReference *sourceMR) // sourceMR can be NULL
{
    ...
    if (node->getReferenceCount() > 1) {
        ...
        if (self()->traceBCDCodeGen())
            comp->getLogger()->printf(
                "\t\t-->needsClobber=true due to srcStorageReference #%d (%s) owningRegCount %d\n",
        ...
        if (self()->traceBCDCodeGen()) {
            comp->getLogger()->printf(
                "\t\t-->needsClobber=false, srcStorageReference ref #%d (%s) owningRegCount=%d)\n",
    ...
    if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse) {
        ...
        if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
            comp->getLogger()->printf("\tsrcRegister %s had leftAlignedZeroDigits %d on original ref #%d\n",
                ...
                srcStorageReference->getReferenceNumber());
            comp->getLogger()->printf("\t\t* setting surviving leftAlignedZeroDigits %d on srcRegister %s with new "
                                      "copy ref #%d (digitsInSrc - digitsToClear = %d - %d = %d)\n",
            ...
            for (TR::Node *listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
                ...
                if (listReg != srcRegister) {
                    if (listReg->getStorageReference() != srcStorageReference) {
                        ...
                        //
                        if (self()->traceBCDCodeGen())
                            comp->getLogger()->printf("\ty^y : skip update on reg %s node %s (%p), storageRef has "
                                                      "already changed -- was #%d (%s), now #%d (%s)\n",
    ...
    TR_BitVector &availableRegisters)
{
    ...
    while (loe.hasMoreElements()) {
        ...
        //      comp()->getLogger()->printf("For register candidate %d, considering block %d.  Looking at node
...
bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node *node)
{
    ...
    if (node->getOpCode().isPackedRightShift()) {
        ...
        if (roundAmount == 0 && shiftAmount != 0 && ((shiftAmount & 0x1) == 0)) {
            if (self()->traceBCDCodeGen())
                comp->getLogger()->printf("\tnodeRequiresATemporary = true for packedShiftRight %s (%p)\n",
                    node->getOpCode().getName(), node);
            return true;
        }
        ...
        if (self()->traceBCDCodeGen())
            comp->getLogger()->printf("\tnodeRequiresATemporary = true for packedDivide %s (%p)\n",
                node->getOpCode().getName(), node);
        return true;
    } else if (node->getOpCode().isConversion() && node->getType().isAnyZoned()
        && node->getFirstChild()->getType().isZonedSeparateSign()) {
        if (self()->traceBCDCodeGen())
            comp->getLogger()->printf("\tnodeRequiresATemporary = true for zonedSepToZoned %s (%p)\n",
                node->getOpCode().getName(), node);
        return true;
    } else
#endif
        ...
        if (self()->traceBCDCodeGen())
            comp->getLogger()->printf("\tnodeRequiresATemporary = true for aggrShiftRight %s (%p)\n",
                node->getOpCode().getName(), node);
        return true;
    }
    return false;
...
{
    if (self()->traceBCDCodeGen())
        self()->comp()->getLogger()->printf("\t\texamining node %s (%p) : register %s\n", node->getOpCode().getName(),
            node, node->getRegister() ? self()->getDebug()->getName(node->getRegister()) : "NULL");

    if (self()->possiblyConflictingNode(node)) {
        if (self()->traceBCDCodeGen())
            self()->comp()->getLogger()->printf("\t\t\tadd %s (%p) to conflicting nodes list (refCount %d)\n",
                node->getOpCode().getName(), node, node->getReferenceCount());
...
bool OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node *node1, TR::Node *node2)
{
    ...
    if (node1->getSize() != node2->getSize()) {
        if (comp->getOption(TR_TraceCG))
            comp->getLogger()->printf("\t\tloadOrStoreAddressesMatch = false (sizes differ) : node1 %s (%p) size = %d "
                                      "and node2 %s (%p) size = %d\n",
    ...
    if (comp->getOption(TR_TraceCG))
        comp->getLogger()->printf("\t\tloadOrStoreAddressesMatch = %s : node1 %s (%p) and node2 %s (%p)\n",
