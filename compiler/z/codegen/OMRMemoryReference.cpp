...
void OMR::Z::MemoryReference::addToTemporaryNegativeOffset(TR::Node *node, int32_t offset, TR::CodeGenerator *cg)
   ...
   if (offset < 0)
      {
      trprintf(trace, log, "\taddToTemporaryNegativeOffset : %s (%p) new offset %d, existing mr offset %d (existing mr hasTempNegOffset = %s)\n",
      ...
      if (self()->getOffset() < 0 && !self()->hasTemporaryNegativeOffset()) // if there is a 'real' negative offset then deal with this first before adding in the temporary negative offset
         {
         trprintf(trace, log, "\t\texisting mr->offset %d < 0 so call enforceSSFormatLimits to clear this up before setting HasTemporaryNegativeOffset\n", self()->getOffset());
...
bool OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator * cg, TR::Node * addressChild)
   ...
   if (addressChild->getRegister() != NULL)
      ...
      //    node/tree
      //    iconst
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) has register %s\n",
   ...
   if (!addressChild->getOpCode().isAdd())
      {
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) is not an add type - most likely cannot fold\n",
            addressChild->getOpCode().getName(), addressChild);
   ...
   if (!addressChild->getSecondChild()->getOpCode().isLoadConst())
      {
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) second child is not a load const - cannot fold\n",
            addressChild->getOpCode().getName(), addressChild);
   ...
   if (addressChild->getSecondChild()->getRegister() != NULL)
      ...
      //    node/tree
      //    iconst (reg)
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) const child %s (%p) has register %s, cannot fold.\n",
            addressChild->getOpCode().getName(), addressChild,
   ...
   if (addressChild->getReferenceCount() != 1)
      {
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) has ref count %d, cannot fold, need value in register.\n",
   ...
   if (!allConvNodesAreUnneeded)
      {
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. which are needed, folding after evaluation.\n",
            addressChild->getOpCode().getName(), addressChild);
   ...
   if (!eventualNonConversionIsSuitableForFolding)
      {
      trprintf(trace, log, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) eventual non conversion %s (%p) is not suitable for folding.\n",
            addressChild->getOpCode().getName(), addressChild,
   ...
   if (!eventualNonConversion->getOpCode().isIndirect())
      ...
      //    iconst
      trprintf(trace, log,
            " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register and refCount==1 and is a aload+const so setForceFolding=true\n",
            eventualNonConversion->getOpCode().getName(), eventualNonConversion);
...
void recursivelyDecrementIncrementedNodesIfUnderRegister(TR::CodeGenerator * cg, TR::Node * cur, List<TR::Node> & _incNodesList, List<TR::Node> &nodesToNotRecurse,
   ...
   for (listNode = listIt.getFirst(); listNode; listNode = listIt.getNext())
      ...
      if (listNode == cur)
         ...
         if (startRemoving)
            {
            trprintf(trace, log, "\t decReferenceCount on incremented node %p (%d) (%d->%d)\n",
   ...
   if (cur->getRegister() != NULL)
      {
      trprintf(trace, log, "\t %s [%p] (%d) has a register, so start removing under it\n",
...
void ArtificiallyInflateReferenceCountWhenNecessary(TR::MemoryReference * mr, const char * functionString, TR_Array< TR::Node * > & nodeArray,
   ...
   for (uint32_t i = 0; i < nodeArray.size(); i++)
      ...
      if ((nodeArray[i]->getReferenceCount() == 1) && (nodeArray[i]->getRegister() != NULL))
         {
         trprintf(trace, log, " ArtificiallyInflateReferenceCountWhenNecessary: bug potential with node %p (reg %p %s%s): ref count after %s is 1.\n",
               nodeArray[i],
         ...
         if (incrementedNodesList)
            ...
            if (listNode == nodeArray[i])
               ...
               trprintf(trace, log, " ArtificiallyInflateReferenceCountWhenNecessary: skip inflating node %p (reg %p %s%s) because it exists in incrementedNodesList.\n",
                     nodeArray[i],
                     nodeArray[i]->getRegister(),
                     nodeArray[i]->getRegister() ? ( nodeArray[i]->getRegister()->isPlaceholderReg() ? "D_" : "" ) : "?",
                     comp->getDebug()->getName(nodeArray[i]->getRegister()),
...
bool OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator* cg, TR::Node* loadStore, TR::Node* topAdd)
   ...
   trprintf(debug, log, "&&& TBID load=%llx addr1=%llx addr2=%llx\n", loadStore, topAdd, addressChild);
   ...
   if (topAdd->getReferenceCount() == 1)
      {
      trprints(debug, log, "&&& TBID recursive decrement\n");
...
OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (_offset < MAXDISP &&               // skip if going to fixup naturally anyway
      ...
      if (_offset + maxAlignmentBump >= MAXDISP)
         {
         trprintf(cg->traceBCDCodeGen(), cg->comp()->log(),
               "\tz^z : node %s (%p) : _offset %d + maxAlignmentBump %d >= MAXDISP %d : (%d >= %d) -- force 4K fixup\n",
...
OMR::Z::MemoryReference::calcDisplacement(uint8_t * cursor, TR::Instruction * instr, TR::CodeGenerator * cg)
   ...
   if (self()->rightAlignMemRef())
      ...
      trprintf(trace, log, "instr %p : right aligning memRef with symRef #%d (%s, isTemp %s) _offset %d, totalSize %d : bump = ",
            instr,
   ...
   if (self()->leftAlignMemRef())
      ...
      trprintf(trace, log, "instr %p : left aligning memRef with symRef #%d (%s, isTemp %s), _offset %d : bump = totalSize - leftMostByte = %d - %d = ",
            instr,
            self()->getSymbolReference()->getReferenceNumber(),
            cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
            self()->getSymbolReference()->isTempVariableSizeSymRef() ? "yes":"no",
   ...
   if (self()->getSymbolReference()->isTempVariableSizeSymRef())
      {
      trprintf(trace, log, "instr %p : bumping tempVariableSizeSymRef memRef with symRef #%d (%s), _offset %d : bump = symSize - totalSize = %d - %d = ",
            instr,
            self()->getSymbolReference()->getReferenceNumber(),
            cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
            _offset,
...
OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t * cursor, TR::CodeGenerator * cg, TR::Instruction * instr)
   ...
   if (displacement < MINDISP || displacement >= MAXDISP)
      ...
      if ((displacement < MAXLONGDISP && displacement > MINLONGDISP)
         ...
         trprintf(comp->getOption(TR_TraceCG), comp->log(), "[%p] Long Disp Inst using %s as scratch reg\n", instr, cg->getDebug()->getName(scratchReg));
         }
      }
...
OMR::Z::MemoryReference::doEvaluate(TR::Node * subTree, TR::CodeGenerator * cg)
   ...
   if (self()->forceFirstTimeFolding())
      {
      trprintf(cg->traceBCDCodeGen(), comp->log(), "\tforceFirstTimeFolding=true for subTree %s (%p) refCount %d, reg %s (reset firstTimeFlag for next doEvaluate)\n",
