...
void OMR::Z::MemoryReference::addToTemporaryNegativeOffset(TR::Node *node, int32_t offset, TR::CodeGenerator *cg)
   ...
   if (offset < 0)
      ...
      if (cg->traceBCDCodeGen())
         comp->log()->printf("\taddToTemporaryNegativeOffset : %s (%p) new offset %d, existing mr offset %d (existing mr hasTempNegOffset = %s)\n",
      ...
      if (self()->getOffset() < 0 && !self()->hasTemporaryNegativeOffset()) // if there is a 'real' negative offset then deal with this first before adding in the temporary negative offset
         {
         if (cg->traceBCDCodeGen())
            comp->log()->printf("\t\texisting mr->offset %d < 0 so call enforceSSFormatLimits to clear this up before setting HasTemporaryNegativeOffset\n",self()->getOffset());
...
bool OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator * cg, TR::Node * addressChild)
   ...
   if (addressChild->getRegister() != NULL)
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf(" inside setForceFoldingIfAdvantageous, addressChild %s (%p) has register %s\n",
   ...
   if (!addressChild->getOpCode().isAdd())
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf(" inside setForceFoldingIfAdvantageous, addressChild %s (%p) is not an add type - most likely cannot fold\n",
                  addressChild->getOpCode().getName(), addressChild);
         }
      return false;
      }
   ...
   if (!addressChild->getSecondChild()->getOpCode().isLoadConst())
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf(" inside setForceFoldingIfAdvantageous, addressChild %s (%p) second child is not a load const - cannot fold\n",
                  addressChild->getOpCode().getName(), addressChild);
         }
      return false;
      }
   ...
   if (addressChild->getSecondChild()->getRegister() != NULL)
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf(" inside setForceFoldingIfAdvantageous, addressChild %s (%p) const child %s (%p) has register %s, cannot fold.\n",
                  addressChild->getOpCode().getName(), addressChild,
   ...
   if (addressChild->getReferenceCount() != 1)
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf(" inside setForceFoldingIfAdvantageous, addressChild %s (%p) has ref count %d, cannot fold, need value in register.\n",
   ...
   if (foundRegister)
      ...
      if (cg->traceBCDCodeGen())
         ...
         comp->log()->printf("\t\taddressChild %s (%p) node %s (%p) is evaluatedReg+const (%s + %lld) so %s=true\n",
                  addressChild->getOpCode().getName(),addressChild,
   ...
   if (!eventualNonConversion->getOpCode().isIndirect())
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf(" inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register and refCount==1 and is a aload+const so setForceFolding=true\n",
                  eventualNonConversion->getOpCode().getName(),eventualNonConversion);
         }
      self()->setForceFolding();
      return true;
...
void recursivelyDecrementIncrementedNodesIfUnderRegister(TR::CodeGenerator * cg, TR::Node * cur, List<TR::Node> & _incNodesList, List<TR::Node> &nodesToNotRecurse,
   ...
   for (listNode = listIt.getFirst(); listNode; listNode = listIt.getNext())
      ...
      if (listNode == cur)
         ...
         if (startRemoving)
            ...
            if (cg->traceBCDCodeGen())
               {
               comp->log()->printf("\t decReferenceCount on incremented node %p (%d) (%d->%d)\n",
   ...
   if (cur->getRegister() != NULL)
      ...
      if (cg->traceBCDCodeGen())
         {
         comp->log()->printf("\t %s [%p] (%d) has a register, so start removing under it\n",
...
bool OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator* cg, TR::Node* loadStore, TR::Node* topAdd)
   ...
   if (debug)
      comp->log()->printf("&&& TBID load=%llx addr1=%llx addr2=%llx\n",
   ...
   if (topAdd->getReferenceCount() == 1)
      {
      if (debug) comp->log()->prints("&&& TBID recursive decrement\n");
...
OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (_offset < MAXDISP &&               // skip if going to fixup naturally anyway
      ...
      if (_offset + maxAlignmentBump >= MAXDISP)
         {
         if (cg->traceBCDCodeGen())
            cg->comp()->log()->printf("\tz^z : node %s (%p) : _offset %d + maxAlignmentBump %d >= MAXDISP %d : (%d >= %d) -- force 4K fixup\n",
...
OMR::Z::MemoryReference::calcDisplacement(uint8_t * cursor, TR::Instruction * instr, TR::CodeGenerator * cg)
   ...
   if (self()->rightAlignMemRef())
      ...
      if (cg->traceBCDCodeGen())
         comp->log()->printf("instr %p : right aligning memRef with symRef #%d (%s, isTemp %s) _offset %d, totalSize %d : bump = ",
            instr,
   ...
   if (self()->leftAlignMemRef())
      ...
      if (cg->traceBCDCodeGen())
         comp->log()->printf("instr %p : left aligning memRef with symRef #%d (%s, isTemp %s), _offset %d : bump = totalSize - leftMostByte = %d - %d = ",
            instr,
            self()->getSymbolReference()->getReferenceNumber(),
            cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
            self()->getSymbolReference()->isTempVariableSizeSymRef() ? "yes":"no",
   ...
   if (self()->getSymbolReference()->isTempVariableSizeSymRef())
      {
      if (cg->traceBCDCodeGen())
         comp->log()->printf("instr %p : bumping tempVariableSizeSymRef memRef with symRef #%d (%s), _offset %d : bump = symSize - totalSize = %d - %d = ",
            instr,
            self()->getSymbolReference()->getReferenceNumber(),
            cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
            _offset,
      ...
      if ((displacement < MAXLONGDISP && displacement > MINLONGDISP)
         ...
         if (TR::MemoryReference::canUseTargetRegAsScratchReg(instr))
            ...
            if (!is2ndSSMemRef)
               ...
               //comp->log()->printf( "Using first spillReg %p \n", instr);
               }
            else
               {
               ...
               //comp->log()->printf( "Using another spillReg, increment spill slot %p \n", instr);
               }
            spillNeeded = true;
            }
         ...
         if (comp->getOption(TR_TraceCG))
            comp->log()->printf("[%p] Long Disp Inst using %s as scratch reg\n", instr, cg->getDebug()->getName(scratchReg));
         }
      }
...
OMR::Z::MemoryReference::doEvaluate(TR::Node * subTree, TR::CodeGenerator * cg)
   ...
   if (self()->forceFirstTimeFolding())
      {
      if (cg->traceBCDCodeGen())
         comp->log()->printf("\tforceFirstTimeFolding=true for subTree %s (%p) refCount %d, reg %s (reset firstTimeFlag for next doEvaluate)\n",
