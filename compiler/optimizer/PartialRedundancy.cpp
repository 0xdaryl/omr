...
static void fixDecimalPrecision(TR::Node *store, TR::Node *child, TR::Compilation *comp)
{
    if (child->getType().isBCD()) {
        ...
        if (comp->cg()->traceBCDCodeGen())
            comp->getLogger()->printf("%s setting precision on node:%p to be equal to node:%p\n", OPT_DETAILS, store, child);
    }
}
#endif
    ...
    int newNumChildren)
{
    ...
    if (comp()->cg()->traceBCDCodeGen())
        log->printf("%s", node->getOpCode().getName());

#ifdef J9_PROJECT_SPECIFIC
    if (wasBCDNonLoad && node->getOpCode().isBCDLoad()) {
        node->setHasSignStateOnLoad(true);
        if (comp()->cg()->traceBCDCodeGen())
            log->prints(" and setting hasSignState flag to true\n");
    } else {
        if (comp()->cg()->traceBCDCodeGen())
            log->println();
    }
                ...
                if (isSupportedOpCode(nullCheckReferenceNode, NULL) && !nullCheckReferenceOpCode.isLoadVarDirect()) {
                    if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT)
                        ...
                        if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] > -1)
                            ...
                            if (trace())
                                log->printf("Duplicate null check had its new null check reference %p replaced by %p "
                                            "with symRef #%d\n",
            ...
            if (shouldAllowOptimalSubNodeReplacement(comp())
                ...
                if (trace())
                    comp()->getLogger()->printf(
                        "Duplicate parent %p had its old child %p replaced by %p with symRef #%d\n", duplicateParent,
                        duplicateOptimalNode, newLoad, newLoad->getSymbolReference()->getReferenceNumber());
                if (duplicateParent->getOpCode().isNullCheck())
                    TR::Node::recreate(duplicateParent, TR::treetop);
            } else {
                if (trace())
                    comp()->getLogger()->printf("Note : Duplicate parent %p wanted to replace its child %p by possibly "
                                                "already available symRef but FAILED to do so\n",
