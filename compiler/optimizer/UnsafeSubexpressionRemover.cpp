...
void
OMR::UnsafeSubexpressionRemover::eliminateStore(TR::TreeTop *treeTop, TR::Node *node)
   {
   ...
   if (rootNode->getOpCode().isSpineCheck() && rootNode->getFirstChild() == node)
      ...
      if (isUnsafe(child))
         ...
         trprintf(trace(), comp()->log(), "  - replace unsafe child %s n%dn with dummy %s n%dn\n",
               child->getOpCode().getName(), child->getGlobalIndex(),
      ...
      if (node->getReferenceCount() >= 1)
         ...
         TR_ASSERT_FATAL_WITH_NODE(rootNode, (rootNode->getFirstChild() == node) && (rootNode->getOpCode().isCheck() || rootNode->getOpCodeValue() == TR::compressedRefs), "Expected rootNode n%dn to be a check operation or compressedRefs, and its child n%dn to be the store operation that is to be eliminated\n", rootNode->getGlobalIndex(), node->getGlobalIndex());
