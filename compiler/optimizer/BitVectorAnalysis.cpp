...
template<class Container>
void TR_BasicDFSetAnalysis<Container *>::allocateTempContainer(Container **result, Container *)
{
    *result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
template<class Container>
void TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfo(TR_RegionStructure *regionStructure,
    TR_BitVector &pendingList)
{
        ...
        if (node != regionStructure->getEntry()) {
            ...
            for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
                ...
                if (this->traceBVA()) {
                    ...
                    _currentRegularGenSetInfo->print(log, this->comp());
                    dumpOptDetails(this->comp(), "\n");
                    _currentRegularKillSetInfo->print(log, this->comp());
                    dumpOptDetails(this->comp(), "\n");
                }
            }
...
template<class Container>
bool TR_ForwardDFSetAnalysis<Container *>::analyzeNodeIfPredecessorsAnalyzed(TR_RegionStructure *regionStructure,
    TR_BitVector &pendingList)
{
    OMR::Logger *log = this->comp()->log();
...
template<class Container>
void TR_ForwardDFSetAnalysis<Container *>::analyzeNode(TR::Node *node, vcount_t visitCount,
    TR_BlockStructure *blockStructure, Container *analysisInfo)
{}
