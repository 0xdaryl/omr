...
bool OMR::CFGSimplifier::simplifyBoundCheckWithThrowException(bool needToDuplicateTree)
{
    ...
    if (secondCompareNode->getOpCodeValue() != TR::ificmplt && secondCompareNode->getOpCodeValue() != TR::ificmpge) {
        if (trace())
            log->prints("   Abort simplifyBoundCheckWithThrowException : The second compare block does not contain "
                        "ificmplt or ificmpge\n");
        return false;
    }
    ...
    if (!firstSymRef || firstSymRef != secondSymRef) {
        if (trace())
            log->prints("Abort simplifyBoundCheckWithThrowException : compare nodes uses different variables, pattern "
                        "not matched\n");
        return false;
    }
    ...
    if (trace())
        log->printf("   Removed %s block (block_%d) which either branches to throw block (block_%d) or normal block "
                    "(block_%d)\n",
