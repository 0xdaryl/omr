...
bool OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(bool needToDuplicateTree)
   ...
   if (trace())
      log->printf("   Found an ificmp[eq/ne] n%dn\n", compareNode->getGlobalIndex());
   ...
   if (trace())
      log->printf("   Found an ificmp[eq/ne] against zero n%dn\n", compareNode->getGlobalIndex());

   if (compareNode->getFirstChild()->getOpCodeValue() != TR::instanceof)
      return false;

   if (trace())
      log->printf("   Found an ificmp[eq/ne] of an instanceof against 0 n%dn\n", compareNode->getGlobalIndex());

   if (compareNode->getFirstChild()->getSecondChild()->getOpCodeValue() != TR::loadaddr)
      return false;

   if (trace())
      log->printf("   Found an ificmp[eq/new] of an instanceof a constant class against zero n%dn\n", compareNode->getGlobalIndex());
   ...
   if (compareNode->getOpCodeValue() == TR::ificmpeq)
      ...
      if (trace())
         log->prints("   Found an ificmpeq of an instanceof against zero which throws on taken size\n");
      throwBlock = _next2;
      ...
      if (trace())
         log->prints("   Found an ificmpne of an instance of against zero which throws on the fallthrough path\n");
      throwBlock = _next1;
   ...
   if (trace())
      log->printf("Remove compareTreeTop n%dn\n", compareTreeTop->getNode()->getGlobalIndex());
   ...
   if (trace())
      log->printf("Create checkcastAndNULLCHK Node n%dn\n", checkcastAndNULLCHKNode->getGlobalIndex());
   ...
   if (trace())
      log->prints("End simplifyInstanceOfTestToCheckcast.\n");
...
bool OMR::CFGSimplifier::simplifyBoundCheckWithThrowException(bool needToDuplicateTree)
   ...
   if (opCode != TR::ificmplt && opCode != TR::ificmpge)
      {
      if (trace())
         log->prints("   Abort simplifyBoundCheckWithThrowException : pattern not matched\n");
      return false;
      }
   ...
   if (trace())
      log->printf("   Matched with first %s in block_%d\n", isCompareLT ? "ificmplt" : "ificmpge",
                  _block->getNumber());
   ...
   if (secondCompareNode->getOpCodeValue() != TR::ificmplt && secondCompareNode->getOpCodeValue() != TR::ificmpge)
      {
      if (trace())
         log->prints("   Abort simplifyBoundCheckWithThrowException : The second compare block does not contain ificmplt or ificmpge\n");
      return false;
      }
   ...
   if (!patternMatched || !firstChildBndChk || !secondChildBndChk || throwBlock != secondThrowBlock)
      {
      if (trace())
         log->printf("Abort simplifyBoundCheckWithThrowException : %s\n", !patternMatched ? "Pattern did not matched" :
   ...
   if (!firstSymRef || firstSymRef != secondSymRef)
      {
      if (trace())
         log->prints("Abort simplifyBoundCheckWithThrowException : compare nodes uses different variables, pattern not matched\n");
      return false;
      }

   if (trace())
      log->printf("   Matched with second %s in block_%d\n", isSecondCompareLT ? "ificmplt" : "ificmpge",
   ...
   if (isLastBlockRetBlock)
      ...
      if (trace())
         log->printf("   Added a new goto node n%dn that branches to return block (block_%d)\n",
   ...
   if (trace())
      log->printf("   Removed %s block (block_%d) which either branches to throw block (block_%d) or normal block (block_%d)\n",
...
bool OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree)
   ...
   if (trace())
      {
      log->printf("   block%d triangle1: %d triangle2: %d\n", _block->getNumber(), triangle1, triangle2);
      }
   ...
   while (treeCursor->getNode()->getOpCode().isStoreDirect()
      ...
      if (trace())
         log->printf("Created select node n%dn\n", select->getGlobalIndex());
   ...
   if (trace())
      log->prints("End simplifyCondStoreSequence.\n");
...
bool OMR::CFGSimplifier::simplifySimpleStore(bool needToDuplicateTree)
   ...
   if (trace())
      {
      log->printf("   block%d triangle1: %d triangle2: %d diamond: %d\n", _block->getNumber(), triangle1, triangle2, diamond);
      }
   ...
   if (trace())
      log->prints("   compareTree has correctType\n");
   ...
   if (triangle2 || diamond)
      ...
      if (trace())
         log->prints("   Take side has an appropriate store as the first tree\n");
      ...
      if (trace())
         log->prints("   Value on taken side is not internal pointer\n");
      ...
      if (trace())
         log->prints("   Taken side checks out\n");
      }
   ...
   if (triangle1 || diamond)
      ...
      if (!treeCursor->getNode()->getOpCode().isStoreDirect()
         ...
         return false;
      if (trace())
         log->prints("   Fallthrough side has an appropriate store as the first tree\n");

      if (treeCursor->getNode()->getFirstChild()->isInternalPointer())
         return false;

      if (trace())
         log->prints("   Value on fallthrough side is not internal pointer\n");
      ...
      if (trace())
         log->prints("   Fallthrough checks out\n");
      }
   ...
   if (trace())
      log->prints("   StoreNode data type checks out\n");

   if (!diamond && !storeNode->getSymbolReference()->getSymbol()->isAutoOrParm())
      return false;

   if (trace())
      log->prints("   StoreNode symRef checks out\n");
   ...
   if (trace())
      log->printf("End simplifySimpleStore. New select node is n%dn\n", select->getGlobalIndex());
...
bool OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree)
   ...
   if (trace())
      log->printf("   Found an ifacmp[eq/ne] n%dn\n", compareNode->getGlobalIndex());
   ...
   if (trace())
      log->printf("   Matched nullBlock %d\n", nullBlock->getNumber());
   ...
   if (trace())
      log->printf("End simplifyNullToException. New NULLCHK node is n%dn\n", nullchkNode->getGlobalIndex());
...
bool OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree)
   ...
   if (trace())
      log->prints("   Control flow checks out\n");
   ...
   if (trace())
      log->printf("   Found a Compare node n%dn\n", compareNode->getGlobalIndex());
   ...
   if (!store1->getOpCode().isStore())
      return false;
   if (trace())
      log->printf("   Successor block_%d is single store\n", _next1->getNumber());
   ...
   if (value1->isInternalPointer() || value2->isInternalPointer())
      return false;
   if (trace())
      log->prints("   Store values are not internal pointers\n");
   ...
   if (store1->getSymbolReference()->getSymbol() != store2->getSymbolReference()->getSymbol())
      return false;
   if (trace())
      log->prints("   Store nodes opcode and symref checks out\n");
   ...
   if (valueIndex == 1) // indirect store, check base objects
      ...
      if (!checkEquivalentIndirectLoadChain(base1, base2))
         return false;
      if (trace())
         log->prints("   Indirect store base node opcode and symref checks out\n");
      }
   ...
   if (trace())
      log->prints("   Comparison values check out\n");
   ...
   if (trace())
      log->printf("End simplifyBooleanStore. New store node is n%dn\n", storeNode->getGlobalIndex());
   return true;
   }
