...
int32_t OMR::CFGSimplifier::perform()
   ...
   trprints(trace(), log, "Starting CFG Simplification\n");
...
bool OMR::CFGSimplifier::simplifyIfStructure()
   {
   trprintf(trace(), comp()->log(), "Attempting if simpliciaton on block_%d\n", _block->getNumber());
...
bool OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(bool needToDuplicateTree)
   ...
   trprintf(trace(), log, "Start simplifyInstanceOfTestToCheckcast block_%d\n", _block->getNumber());
   ...
   trprintf(trace(), log, "   Found an ificmp[eq/ne] n%dn\n", compareNode->getGlobalIndex());

   if (compareNode->getSecondChild()->getOpCodeValue() != TR::iconst
       || compareNode->getSecondChild()->getInt() != 0)
      return false;

   trprintf(trace(), log, "   Found an ificmp[eq/ne] against zero n%dn\n", compareNode->getGlobalIndex());

   if (compareNode->getFirstChild()->getOpCodeValue() != TR::instanceof)
      return false;

   trprintf(trace(), log, "   Found an ificmp[eq/ne] of an instanceof against 0 n%dn\n", compareNode->getGlobalIndex());

   if (compareNode->getFirstChild()->getSecondChild()->getOpCodeValue() != TR::loadaddr)
      return false;

   trprintf(trace(), log, "   Found an ificmp[eq/new] of an instanceof a constant class against zero n%dn\n", compareNode->getGlobalIndex());
   ...
   if (compareNode->getOpCodeValue() == TR::ificmpeq)
      ...
      trprints(trace(), log, "   Found an ificmpeq of an instanceof against zero which throws on taken size\n");
      throwBlock = _next2;
      ...
      trprints(trace(), log, "   Found an ificmpne of an instance of against zero which throws on the fallthrough path\n");
      throwBlock = _next1;
   ...
   trprintf(trace(), log, "Remove compareTreeTop n%dn\n", compareTreeTop->getNode()->getGlobalIndex());
   ...
   trprintf(trace(), log, "Create checkcastAndNULLCHK Node n%dn\n", checkcastAndNULLCHKNode->getGlobalIndex());
   ...
   trprints(trace(), log, "End simplifyInstanceOfTestToCheckcast.\n");
...
bool OMR::CFGSimplifier::simplifyBoundCheckWithThrowException(bool needToDuplicateTree)
   ...
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "Start simplifyBoundCheckWithThrowException\n");
   ...
   if (opCode != TR::ificmplt && opCode != TR::ificmpge)
      {
      trprints(trace(), log, "   Abort simplifyBoundCheckWithThrowException : pattern not matched\n");
      return false;
      }
   ...
   trprintf(trace(), log, "   Matched with first %s in block_%d\n", isCompareLT ? "ificmplt" : "ificmpge", _block->getNumber());
   ...
   if (secondCompareNode->getOpCodeValue() != TR::ificmplt && secondCompareNode->getOpCodeValue() != TR::ificmpge)
      {
      trprints(trace(), log, "   Abort simplifyBoundCheckWithThrowException : The second compare block does not contain ificmplt or ificmpge\n");
      return false;
      }
   ...
   if (!patternMatched || !firstChildBndChk || !secondChildBndChk || throwBlock != secondThrowBlock)
      {
      trprintf(trace(), log, "Abort simplifyBoundCheckWithThrowException : %s\n", !patternMatched ? "Pattern did not matched" :
   ...
   if (!firstSymRef || firstSymRef != secondSymRef)
      {
      trprints(trace(), log, "Abort simplifyBoundCheckWithThrowException : compare nodes uses different variables, pattern not matched\n");
      return false;
      }

   trprintf(trace(), log, "   Matched with second %s in block_%d\n", isSecondCompareLT ? "ificmplt" : "ificmpge", nextCompareBlock->getNumber());
   ...
   if (throwNode->getNumChildren() < 1 || throwNode->getFirstChild()->getOpCodeValue() != TR::athrow)
      {
      trprintf(trace(), log, "Abort simplifyBoundCheckWithThrowException : %s\n",
            throwNode->getNumChildren() < 1 ?
               "Expected throwNode does not contain children" :
   ...
   if (isLastBlockRetBlock)
      ...
      trprintf(trace(), log, "   Added a new goto node n%dn that branches to return block (block_%d)\n",
   ...
   trprintf(trace(), log, "   Removed %s block (block_%d) which either branches to throw block (block_%d) or normal block (block_%d)\n",
...
bool OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree)
   ...
   TR::Logger *log = comp()->log();
   trprintf(trace(), log, "Start simplifyCondStoreSequence block_%d\n", _block->getNumber());
   ...
   trprintf(trace(), log, "   block%d triangle1: %d triangle2: %d\n", _block->getNumber(), triangle1, triangle2);
   ...
   while (treeCursor->getNode()->getOpCode().isStoreDirect()
      ...
      trprintf(trace(), log, "Created select node n%dn\n", select->getGlobalIndex());
   ...
   trprints(trace(), log, "End simplifyCondStoreSequence.\n");
...
bool OMR::CFGSimplifier::simplifySimpleStore(bool needToDuplicateTree)
   ...
   TR::Logger *log = comp()->log();
   trprintf(trace(), log, "Start simplifySimpleStore block_%d\n", _block->getNumber());
   ...
   trprintf(trace(), log, "   block%d triangle1: %d triangle2: %d diamond: %d\n", _block->getNumber(), triangle1, triangle2, diamond);
   ...
   trprints(trace(), log, "   compareTree has correctType\n");
   ...
   if (triangle2 || diamond)
      ...
      trprints(trace(), log, "   Take side has an appropriate store as the first tree\n");

      trueValue = treeCursor->getNode()->getFirstChild();
      if (treeCursor->getNode()->getFirstChild()->isInternalPointer())
         return false;

      trprints(trace(), log, "   Value on taken side is not internal pointer\n");
      ...
      trprints(trace(), log, "   Taken side checks out\n");
      }
   ...
   if (triangle1 || diamond)
      ...
      if (!treeCursor->getNode()->getOpCode().isStoreDirect()
          ...
          || containsIndirectOperation(comp(), treeCursor))
         return false;
      trprints(trace(), log, "   Fallthrough side has an appropriate store as the first tree\n");

      if (treeCursor->getNode()->getFirstChild()->isInternalPointer())
         return false;

      trprints(trace(), log, "   Value on fallthrough side is not internal pointer\n");
      ...
      trprints(trace(), log, "   Fallthrough checks out\n");
      }
   ...
   trprints(trace(), log, "   StoreNode data type checks out\n");

   if (!diamond && !storeNode->getSymbolReference()->getSymbol()->isAutoOrParm())
      return false;

   trprints(trace(), log, "   StoreNode symRef checks out\n");
   ...
   trprintf(trace(), log, "End simplifySimpleStore. New select node is n%dn\n", select->getGlobalIndex());
...
bool OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree)
   ...
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "Start simplifyNullToException\n");
   ...
   trprintf(trace(), log, "   Found an ifacmp[eq/ne] n%dn\n", compareNode->getGlobalIndex());
   ...
   trprintf(trace(), log, "   Matched nullBlock %d\n", nullBlock->getNumber());
   ...
   trprintf(trace(), log, "End simplifyNullToException. New NULLCHK node is n%dn\n", nullchkNode->getGlobalIndex());
...
bool OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree)
   ...
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "Start simplifyBooleanStore\n");
   ...
   trprints(trace(), log, "   Control flow checks out\n");
   ...
   trprintf(trace(), log, "   Found a Compare node n%dn\n", compareNode->getGlobalIndex());
   ...
   if (!store1->getOpCode().isStore())
      return false;
   trprintf(trace(), log, "   Successor block_%d is single store\n", _next1->getNumber());
   ...
   if (value1->isInternalPointer() || value2->isInternalPointer())
      return false;
   trprints(trace(), log, "   Store values are not internal pointers\n");
   ...
   if (store1->getSymbolReference()->getSymbol() != store2->getSymbolReference()->getSymbol())
      return false;
   trprints(trace(), log, "   Store nodes opcode and symref checks out\n");
   ...
   if (valueIndex == 1) // indirect store, check base objects
      ...
      if (!checkEquivalentIndirectLoadChain(base1, base2))
         return false;
      trprints(trace(), log, "   Indirect store base node opcode and symref checks out\n");
      }
   ...
   trprints(trace(), log, "   Comparison values check out\n");
   ...
   trprintf(trace(), log, "End simplifyBooleanStore. New store node is n%dn\n", storeNode->getGlobalIndex());
   return true;
   }
...
bool OMR::CFGSimplifier::simplifyCondCodeBooleanStore(TR::Block *joinBlock, TR::Node *branchNode, TR::Node *store1Node, TR::Node *store2Node)
   ...
   if (compareNode->getOpCode().isCompareForEquality())
      ...
      {
      trprints(trace(), comp()->log(), "CFGSimplifier condCode pattern matches but uses test for ordering, not equality\n");
      return false;
      }
