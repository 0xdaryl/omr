...
void TR_VirtualGuardTailSplitter::initializeDataStructures()
   ...
   trprints(trace(), log, "Disjoint set forest:\n");
...
void TR_VirtualGuardTailSplitter::remergeGuard(TR_BlockCloner &cloner, VGInfo *info)
   ...
   TR::Logger *vcgLog = TR::CStdIOStreamLogger::create(file);
   comp->getDebug()->printVCG(vcgLog, _cfg, fileName);
   vcgLog->close();
   fclose(file);
#endif

   trprintf(trace(), comp()->log(), "Split Guard Block %d->(%d,%d), %d->(%d,%d)\n",
         block->getNumber(), blockA->getNumber(), blockB->getNumber(), cloneG->getNumber(), cloneA->getNumber(), cloneB->getNumber());
   }
...
TR_InnerPreexistence::transform()
   ...
   for (i = 0; i < _numInlinedSites; ++i)
      ...
      if (trace())
         ...
         log->printf("Site %d (block_%d, parent-block_%d): thisVN: %d, argsVNs: {",
