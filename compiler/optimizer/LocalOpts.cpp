...
int32_t TR_ExtendBasicBlocks::orderBlocksWithoutFrequencyInfo()
   ...
   for (; block; prevBlock = block, block = block->getNextBlock())
      ...
      if (trace())
         comp()->getLogger()->printf("    Current block [%d] looking for best successor to extend it...\n", prevBlock->getNumber());
...
TR::Block *TR_BlockManipulator::getBestChoiceForExtension(TR::Block *block1)
   ...
   for (auto nextEdge = block1->getSuccessors().begin(); nextEdge != block1->getSuccessors().end(); ++nextEdge)
      ...
      if (trace())
         comp()->getLogger()->printf("    Estimating hotness for BB [%d], next BB [%d], estimated hotness %d\n", block1->getNumber(), nextBlock->getNumber(), nextHotness);
   ...
   for (treeTop = startTree; (treeTop != endTree); treeTop = treeTop->getNextTreeTop())
      ...
      if ((lastRealNode->getOpCode().isBranch() &&
         ...
         while(seenAndNode)
            ...
            if (trace())
               comp()->getLogger()->trprintf("Comparing current AND node %p with old AND node\n", lastRealNode, andNode);
            ...
            // (a&b && a&c) ==> ((a & b|c) == b|c)
               ...
               {
               if (trace())
                   comp()->getLogger()->trprintf("Found two iand nodes: %p %p\n", andNode, lastRealNode);
         ...
         if (!seenAndBefore)
            ...
            if (!removingConditional)
               {
               if (trace())
                  comp()->getLogger()->trprintf("Adding new AND node %p\n", lastRealNode);
      ...
      if (!containsTreesOtherThanGoto && !block->getExceptionSuccessors().empty())
         ...
         while (!excSuccs.empty())
            ...
            if (trace())
               ...
               comp()->getLogger()->printf("Remove exception edge: block_%d (isValid %d) -> block_%d (isValid %d)\n", from->getNumber(), from->isValid(), to->getNumber(), to->isValid());
               }
...
bool TR_Rematerialization::examineNode(TR::TreeTop *treeTop, TR::Node *parent, TR::Node *node,
   ...
   if (node->getOpCodeValue() == TR::GlRegDeps)
    ...
    if (trace()) log->trprintf("Parent adjust %d node %p parent %p\n", adjustments.adjustmentFromParent, node, parent);
   ...
   for (i = 0; i < node->getNumChildren(); ++i)
      ...
      if ((child->getFutureUseCount() & 0x7fff) < child->getReferenceCount())
         {
         if (trace()) log->trprintf("Adding child %p to parent %p\n", child, node);
         seenChildren.set(i);
      ...
      if (maxChild)
         ...
         if (((maxChild->getFutureUseCount() & 0x7fff) == 0) &&
           ...
           if (maxChild->getOpCode().isFloatingPoint())
              ...
              {
              childAdjustment++;
              if (trace()) log->trprintf("Child adjust %d child %p node %p\n", childAdjustment, maxChild, node);
              }
            }
         }
      else
   ...
   if (node->getNumChildren() != seenChildren.getSize())
      {
      log->trprintf("Node %p num children %d seen children %d\n", node, node->getNumChildren(), seenChildren.getSize());
   ...
   */
   ...
   if ((regPress != NULL) &&
      ...
      if (node->getSecondChild()->getOpCode().isLoadConst() ||
         ...
         {
         if (trace()) log->trprintf("1Removing node %p\n", node);
         if (node->getOpCode().isFloatingPoint())
   ...
   if (removeNodes)
      {
      if (trace()) log->trprintf("2Removing node %p\n", node);
      if (node->getOpCode().isFloatingPoint())
            ...
            if (isRematerializableLoad(node, parent))
               ...
               if (node->getOpCode().isFloatingPoint())
                  ...
                  if (!state->_fpLoadsAlreadyVisitedThatCannotBeRematerialized.find(node))
                     {
                     // log->trprintf("1Adding node %p to _currentlyCommonedFPLoads\n", node);
                  ...
                  if (!state->_vector128LoadsAlreadyVisitedThatCannotBeRematerialized.find(node))
                     {
                     //log->trprintf("1Adding node %p to _currentlyCommonedVector128Loads\n", node);
                  ...
                  if (!state->_loadsAlreadyVisitedThatCannotBeRematerialized.find(node))
                     {
                     //log->trprintf("1Adding node %p to _currentlyCommonedLoads\n", node);
               ...
               if (node->getOpCode().isFloatingPoint())
                  {
                  //log->trprintf("2Adding node %p to _currentlyCommonedFPCandidates\n", node);
                  ...
                  {
                  //log->trprintf("2Adding node %p to _currentlyCommonedVector128Candidates\n", node);
                  ...
                  {
                  //log->trprintf("2Adding node %p to _currentlyCommonedCandidates\n", node);
...
void TR_Rematerialization::rematerializeNode(TR::TreeTop *treeTop, TR::Node *parent, TR::Node *node, vcount_t visitCount, List<TR::Node> *currentlyCommonedNodes, List<TR::Node> *currentlyCommonedCandidates, List< List<TR::Node> > *parents, List<TR::Node> *currentlyCommonedLoads, List< List<TR::Node> > *parentsOfCommonedLoads, List<TR::Node> *loadsAlreadyVisited, List<TR::Node> *loadsAlreadyVisitedThatCannotBeRematerialized, bool rematSpecialNode)
   {
   if (trace())
      comp()->getLogger()->trprintf("rematerializeNode: parent = %p node = %p\n", parent, node);
   ...
   if (isAdjunctNode)
      comp()->getLogger()->trprintf("Prevented adjunct node %p from being rematerialized\n", nodeToBeRematerialized);
   ...
   if (!isAdjunctNode && performTransformation(comp(), "%sRematerializing node %p(%s)\n", optDetailString(), nodeToBeRematerialized, nodeToBeRematerialized->getOpCode().getName()))
      ...
      if (nodeToBeRematerialized->getNumChildren() > 0)
         ...
         if (isRematerializable(nodeToBeRematerialized, firstChild) &&
             ...
             //log->trprintf("new node %p new first child %p\n", newNode, newFirstChild);
         ...
         if (parentNode /* &&
            ...
            for (int32_t j=0;j<parentNode->getNumChildren();j++)
               ...
               if (parentNode->getChild(j) == nodeToBeRematerialized)
                  {
                  if (trace())
                     comp()->getLogger()->trprintf("\tin parent node %p\n", parentNode);
...
int32_t TR_BlockSplitter::pruneAndPopulateBlockMapper(TR_LinkHeadAndTail<BlockMapper>* bMap, int32_t depth)
   ...
   if (depth)
      ...
      if (trace())
         ...
         for (BlockMapper* itr = bMap->getFirst(); itr; itr = itr->getNext())
            {
            log->trprintf("prune bMap iterator, from 0x%p to 0x%p\n", itr->_from, itr->_to);
            }
         }
      ...
      for(itr = bMap->getFirst(); itr; itr = itr->getNext())
         ...
         ++cloned;
         if (trace())
            log->trprintf("prune bMap iterator for join, from 0x%p to 0x%p\n", itr->_from, itr->_to);
...
bool TR_BlockSplitter::hasIVUpdate(TR::Block *block)
   ...
   for (OMR::TreeTop* treeTopItr = block->getEntry(); treeTopItr && treeTopItr != block->getExit()->getNextTreeTop(); treeTopItr = treeTopItr->getNextTreeTop())
      ...
      if (treeTopItr->getNode()->getOpCode().isStoreDirect()
         ...
         {
         if (trace())
            log->trprintf("    treetop %p has IVUpdate\n", treeTopItr->getNode());
         return true;
         }
      }
   if (trace())
...
bool TR_BlockSplitter::hasLoopAsyncCheck(TR::Block *block)
   ...
   if (trace())
      comp()->getLogger()->printf("   checking for loopAsyncCheck in block_%d\n", block->getNumber());
   for (OMR::TreeTop* treeTopItr = block->getEntry(); treeTopItr && treeTopItr != block->getExit()->getNextTreeTop(); treeTopItr = treeTopItr->getNextTreeTop())
      {
      if (treeTopItr->getNode()->getOpCodeValue() == TR::asynccheck)
         {
         if (trace())
            comp()->getLogger()->trprintf("    treetop %p is asncycheck\n", treeTopItr->getNode());
         return true;
         }
      }
   return false;
  ...
  // Now iterate over the trees, check if there is a store to any parm symbol that is in the
   ...
   for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL; p = parms.getNext())
       ...
       if ((koi != TR::KnownObjectTable::UNKNOWN)
          ...
          if (enableTrace)
             log->trprintf("PREX:      parm %d class %p is %.*s\n", index, clazz, len, sig);
   ...
   //so TR_InvariantArgumentPreexistence can validate preexistence args and propagate info onto parmSymbols
      ...
      if (enableTrace)
         log->trprintf("PREX:    Populating parmInfo of peeked method %s %p\n", feMethod->signature(trMemory()), peekInfo);
      ...
      if (argInfo)
         {
         if (enableTrace)
            log->trprintf("PREX:    Populating parmInfo of inlined method %s from argInfo %p\n", feMethod->signature(trMemory()), argInfo);
         ...
         for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL; p = parms.getNext())
            ...
            if (enableTrace)
               log->trprintf("PREX:      Parm %d is arg %p parmInfo %p\n", index, arg, &parmInfo);
...
bool TR_InvariantArgumentPreexistence::devirtualizeVirtualCall(TR::Node *node, TR::TreeTop *treeTop, TR_OpaqueClassBlock* clazz)
   ...
   if (!resolvedMethod)
      {
      if (trace())
         comp()->getLogger()->prints("Method is not resolved, can't devirtualize\n");
      return false;
      }
   ...
   if (!refinedMethod)
      {
      if (trace())
         comp()->getLogger()->trprintf("Can't find a method from class %p with offset %d\n", clazz, offset);
      return false;
      }
...
void TR_InvariantArgumentPreexistence::processIndirectCall(TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount)
   ...
   if (receiver->getOpCode().isLoadDirect())
      ...
      if (methodSymbol->isVirtual() || methodSymbol->isInterface())
         ...
         if (trace())
            log->trprintf("PREX:        Receiver is %p incoming Parm %d parmInfo %p\n", receiver, receiverParmOrdinal, existingInfo);
         }
      }
...
void TR_InvariantArgumentPreexistence::processIndirectLoad(TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount)
   ...
   if (trace())
      comp()->getLogger()->trprintf("PREX:        [%p] %s %s\n", node, node->getOpCode().getName(), node->getSymbolReference()->getName(comp()->getDebug()));

   if (addressChild->getSymbolReference()->isUnresolved())
      {
      if (trace())
         comp()->getLogger()->prints("PREX:          - unresolved\n");
      return;
      }
   ...
   if (addressChild->getSymbolReference()->hasKnownObjectIndex())
      ...
      ParmInfo *info = _parmInfo + index;
      if (trace())
         comp()->getLogger()->trprintf("PREX:          Indirect load through incoming Parm %d parmInfo %p\n", index, info);
   ...
   if (somethingMayHaveChanged && nodeToNullCheck)
      ...
      if (trace())
         comp()->getLogger()->trprintf("PREX:          Anchored [%p] formerly under %s [%p]\n", node, ttNode->getOpCode().getName(), ttNode);
      }
...
TR_ColdBlockMarker::isBlockCold(TR::Block *block)
   ...
   for (TR::PreorderNodeIterator iter(block->getFirstRealTreeTop(), comp()); iter != block->getExit(); ++iter)
      ...
      if (_notYetRunMeansCold && hasNotYetRun(node))
         ...
         if (trace())
            {
            comp()->getLogger()->trprintf("%s n%dn [%p] has not yet run\n", node->getOpCode().getName(), node->getGlobalIndex(), node);
            }
      ...
      if (  _notYetRunMeansCold
         ...
         if (calleeSymbol->getResolvedMethod()->isCold(comp(), node->getOpCode().isCallIndirect(), calleeSymbol))
            ...
            if (trace())
               {
               comp()->getLogger()->trprintf("Infrequent interpreted call node %p\n", node);
               }
...
TR_ColdBlockOutlining::perform()
   ...
   if (trace())
      {
      comp()->getLogger()->prints("After outlining cold Block ");
      orderBlocks.dumpBlockOrdering(comp()->getMethodSymbol()->getFirstTreeTop());
...
TR_ColdBlockOutlining::reorderColdBlocks()
   ...
   if (trace())
      comp()->getLogger()->printf("Cold Block Outlining: outlined %d cold blocks so far:\n", numBlocksSoFar);
   }
   ...
   if (ttNode->getOpCodeValue() == TR::treetop &&
      ...
      if (firstChild->getReferenceCount() == 1)
         ...
         if (!firstChild->getOpCode().hasSymbolReference() &&
            ...
            {
            if (trace())
               log->trprintf("\tfound trivially anchored ttNode %p with firstChild %s (%p -- refCount == 1)\n",
            ...
            for (int32_t i=0; i < firstChild->getNumChildren(); i++)
               ...
               if (!grandChild->getOpCode().isLoadConst() || grandChild->anchorConstChildren())
                  {
                  if (trace())
                     log->trprintf("\t\tcreate new treetop for firstChild->getChild(%d) = %s (%p)\n",i,grandChild->getOpCode().getName(),grandChild);
            ...
            if (trace())
               log->trprintf("\t\tremove trivially anchored ttNode %p with firstChild %s (%p) treetop\n",
                  ttNode,firstChild->getOpCode().getName(),firstChild);
            treeTop->unlink(true);
            }
         }
         ...
         {
         if (trace())
            log->trprintf("\tadd ttNode %p with firstChild %s (%p, refCount %d) to commonedTreeTopList\n",
...
TR_TrivialDeadTreeRemoval::postProcessTreetop(TR::TreeTop *treeTop, List<TR::TreeTop> &commonedTreeTopList, const char *optDetails, TR::Compilation *comp)
   ...
   if (treeTop->isPossibleDef())
      {
      if (trace())
         comp->getLogger()->trprintf("\tfound a possible def at node %p so clear _commonedTreeTopList list\n",treeTop->getNode());
...
TR_TrivialDeadTreeRemoval::processCommonedChild(TR::Node *child, TR::TreeTop *treeTop, List<TR::TreeTop> &commonedTreeTopList, const char *optDetails, TR::Compilation *comp)
   ...
   if (child->getReferenceCount() > 1)
      ...
      if (!commonedTreeTopList.isEmpty())
         ...
         if (trace())
            log->trprintf("commonedTreeTopList is not empty and found a commoned child %s (%p, refCount %d)\n",child->getOpCode().getName(),child,child->getReferenceCount());
         while (listTT)
            {
            ...
            //
            if (trace())
               log->trprintf("\tcomparing listTT %p with firstChild %s (%p) to commoned child %s (%p, refCount %d) (listTT == _currentTreeTop -- %s)\n",
...
TR_TrivialDeadTreeRemoval::transformBlock(TR::TreeTop * entryTree, TR::TreeTop * exitTree)
   ...
   if (trace())
      comp()->getLogger()->trprintf("TrivialDeadTreeRemoval on block_%d : entryTreeNode %p -> exitTreeNode %p\n",_currentBlock->getNumber(),entryTree->getNode(),exitTree->getNode());
