...
TR::VPConstraint *OMR::ValuePropagation::addConstraintToList(TR::Node *node, int32_t valueNumber, int32_t relative, TR::VPConstraint *constraint, ValueConstraints *valueConstraints, bool replaceExisting)
   ...
   if (!existingGlobalConstraint)
      ...
      if (!rel && numRelatives > maxRelDepth)
         ...
         _reachedMaxRelationDepth = true;
         if (trace())
            log->printf("===>Reached Max Relational Propagation Depth: %d\n", numRelatives);
         }
            ...
            for (storeRel = store->relationships.getFirst(), prev = NULL; storeRel; prev = storeRel, storeRel = storeRel->getNext())
               ...
               if (storeConstraint)
                  ...
                  if (storeConstraint != storeRel->constraint)
                     ...
                     if (trace() && node)
                        {
                        log->trprintf("   %s [%p] gets new store constraint:", node->getOpCode().getName(), node);
...
TR::VPConstraint *OMR::ValuePropagation::addGlobalConstraint(TR::Node *node, int32_t valueNumber, TR::VPConstraint *constraint, int32_t relative)
   ...
   if (trace() && c == NULL)
      log->trprintf("Cannot intersect constraints on %s [%p]", node->getOpCode().getName(), node);
   ...
   if (newOrChanged)
      ...
      if (trace() && node)
         {
         log->trprintf("   %s [%p] gets new global constraint:", node->getOpCode().getName(), node);
         rel->print(log, this, valueNumber, 1);
         }
      ...
      if (trace() && node)
         {
         log->trprintf("   %s [%p] found existing global constraint value number %d (%p): ", node->getOpCode().getName(), node, valueNumber, c);
         c->print(log, comp());
         log->println();
         }
      }
...
void OMR::ValuePropagation::createExceptionEdgeConstraints(uint32_t exceptions, ValueConstraint *extraConstraint, TR::Node *reason)
   ...
   for (auto edge= _curBlock->getExceptionSuccessors().begin(); edge != _curBlock->getExceptionSuccessors().end(); ++edge)
      ...
      if (trace())
         comp()->getLogger()->trprintf("   %s [%p] can throw exception to block_%d\n", reason->getOpCode().getName(), reason, target->getNumber());
...
TR::VPConstraint *OMR::ValuePropagation::getStoreConstraint(TR::Node *node, TR::Node *relative)
   ...
   if (rel)
      ...
      if (trace())
         {
         comp()->getLogger()->trprintf("   %s [%p] has existing store constraint:", node->getOpCode().getName(), node);
...
void OMR::ValuePropagation::checkTypeRelationship(TR::VPConstraint *lhs, TR::VPConstraint *rhs,
   ...
   if (lhs->asClass() && rhs->asClass())
      ...
      if (jlKlass && noPresenceResult)
         {
         if (trace())
            log->prints("presences are incompatible\n");
         result = 0;
         }
      else if (jlKlass && noTypeResult)
         {
         if (trace())
            log->prints("types are incompatible\n");
         result = 0;
         ...
         if (isInstanceOf || isCheckCast)
            ...
            if ((objectConstraint->isClassObject() == TR_yes) &&
               ...
               {
               if (trace())
                  log->prints("object is a classobject but cast maybe Class\n");
               result = value;
               }
            }
         }
         ...
         if (!objectClass->getClassType() &&
            ...
            {
            if (trace())
               log->prints("object is a classobject but cast is not a Class\n");
            result = 0;
            }
         /*
         ...
         */
            ...
            result = 0;
            if (trace())
               log->prints("object is not a classobject but cast is java/lang/Class\n");
            }
         ...
         //
             ...
             result = 1;
             if (trace())
                log->prints("object is a non-null classobject and cast is java/lang/Class\n");
             }
         }
      }
...
void OMR::ValuePropagation::invalidateParmConstraintsIfNeeded(TR::Node *node, TR::VPConstraint *constraint)
   ...
   if (symRef && symRef->getSymbol()->getParmSymbol())
      ...
      if (parmConstraint)
         {
         if (trace())
            comp()->getLogger()->trprintf("Checking compatibility of store node %p parm %d with value\n", node, parmNum);
         int32_t result = 1;
         checkTypeRelationship(parmConstraint, constraint, result, false, false);
         if (!result) // there is some incompatibility
            {
            if (trace())
               comp()->getLogger()->trprintf("   Store node %p to parm %d is not compatible with rhs, invalidating _parms entry %p\n", node, parmNum, _parmValues[parmNum]);
...
TR::VPConstraint *OMR::ValuePropagation::mergeDefConstraints(TR::Node *node, int32_t relative, bool &isGlobal, bool forceMerge)
   ...
   if (_defMergedNodes->get(node->getGlobalIndex()))
      {
      if (trace())
         log->printf("Node n%dn has already been processed by mergeDefConstraints - returning NULL\n", node->getGlobalIndex());
      return NULL;
      }
   ...
   if (!forceMerge && defNode)
      {
      if (trace())
         log->trprintf("   %s [%p] has singleDefiningLoad [%p] ", node->getOpCode().getName(), node, defNode);
      if (defNode->getVisitCount() >= _visitCount)
         {
         if (trace())
            log->prints("  whose constraints already computed\n");
         return NULL;
         }
      if (trace())
         log->prints("  going to merge\n");
      }
   ...
   for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne())
      ...
      if (defIndex < _useDefInfo->getFirstRealDefIndex())
         {
         if (trace())
            log->trprintf("   %s [%p] looking at method entry def point: ", node->getOpCode().getName(), node);
         ...
         if (node->getSymbol()->isAutoOrParm()
            ...
            {
            if (trace())
               log->printf("symRef %d is not stored on all paths - treating as unknown\n", defNode->getSymbolReference()->getReferenceNumber());
         ...
         //
         if (trace())
            log->trprintf("   %s [%p] looking at def value %d, %s [%p]: ", node->getOpCode().getName(), node, getValueNumber(defNode), defNode->getOpCode().getName(), defNode);
               ...
               if (defValueConstraint)
                  ...
                  if (rel)
                     ...
                     if (rel->constraint->asUnreachablePath())
                        ...
                        //
                        if (trace())
                           log->prints("def is unreachable (constraint), ignored\n");
                        continue;
                        }
                     isGlobal = false;
                     }
                  ...
                  if (!isDefInUnreachableBlock(defIndex))
                     ...
                     {
                     if (trace())
                        log->prints("def is unreachable (defInUnreachableBlock), ignored\n");
                     continue;
                     }
#if 0
      ...
      if (defIsUnseen)
         {
         if (trace())
            log->prints("def has not been seen yet\n");
         }
      else if (!defConstraint)
         {
         if (trace())
            log->prints("no constraint\n");
         constraint = NULL;
         unconstrained = true;
         }
      else
         ...
         if (mergeConstraint)
            {
            if (trace())
               log->trprintf(" merging constraint at def %p\n", defNode);
            if (constraint)
               {
            ...
            log->trprintf(" NOT merging constraint at def %p\n", defNode);
         }
      }
   ...
   if (!bigDecimalConstraints.isEmpty())
      ...
      if (constraint && mergeDeferredConstraints)
         ...
         for (c = cIt.getFirst(); c; c = cIt.getNext())
            {
            if (trace())
               log->trprintf(" doing late merging constraint at def %p\n", defNode);
   ...
   if (relative != AbsoluteConstraint)
      {
      if (trace())
         log->prints("we are processing a relative constraint and merging with backedge constraints is not supported\n");
      return NULL;
      }
   ...
   for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne())
      ...
      if (node->getSymbol()->isAutoOrParm()
         ...
         {
         if (trace())
            log->printf("symref %d is not defined on all paths - consulting backedge constraints\n", node->getSymbolReference()->getReferenceNumber());
         ...
         for (loopInfo = _loopInfo; loopInfo && loopInfo != container; loopInfo = loopInfo->_parent)
            {
            if (trace())
               log->trprintf("   %s [%p] looking at back edges in loop %d for def %s [%p]: ", node->getOpCode().getName(), node, loopInfo->_loop->getNumber(), defNode->getOpCode().getName(), defNode);
            ...
            if (!loopInfo ||
               ...
               {
               if (trace())
                  log->prints("not seen on this back edge, ignored\n");
               continue;
               }
            ...
            if (!hasBeenStored(defValueNumber, sym, loopInfo->_backEdgeConstraints->valueConstraints))
               {
               if (trace())
                  log->prints("no backedge constraint, ignored\n");
               continue;
               }
            ...
            if (defConstraint &&
               ...
               {
               if (trace())
                  log->prints("trivially equal, ignored\n");
               continue;
               }
            ...
            if (defConstraint->asUnreachablePath())
               {
               if (trace())
                  log->prints("def is unreachable, ignored\n");
               continue;
               }
            ...
            if (mergeConstraint)
               {
               if (trace())
                  log->trprintf(" mmerging constraint at def %p\n", defNode);
               if (constraint)
                  {
                  //
               ...
               log->trprintf(" NOT mmerging constraint at def %p\n", defNode);
            }
         }
   ...
   if (!bigDecimalConstraints.isEmpty())
      ...
      if (constraint && mergeDeferredConstraints)
         ...
         for (c = cIt.getFirst(); c; c = cIt.getNext())
            {
            if (trace())
               log->trprintf(" late mmerging constraint at def %p\n", defNode);
            constraint = constraint->merge(c, this);
            if (!constraint)
               {
               unconstrained = true;
...
void OMR::ValuePropagation::checkForInductionVariableIncrement(TR::Node *node)
   ...
   if (trace())
      log->trprintf("   %s [%p] may be induction variable [%p]\n", node->getOpCode().getName(), node, sym);
      ...
      if (!_useDefInfo->getUseDef(defs, useIndex))
         ...
         for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne())
            ...
            if (defIndex < _useDefInfo->getFirstRealDefIndex())
               ...
               isInductionVariable = false;
               if (trace())
                  log->trprintf("      ivInfo is invalid for sym [%p] as method entry is def point\n", sym);
               break;
               }
         ...
         for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne())
            ...
            if ((defVN != incrementVN) && !notInLoop)
               ...
               isInductionVariable = false;
               if (trace())
                  log->trprintf("      ivInfo is invalid for sym [%p], def [%d] node %p does not have the same value number as another increment\n", sym, defVN, defNode);
               break;
               }
            ...
            if (entryVN >= 0)
               ...
               if (_loopInfo->_seenDefs->get(defVN))
                  ...
                  isInductionVariable = false;
                  if (trace())
                     log->trprintf("      ivInfo is invalid for sym [%p], def [%d] already seen in loop\n", sym, defVN);
                  break;
                  }
               else
                  {
                  if (trace())
                     {
                     log->trprintf("      Sym [%p] has more than two defs, extra def due to VN [%d]\n", sym, entryVN);
            ...
            if ((defVN != entryVN) && notInLoop)
               ...
               isInductionVariable = false;
               if (trace())
                  log->trprintf("      ivInfo is invalid for sym [%p], def [%d] node %p is likely in some inner loop\n", sym, defVN, defNode);
               break;
               }
            }
         }
   ...
   if (iv)
      ...
      if (maybeInductionVariable)
         ...
         if (checkLoopTestBlock(sym))
            ...
            iv->_entryDef = NULL;
            if (trace())
               log->trprintf("      Guessed increment of the iv for sym [%p]\n", sym);
            }
         else
            if (trace())
               log->trprintf("      Could not guess the increment for sym [%p], not marked as induction variable\n", sym);
         }
...
void TR::GlobalValuePropagation::processNaturalLoop(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
   ...
   for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext())
      {
      if (trace())
         log->printf("   defined on all paths for exit %d->%d:", edge->getFrom()->getNumber(), edge->getTo()->getNumber());
...
void TR::GlobalValuePropagation::processBlock(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
   ...
   if (trace())
      {
      log->printf("GVP: Processing block_%i\n", _curBlock->getNumber());
      }
...
bool OMR::ValuePropagation::removeConstraints(int32_t valueNumber, ValueConstraints *vc, bool findStores)
   ...
   if (trace())
      log->prints("   Setting fall-through as unreachable\n");
   // mark current X as unreachable
   // X = block
   if (findStores)
      {
      if (trace())
         log->printf("   block following block_%d is unreachable\n", _curBlock->getNumber());
      setUnreachablePath();
      }
...
TR::TreeTop* TR::ArraycopyTransformation::createArrayNode(TR::TreeTop* tree, TR::TreeTop* newTree, TR::SymbolReference* srcRef, TR::SymbolReference* dstRef, TR::Node* len, TR::SymbolReference* srcObjRef, TR::SymbolReference* dstObjRef, bool isForward)
   ...
   if (!root->isReferenceArrayCopy())
      ...
      if (trace())
         comp()->getLogger()->printf("Created 3-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node), comp()->getDebug()->getName(root));
      }
   else
      {
      //src = TR::Node::createLoad(root, srcRef);
      ...
      if (trace())
         comp()->getLogger()->printf("Created 5-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node), comp()->getDebug()->getName(root));
      }
   ...
   for (ArrayLengthToVersion *arrayLength = blockInfo->_arrayLengths->getFirst(); arrayLength; arrayLength = arrayLength->getNext())
      ...
      for (ArrayIndexInfo *arrayIndex = arrayLength->_arrayIndicesInfo->getFirst(); arrayIndex; arrayIndex = arrayIndex->getNext())
         ...
         if (performTransformation(comp(), "%s Creating tests outside block_%d for versioning arraylenth %p \n", OPT_DETAILS, blockInfo->_block->getNumber(), arrayLength->_arrayLen))
            ...
            if (trace())
               comp()->getLogger()->trprintf("Second test - Creating %p (%s)\n", nextComparisonNode, nextComparisonNode->getOpCode().getName());
      ...
      for (ArrayIndexInfo *index = array->_arrayIndicesInfo->getFirst(); index; index = index->getNext())
         ...
         if (!index->_bndChecks->isEmpty() && !index->_bndChecks->isSingleton())
            ...
            for (TR::Node *bndchk = iter.getFirst(); bndchk; bndchk = iter.getNext())
               ...
               if (trace())
                  comp()->getLogger()->trprintf("Block versioner: Remove bndchk %p \n", bndchk );
                  ...
                  if (guard && (guard->getSuccessors().size() == 2) && guard->getExit())
                     ...
                     if (guardNode &&
                        ...
                        if (guardNode->getOpCodeValue() == TR::ifacmpne)
                           {
                           if (trace())
                              comp()->getLogger()->trprintf("Got guard [%p] as ifacmpne\n", guardNode);
                           guardType = 1; // change if to goto
                           }
                        else if (guardNode->getOpCodeValue() == TR::ifacmpeq)
                           {
                           if (trace())
                              comp()->getLogger()->trprintf("Got guard [%p] as ifacmpeq\n", guardNode);
                        ...
                        if (classTypeNode->getOpCodeValue() == TR::aconst &&
                           ...
                           if (trace())
                              comp()->getLogger()->trprintf("typeCompatibleStatus [%p] %d\n", guardNode, typeCompatibleStatus);
                           }
                        else
                           {
         ...
         if (!callGuarded || !callUnreachable)
            ...
            if (methodSymbol->isJNI())
               ...
               if (comp()->getMethodHotness() <= warm && comp()->getOption(TR_DisableInliningDuringVPAtWarm))
                  {
                  if (trace()) comp()->getLogger()->trprintf("\tDo not inline call at [%p]\n", callNode);
                  }
               else
                  {
#ifdef J9_PROJECT_SPECIFIC
