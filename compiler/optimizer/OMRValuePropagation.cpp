    ...
    bool forceMerge)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
            ...
            // value unconstrained.
            //
            trprintf(trace(), log, "   %s [%p] looking at def value %d, %s [%p]: ", node->getOpCode().getName(), node,
    ...
    bool insideLoop)
{
    ...
    for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
        trprintf(trace(), log, "   defined on all paths for exit %d->%d:", edge->getFrom()->getNumber(),
...
void OMR::ValuePropagation::Relationship::print(TR::Logger *log, OMR::ValuePropagation *vp, int32_t valueNumber,
    int32_t indent)
{
...
void OMR::ValuePropagation::StoreRelationship::print(TR::Logger *log, OMR::ValuePropagation *vp, int32_t valueNumber,
    int32_t indent)
{
    ...
    TR::SymbolReference *dstObjRef, bool isForward)
{
    ...
    if (!root->isReferenceArrayCopy()) {
        ...
        trprintf(trace(), log, "Created 3-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node),
        ...
        trprintf(trace(), log, "Created 5-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node),
            comp()->getDebug()->getName(root));
    }
