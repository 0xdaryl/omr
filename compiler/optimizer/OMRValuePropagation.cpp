    ...
    bool forceMerge)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
            ...
            //
            if (trace())
                log->printf("   %s [%p] looking at def value %d, %s [%p]: ", node->getOpCode().getName(), node,
        ...
        if (!_useDefInfo->getUseDef(defs, useIndex))
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if ((defVN != incrementVN) && !notInLoop) {
                    ...
                    isInductionVariable = false;
                    if (trace())
                        log->printf("      ivInfo is invalid for sym [%p], def [%d] node %p does not have the same "
                                    "value number as another increment\n",
                            sym, defVN, defNode);
                    break;
                }
    ...
    bool insideLoop)
{
    ...
    for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
        if (trace())
            log->printf("   defined on all paths for exit %d->%d:", edge->getFrom()->getNumber(),
