      ...
      for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge)
         ...
         if (liveOnSomePaths->_blockAnalysisInfo[toBlockNumber])
            ...
            if (isBackEdge)
               ...
               trprintf(trace, log, "    Adding backedge live vars from block_%d to LiveOnAllPaths for block_%d\n", toBlockNumber, b);
...
void TR_SinkStores::recordPlacementForDefAlongEdge(TR_EdgeStorePlacement *edgePlacement)
   ...
   trprintf(trace(), log, "            RECORD placement along edge (%d->%d), for tt [" POINTER_PRINTF_FORMAT "] (copy=%d)\n",
   ...
   if (_placementsForEdgesToBlock[toBlockNumber] != NULL)
      ...
      while (ptr != NULL)
         ...
         if (placementEdgeInfo != NULL)
            {
            trprints(trace(), log, "                adding tt to stores on this edge\n");
   ...
   if (ptr == NULL)
      ...
      trprints(trace(), log, "                edge isn't in list already\n");
      TR::Block *from=succEdge->getFrom()->asBlock();
      if (from->isGotoBlock(comp()))
         {
         trprintf(trace(), log, "                from block_%d is a goto block\n", from->getNumber());
...
void TR_SinkStores::recordPlacementForDefInBlock(TR_BlockStorePlacement *blockPlacement)
   ...
   trprintf(trace(), log, "            RECORD placement at beginning of block_%d for tt [" POINTER_PRINTF_FORMAT "] (copy=%d)\n", block->getNumber(), tt, (int32_t)copyStore);
...
void TR_SinkStores::lookForSinkableStores()
   ...
   trprints(trace(), log, "Starting to look for stores to sink\n");
   ...
   for (int32_t i=0;i < cfg->getBackwardTraversalLength();i++)
      ...
      trprintf(trace(), log, "  Looking for stores to sink in block_%d\n", blockNumber);
      ...
      if (block->getEntry() != NULL)
         ...
         do
            ...
            if (node && node->getOpCodeValue() == TR::BBStart)
               {
               trprints(trace(), log, "    Found BBStart, at beginning of block\n");
               break;
               }

            trprintf(trace(), log, "    Examining node [" POINTER_PRINTF_FORMAT "] in block_%d\n", node, blockNumber);

            if (node->exceptionsRaised() && block->hasExceptionSuccessors())
               {
               trprints(trace(), log, "    Raises an exception in try region, cannot move any earlier trees\n");
               foundException = true;
               }
            ...
            trprintf(trace(), log, "      calling findLocalUses on node %p with treeVisitCount %d\n", tt->getNode(), treeVisitCount);
            ...
            if (local != NULL)
               ...
               if (symIdx != INVALID_LIVENESS_INDEX)
                  ...
                  trprintf(trace(), log, "      setting %d in killedSymbols\n", symIdx);
            ...
            if (local &&
               ...
               trprintf(trace(), log, "      updating killedLiveCommonedLoads with savedLiveCommonedLoads, setting index %d\n", symIdx);
            ...
            if (trace())
              {
               ...
               while (bviKilledLiveCommoned.hasMoreElements())
                  ...
                  trprints(trace(), log, "      symbol stores to a live commoned reference, searching stores below that load this symbol via commoned reference ...\n");
                  _killMarkWalks++;
                  ...
                  while (storeElement != NULL)
                     ...
                     if (store->_movable &&
                        ...
                        if (trace())
                           {
                           if (!store->_movable)
                              log->printf("         marking store [" POINTER_PRINTF_FORMAT "] as unmovable because its tree depth is %d\n", store->_useOrKillInfo->_tt->getNode(), store->_depth);
                           else if (store->_needTempForCommonedLoads)
                              log->printf("         marking store [" POINTER_PRINTF_FORMAT "] as movable with temp because it has commoned reference used sym %d\n", store->_useOrKillInfo->_tt->getNode(), killedSymIdx);
                           }
                        }
               ...
               while (bviKilledSymbols.hasMoreElements())
                  ...
                  while (storeElement != NULL)
                     ...
                     if (store->_movable &&
                        ...
                        trprintf(trace(), log, "         marking store [" POINTER_PRINTF_FORMAT "] as having potentially unsafe load of sym %d\n", store->_useOrKillInfo->_tt->getNode(), killedSymIdx);
                        }
               ...
               while (storeElement != NULL)
                  ...
                  if (store->_movable &&
                     ...
                     if (store->_needTempForCommonedLoads->intersects(*firstRefsToKilledSymbols))
                        ...
                        if (trace())
                           {
                           log->printf("            found store [" POINTER_PRINTF_FORMAT "] below that may require a temp\n",store->_useOrKillInfo->_tt->getNode());
            ...
            if (local != NULL &&
               ...
               if (sym && sym->isMethodMetaData() && sym->castToMethodMetaDataSymbol()->getMethodMetaDataType() == TR_MethodMetaDataType_AR)
                  {
                  trprintf(trace(), log, "         not sinking the AR symbol %s\n", sym->castToMethodMetaDataSymbol()->getName());
                  canMoveStore = false;
                  }
               else if (foundException)
                  {
                  trprints(trace(), log, "         can't move store with side effect\n");
                  canMoveStore = false;
                  }
                  ...
                  {
                  trprints(trace(), log, "         can't move store because it fails sinking candidate test\n");
                  canMoveStore = false;
                  }
               else if (treeDepth > MAX_TREE_DEPTH_TO_MOVE)
                  {
                  trprintf(trace(), log, "         can't move store because the tree depth (%d) is too big (MAX_DEPTH = %d)\n", treeDepth, MAX_TREE_DEPTH_TO_MOVE);
                  canMoveStore = false;
                  }

               trprintf(trace(), log, "         store %s interfering and candidate tests\n", canMoveStore ? "passes some" : "fails one or more");
               ...
               if (canMoveStore &&
                  ...
                  if (treeDepth >= MIN_TREE_DEPTH_TO_MOVE)
                     ...
                     trprintf(trace(), log, "         moveStoreWithTemps = %s\n", moveStoreWithTemps ? "true" : "false");
                     }
                  else
                     {
                     canMoveStore = false;
                     trprintf(trace(), log, "         store will not be moved because its tree depth of %d is too small (MIN_DEPTH = %d)\n", treeDepth, MIN_TREE_DEPTH_TO_MOVE);
                     }
                  }
               }
            ...
            if (!usedSymbols->isEmpty() ||
               ...
               {
               trprintf(trace(), log, "      creating use or kill info on %s node [" POINTER_PRINTF_FORMAT "] to track kills and uses\n",
            ...
            if (canMoveStore)
               {
               trprints(trace(), log, "      store is potentially movable, collecting commoned loads and adding to list\n");
            ...
            if (movableStore == NULL)
               ...
               if (local &&
                  ...
                  {
                  trprintf(trace(), log, "      updating killedLiveCommonedLoads with treeCommonedLoads, setting index %d\n", symIdx);
                  // this store kills the currently live commoned loads
                  killedLiveCommonedLoads->set(symIdx);
                  }
               }
            ...
            for (int32_t tempCount = prevNumTemps; tempCount < _numTemps; tempCount++)
               {
               trprintf(trace(), log, "    skipping created temp store [" POINTER_PRINTF_FORMAT "] in backward treetop walk\n", tt->getNode());
               tt = tt->getPrevTreeTop();
               }
            }
      ...
      if (!block->isExtensionOfPreviousBlock())
         ...
         while (!useOrKillInfoList.isEmpty())
            ...
            if (isMovableStore)
               ...
               trprints(trace(), log, " is still movable\n");

               trprintf(trace(), log, "(Transformation #%d start - sink store)\n", _numTransformations);
                  TR::RegisterMappedSymbol *sym = tt->getNode()->getSymbolReference()->getSymbol()->getMethodMetaDataSymbol();
               if (performTransformation(comp(), "%s Finding placements for store [" POINTER_PRINTF_FORMAT "] %s with tree depth %d\n", OPT_DETAILS, tt->getNode(),
                  ...
                  if (sinkStorePlacement(store))
                     {
                     trprints(trace(), log, "        Successfully sunk past a non-live path\n");
                     movedStore = true;
                     _numRemovedStores++;
                     }
                  else
                     {
                     trprints(trace(), log, "        Didn't sink this store\n");
                     }
                  }

               trprintf(trace(), log, "(Transformation #%d was %s)\n", _numTransformations, performThisTransformation() ? "performed" : "skipped");
               _numTransformations++;
               }
            else
               {
               trprintf(trace(), log, " is not movable (isStore = %s)\n", isStore ? "true" : "false");
            ...
            if (movedStore)
               ...
               if (store->_commonedLoadsAfter)
                  ...
                  if (usesDataFlowAnalysis())
                     ...
                     if (0 && trace())
                        {
                        log->printf("      Update for movable store due to commonedLoad: LOAP->blockInfo[%d]=", blockNumber);
                   ...
                   if (trace())
                      {
                      log->printf("        symbolsUsed in block_%d is updated due to commoned loads.  Symbols USED BEF is ", blockNumber);
                      _symbolsUsedInBlock[blockNumber]->print(log, comp());
                      log->println();
                      }
                  (*_symbolsUsedInBlock[blockNumber]) |= (*store->_commonedLoadsAfter);
                   if (trace())
                      {
                      log->printf("        symbolsUsed in block_%d is updated due to commoned loads.  Symbols USED AFT is ", blockNumber);
                      _symbolsUsedInBlock[blockNumber]->print(log, comp());
                      log->println();
                      }
                   }
...
void TR_SinkStores::coalesceSimilarEdgePlacements()
   {
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "Trying to coalesce edge placements:\n");
   ...
   while (firstEdgePtr != NULL)
      ...
      if (trace())
         {
         log->printf("  Examining edge placement (%d,%d) with stores:", firstEdge->getFrom()->getNumber(), firstToBlock->getNumber());
         ListElement<TR_StoreInformation> *firstStorePtr = firstEdgePlacement->_stores.getListHead();
         while (firstStorePtr != NULL)
            {
            log->printf(" [" POINTER_PRINTF_FORMAT "](copy=%d)", firstStorePtr->getData()->_store->getNode(), firstStorePtr->getData()->_copy);
      ...
      while (secondEdgePtr != NULL)
         ...
         if (trace())
            {
            log->printf("    Comparing to edge placement (%d,%d) with stores:", secondEdge->getFrom()->getNumber(), secondToBlock->getNumber());
            ListElement<TR_StoreInformation> *secondStorePtr = secondEdgePlacement->_stores.getListHead();
            while (secondStorePtr != NULL)
               {
               log->printf(" [" POINTER_PRINTF_FORMAT "](copy=%d)", secondStorePtr->getData()->_store->getNode(), secondStorePtr->getData()->_copy);
         ...
         if (secondToBlock->getNumber() == firstToBlock->getNumber())
            ...
            trprints(trace(), log, "      stores have same destination block\n");
            ...
            if (firstStorePtr == NULL && secondStorePtr == NULL)
               {
               trprints(trace(), log, "      store lists are identical so coalescing\n");
               firstEdgePlacement->_edges.add(secondEdgeInfo);
               lastPtr->setNextElement(nextPtr);
               }
            else
               {
               trprints(trace(), log, "      store lists are different so cannot coalesce\n");
               lastPtr = secondEdgePtr;
               }
            }
         else
            {
            trprintf(trace(), log, "      destination blocks are different (%d,%d) so cannot coalesce\n", firstToBlock->getNumber(), secondToBlock->getNumber());
            lastPtr = secondEdgePtr;
            }
...
void TR_SinkStores::placeStoresInBlock(List<TR_StoreInformation> & stores, TR::Block *placementBlock)
   ...
   while (storePtr != NULL)
      ...
      if (0 && trace())
         comp()->log()->printf("        PLACE new store [" POINTER_PRINTF_FORMAT "] (original store [" POINTER_PRINTF_FORMAT "]) at beginning of block_%d\n", newStore->getNode(), store->getNode(), placementBlock->getNumber());
...
void TR_SinkStores::placeStoresAlongEdges(List<TR_StoreInformation> & stores, List<TR_EdgeInformation> & edges)
   ...
   if (!to->getExceptionPredecessors().empty())
      ...
      trprintf(trace(), log, "    block_%d is an exception handler, so creating new catch block\n", to->getNumber());
...
//      placementBlock = from->splitEdge(from, to, comp(), 0, insertionExit);
      ...
      trprintf(trace(), log, "    Split edge from %d to %d to create new split block_%d\n", from->getNumber(), to->getNumber(), placementBlock->getNumber());
      ...
      if (edgeInfo == NULL)
         ...
         for (; edgeInfo != NULL; edgeInfo = edgeInfoIt.getNext())
            ...
            TR::Block *thisTo=edge->getTo()->asBlock();
            TR_ASSERT(thisTo == to, "all edges on edgeList should be to same destination block");
            trprintf(trace(), log, "    changing normal edge (%d,%d) to (%d,%d)\n", from->getNumber(), to->getNumber(), from->getNumber(), placementBlock->getNumber());
...
void TR_SinkStores::doSinking()
   ...
   trprints(trace(), log, "Now performing store placements:\n");
      ...
      if (!placement->_stores.isEmpty())
         ...
         while (!placement->_stores.isEmpty())
            ...
            if (storeInfo->_copy)
               ...
               {
               TR::TreeTop *store=storeInfo->_store;
               trprintf(trace(), log, "    adding store [" POINTER_PRINTF_FORMAT "] to movedStores (block placement)\n", store);
               TR_ASSERT(!movedStores.find(store), "shouldn't find a moved store more than once");
               movedStores.add(store);
               }
            }
   ...
   while (!unsafeStores.isEmpty())
      ...
      if (movedStores.find(originalStore))
         {
         trprints(trace(), log, "  this store has been moved already, so no need to remove it\n");
         }
      else
         {
   ...
   while (!storesToRemove.isEmpty())
      ...
      trprintf(trace(), log, "Removing original store [" POINTER_PRINTF_FORMAT "]\n", originalStore->getNode());
   ...
   if (node->getOpCode().isLoadVarDirect())
      ...
      if (local != NULL)
         ...
         if (symIdx != INVALID_LIVENESS_INDEX && unsafeLoads->get(symIdx))
            ...
            trprintf(trace(), comp()->log(), "Found unsafe load of local %d in node [" POINTER_PRINTF_FORMAT "] n%dn\n", symIdx, node, node->getGlobalIndex());
            }
         }
      }
   else
...
bool TR_SinkStores::treeIsSinkableStore(TR::Node *node, bool sinkIndirectLoads, uint32_t &indirectLoadCount, int32_t &depth, bool &isLoadStatic, vcount_t visitCount)
   ...
   if (numChildren == 0)
      ...
      if (!(node->getOpCode().isLoadConst() || node->getOpCode().isLoadVarDirect())) //|| node->getOpCodeValue() == TR::loadaddr))
         {
         trprints(trace(), log, "      *not a load const or direct load*\n");
         return false;
         }
      if (node->getOpCode().isLoadVarDirect())
         {
         ...
         if (local == NULL)
            ...
            if (!sinkStoresWithStaticLoads())
               {
               trprints(trace(), log, "      *no local found on direct load*\n");
               return false;
               }
            else if (!node->getSymbolReference()->getSymbol()->isStatic())
               {
               trprints(trace(), log, "      *no local found on direct load and not a static load*\n");
               return false;
               }
      ...
      if (node->getOpCode().isStoreDirect() && node->isPrivatizedInlinerArg())
         {
         trprints(trace(), log, "         store is privatized inliner argument, not safe to move it\n");
         return false;
         }

      if (node->getOpCode().hasSymbolReference() && node->getSymbol()->holdsMonitoredObject())
         {
         trprints(trace(), log, "        store holds monitored object, not safe to move it\n");
         return false;
         }
      ...
      if (node->getOpCode().isStore() &&
         ...
         {
         trprints(trace(), log, "         can't move store of pinning array reference or with UseOnlyAliases\n");
         return false;
         }
      }
   ...
   if (!comp()->cg()->getSupportsJavaFloatSemantics() &&
      ...
      {
      trprints(trace(), log, "         fp store failure\n");
      return false;
      }
   ...
   if (numChildren == 0 &&
       ...
       (underCommonedNode || node->getReferenceCount() > 1))
      {
      trprintf(trace(), log, "         commoned static load store failure: %p\n", node);
      return false;
      }
...
bool TR_SinkStores::shouldSinkStoreAlongEdge(int symIdx, TR::CFGNode *block, TR::CFGNode *succBlock, int32_t sourceBlockFrequency, bool isLoadStatic, vcount_t visitCount, TR_BitVector *allEdgeInfoUsedOrKilledSymbols)
   ...
   if (isLoadStatic && blockContainsCall(succBlock->asBlock(), comp()))
      {
      trprintf(trace(), comp()->log(), "            Can't push sym %d to successor block_%d (static load)\n", symIdx, succBlock->getNumber());
      return false;
      }
...
bool TR_SinkStores::checkLiveMergingPaths(TR_BlockListEntry *blockEntry, int32_t symIdx)
   ...
   trprintf(trace(), log, "            Counting LONAP predecessors to compare to propagation count %d\n", blockEntry->_count);
   ...
   for (auto predEdge = inEdges.getFirst(); predEdge; predEdge = inEdges.getNext())
      ...
      if (_liveOnNotAllPaths->_outSetInfo[predBlockNumber]->get(symIdx))
         {
         trprintf(trace(), log, "              found LONAP predecessor %d\n", predBlockNumber);
         numPredsLONAP++;
         }
      }
...
TR_SinkStores::isCorrectCommonedLoad(TR::Node *commonedLoad, TR::Node *searchNode)
   ...
   if (commonedLoad == searchNode)
      {
      trprintf(trace(), comp()->log(), "           found commonedLoad = " POINTER_PRINTF_FORMAT "\n", commonedLoad);
      return true;
      }
...
// FIXME: does isCorrectCommonedLoad need to be called for all symbols or just TR_MethodMetaData symbols
         {
         trprintf(trace(), log, "(Transformation #%d start - create temp store)\n", _numTransformations);
         if (performTransformation(comp(),
            ...
            while (storeElement != NULL)
               ...
               if (store->_movable &&
                  ...
                  {
                  store->_movable = false;
                  trprintf(trace(), log, "\tmarking store candidate [" POINTER_PRINTF_FORMAT "] as unmovable because dependent temp store transformation #%d was skipped\n",
      ...
      if (symIdx != INVALID_LIVENESS_INDEX && needTempForCommonedLoads->get(symIdx))
         ...
         if (symRef0)
            ...
            trprintf(trace(), comp()->log(),
                  "         replacing symRef on duplicate node " POINTER_PRINTF_FORMAT " (of original node " POINTER_PRINTF_FORMAT ") with temp symRef " POINTER_PRINTF_FORMAT "\n",
...
bool TR_GeneralSinkStores::sinkStorePlacement(TR_MovableStore *movableStore)
   ...
   if (movableStore->_isLoadStatic && blockContainsCall(sourceBlock->asBlock(), comp()))
      {
      trprintf(trace(), log, "            TT[" POINTER_PRINTF_FORMAT "] Can't push sym %d to end of source block_%d (static load)\n", tt->getNode(), symIdx, sourceBlock->getNumber());
      return false;
      }
   if (!storeCanMoveThroughBlock(_symbolsKilledInBlock[sourceBlockNumber], _symbolsUsedInBlock[sourceBlockNumber], symIdx, allBlockUsedSymbols, allBlockKilledSymbols))
      {
      trprintf(trace(), log, "            TT[" POINTER_PRINTF_FORMAT "] Can't push sym %d to end of source block_%d\n", tt->getNode(), symIdx, sourceBlock->getNumber());
      return false;
      }
   else if (!(_liveOnNotAllPaths->_outSetInfo[sourceBlockNumber]->get(symIdx)))
      {
      trprintf(trace(), log, "            TT[" POINTER_PRINTF_FORMAT "] LONAP is false for sym %d at the end of source block_%d\n", tt->getNode(), symIdx, sourceBlock->getNumber());
      return false;
      }

   trprintf(trace(), log, "            TT[" POINTER_PRINTF_FORMAT "] Pushed sym %d to end of source block_%d\n", tt->getNode(), symIdx, sourceBlock->getNumber());
   ...
   for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext())
      ...
      if (_liveOnSomePaths->_blockAnalysisInfo[succBlock->getNumber()]->get(symIdx)
         ...
         if (!canSinkOutOfEBB && !isSuccBlockInEBB)
            {
            trprints(trace(), log, "              attempt to sink out of Ebb but commoned syms are killed in the Ebb => don't sink\n");
         ...
         if (!_sinkThruException && succBlock->asBlock()->isCatchBlock()
            ...
            {
            trprints(trace(), log, "              attempt to sink at the catch block => don't sink\n");
            placeDefInBlock = true;
            break;
            }
         if (shouldSinkStoreAlongEdge(symIdx, sourceBlock, succBlock, sourceBlockFrequency, movableStore->_isLoadStatic, visitCount, allEdgeInfoUsedOrKilledSymbols))
            {
            trprintf(trace(), log, "              added %d to worklist\n", succBlock->getNumber());
         ...
         trprints(trace(), log, "              symbol not live in this successor\n");
         ...
         if (isSymUsedInEdgePlacement(sourceBlock, succBlock))
            {
            placeDefInBlock = true;
            trprints(trace(), log, "              symbol is used in early edge store placement\n");
            break;
            }
         }
      }
   ...
   while (!worklist.isEmpty())
      ...
      int32_t blockNumber = block->getNumber();
      trprintf(trace(), log, "            TT[" POINTER_PRINTF_FORMAT "] trying to sink into block_%d\n", tt->getNode(), blockNumber);
      ...
      if (!checkLiveMergingPaths(blockEntry, symIdx))
         ...
         //       (which may be unsafe)
         trprints(trace(), log, "              didn't reach from all live predecessors, splitting edges it did reach on\n");
         ...
         if (_liveOnNotAllPaths->_inSetInfo[blockNumber]->get(symIdx) &&
            ...
            trprints(trace(), log, "              store sunk to end of block\n");
            ...
            for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext())
               ...
               trprintf(trace(), log, "            trying to push to block_%d\n", succBlockNumber);
               ...
               if (_liveOnSomePaths->_blockAnalysisInfo[succBlockNumber]->get(symIdx)
                  ...
                  //   if (trace())
                  //      log->prints("              attempt to sink out of Ebb but commoned syms are killed in the Ebb => don't sink\n");
                  //   placeDefInBlock = true;
                  //
            ...
            if (placeDefInBlock)
               ...
               if (numOfEdgePlacements)
                  {
                  trprints(trace(), log, "              all previously added edge store placements from this block will be removed\n");
         ...
         if (placeDefInBlock)
            ...
            trprints(trace(), log, "              store can't go through block\n");
