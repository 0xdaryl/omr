...
void TR_SinkStores::recordPlacementForDefInBlock(TR_BlockStorePlacement *blockPlacement)
   ...
   if (trace())
      log->printf("            RECORD placement at beginning of block_%d for tt [" POINTER_PRINTF_FORMAT "] (copy=%d)\n", block->getNumber(), tt, (int32_t)copyStore);
   ...
   for (int32_t i=0;i < cfg->getBackwardTraversalLength();i++)
      ...
      if (block->getEntry() != NULL)
         ...
         do
            ...
            if (trace())
               log->printf("    Examining node [" POINTER_PRINTF_FORMAT "] in block_%d\n", node,blockNumber);
            ...
            if (trace())
               log->printf("      calling findLocalUses on node %p with treeVisitCount %d\n",tt->getNode(),treeVisitCount);
               ...
               while (bviKilledSymbols.hasMoreElements())
                  ...
                  while (storeElement != NULL)
                     ...
                     if (store->_movable &&
                        ...
                        if (trace())
                           {
                           log->printf("         marking store [" POINTER_PRINTF_FORMAT "] as having potentially unsafe load of sym %d\n", store->_useOrKillInfo->_tt->getNode(), killedSymIdx);
                           }
                        }
            ...
            if (!usedSymbols->isEmpty() ||
               ...
               {
               if (trace())
                  log->printf("      creating use or kill info on %s node [" POINTER_PRINTF_FORMAT "] to track kills and uses\n",
            ...
            for (int32_t tempCount = prevNumTemps; tempCount < _numTemps; tempCount++)
               {
               if (trace())
                  log->printf("    skipping created temp store [" POINTER_PRINTF_FORMAT "] in backward treetop walk\n",tt->getNode());
               tt = tt->getPrevTreeTop();
               }
            }
...
void TR_SinkStores::placeStoresInBlock(List<TR_StoreInformation> & stores, TR::Block *placementBlock)
   ...
   while (storePtr != NULL)
      ...
      if (0 && trace())
         comp()->getLogger()->printf("        PLACE new store [" POINTER_PRINTF_FORMAT "] (original store [" POINTER_PRINTF_FORMAT "]) at beginning of block_%d\n", newStore->getNode(), store->getNode(), placementBlock->getNumber());
...
//               if (trace())
  //                log->printf("    adding store [" POINTER_PRINTF_FORMAT "] to movedStores (edge placement)\n", store);
      ...
      if (!placement->_stores.isEmpty())
         ...
         while (!placement->_stores.isEmpty())
            ...
            if (storeInfo->_copy)
               ...
               TR::TreeTop *store=storeInfo->_store;
               if (trace())
                  log->printf("    adding store [" POINTER_PRINTF_FORMAT "] to movedStores (block placement)\n", store);
               TR_ASSERT(!movedStores.find(store), "shouldn't find a moved store more than once");
               movedStores.add(store);
               }
            }
   ...
   while (!storesToRemove.isEmpty())
      ...
      if (trace())
         log->printf("Removing original store [" POINTER_PRINTF_FORMAT "]\n", originalStore->getNode());
   ...
   if (node->getOpCode().isLoadVarDirect())
      ...
      if (local != NULL)
         ...
         if (symIdx != INVALID_LIVENESS_INDEX && unsafeLoads->get(symIdx))
            ...
            if (trace())
               {
               comp()->getLogger()->printf("Found unsafe load of local %d in node [" POINTER_PRINTF_FORMAT "] n%dn\n", symIdx, node, node->getGlobalIndex());
               }
            }
         }
      }
...
bool TR_SinkStores::treeIsSinkableStore(TR::Node *node, bool sinkIndirectLoads, uint32_t &indirectLoadCount, int32_t &depth, bool &isLoadStatic, vcount_t visitCount)
   ...
   if (numChildren == 0)
      ...
      if (node->getOpCode().isLoadIndirect())
         ...
         if (sinkIndirectLoads)
            ...
            indirectLoadCount++;
            if (trace())
               log->printf("      found %s at %p so do not search children indirectLoadCount = %d\n", "indirect load" ,node,indirectLoadCount);
   ...
   if (numChildren == 0 &&
      ...
      {
      if (trace())
         log->printf("         commoned static load store failure: %p\n", node);
      return false;
      }
...
TR_SinkStores::isCorrectCommonedLoad(TR::Node *commonedLoad, TR::Node *searchNode)
   ...
   if (commonedLoad == searchNode)
      {
      if (trace())
         comp()->getLogger()->printf("           found commonedLoad = " POINTER_PRINTF_FORMAT "\n", commonedLoad);
      return true;
      }
         ...
         if (performTransformation(comp(),
            ...
            while (storeElement != NULL)
               ...
               if (store->_movable &&
                  ...
                  store->_movable = false;
                  if (trace())
                     comp()->getLogger()->printf("\tmarking store candidate [" POINTER_PRINTF_FORMAT "] as unmovable because dependent temp store transformation #%d was skipped\n", store->_useOrKillInfo->_tt->getNode(), _numTransformations);
                  }
               storeElement = storeElement->getNextElement();
               }
            }
   ...
   if (newNode->getOpCode().isLoadVarDirect() && newNode->getOpCode().hasSymbolReference() &&
      ...
      if (symIdx != INVALID_LIVENESS_INDEX && needTempForCommonedLoads->get(symIdx))
         ...
         if (symRef0)
            {
            if (trace())
               comp()->getLogger()->printf("         replacing symRef on duplicate node " POINTER_PRINTF_FORMAT " (of original node " POINTER_PRINTF_FORMAT ") with temp symRef " POINTER_PRINTF_FORMAT "\n", newNode, origNode, symRef0);
...
bool TR_GeneralSinkStores::sinkStorePlacement(TR_MovableStore *movableStore)
   ...
   if (movableStore->_isLoadStatic && blockContainsCall(sourceBlock->asBlock(), comp()))
      {
      if (trace())
         log->printf("            TT[" POINTER_PRINTF_FORMAT "] Can't push sym %d to end of source block_%d (static load)\n", tt->getNode(), symIdx, sourceBlock->getNumber());
      return false;
      }
   if (!storeCanMoveThroughBlock(_symbolsKilledInBlock[sourceBlockNumber], _symbolsUsedInBlock[sourceBlockNumber], symIdx, allBlockUsedSymbols, allBlockKilledSymbols))
      {
      if (trace())
         log->printf("            TT[" POINTER_PRINTF_FORMAT "] Can't push sym %d to end of source block_%d\n", tt->getNode(), symIdx, sourceBlock->getNumber());
      return false;
      }
   else if (!(_liveOnNotAllPaths->_outSetInfo[sourceBlockNumber]->get(symIdx)))
      {
      if (trace())
         log->printf("            TT[" POINTER_PRINTF_FORMAT "] LONAP is false for sym %d at the end of source block_%d\n", tt->getNode(), symIdx, sourceBlock->getNumber());
      return false;
      }

   if (trace())
      log->printf("            TT[" POINTER_PRINTF_FORMAT "] Pushed sym %d to end of source block_%d\n", tt->getNode(), symIdx, sourceBlock->getNumber());
   ...
   while (!worklist.isEmpty())
      ...
      int32_t blockNumber = block->getNumber();
      if (trace())
         log->printf("            TT[" POINTER_PRINTF_FORMAT "] trying to sink into block_%d\n", tt->getNode(), blockNumber);
