...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node *node)
   ...
   if ((primitiveTransform || referenceTransform) &&
               ...
               if (isValueTypeArrayFlatteningEnabled)
                  ...
                  if (isSrcArrayNullRestricted == TR_yes)
                     ...
                     if (isArrayElementFlattened(srcObject) == TR_yes)
                        {
                        if (trace())
                           log->printf("%s: n%dn %p dst array is identity type and source array is flattened VT array\n", __FUNCTION__, node->getGlobalIndex(), node);
      ...
      if (trace())
         log->printf("%s: n%dn %p transformTheCall %d isSrcArrayNullRestricted %d isDstArrayNullRestricted %d doRuntimeNullRestrictedTest %d\n", __FUNCTION__,
