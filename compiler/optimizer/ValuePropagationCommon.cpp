...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node *node)
   ...
   if ((primitiveTransform || referenceTransform) &&
      ...
      if (trace())
         log->trprintf("%s: n%dn %p transformTheCall %d isSrcArrayNullRestricted %d isDstArrayNullRestricted %d doRuntimeNullRestrictedTest %d\n", __FUNCTION__,
 ...
 slowBlock---> n39n      BBStart <block_-1>
         ...
         for (int32_t i = 0 ; i < oldCallNode->getNumChildren(); ++i)
            ...
            if (child->getOpCode().isLoadConst())
               ...
               if (trace())
                  log->trprintf("Created child n%dn %p #%d for old child n%dn %p of the original call node\n", savedChildNode->getGlobalIndex(), savedChildNode, newSymbolReference->getReferenceNumber(),
         ...
         if (trace())
            {
            log->trprintf("%s: n%dn %p current block_%d slowBlock block_%d newCallTree n%dn %p prevTT n%dn %p nextTT n%dn %p\n", __FUNCTION__, node->getGlobalIndex(), node,
