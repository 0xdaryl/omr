...
void TR_LiveVariableInformation::collectLiveVariableInformation()
   ...
   if (_includeParms)
      ...
      for (p = parms.getFirst(); p != NULL; p = parms.getNext())
         {
         if (traceLiveVarInfo())
            log->trprintf("#%2d : is a parm symbol at %p\n", _numLocals, p);
   ...
   for (p = locals.getFirst(); p != NULL; p = locals.getNext())
      {
      if (traceLiveVarInfo())
         log->trprintf("Local #%2d is symbol at %p\n",_numLocals,p);
...
void TR_LiveVariableInformation::visitTreeForLocals(TR::Node *node, TR_BitVector **blockGenSetInfo, TR_BitVector *blockKillSetInfo,
   ...
   if (traceLiveVarInfo())
      {
      if (movingForwardThroughTrees)
         log->trprintf("         Looking forward for uses in node %p has visitCount = %d and comp() visitCount = %d\n",
                       node, node->getVisitCount(), visitCount);
      else
         log->trprintf("         Looking backward for uses in node %p has future use count = %d and reference count = %d\n",
   ...
   if (local)
      ...
      if (traceLiveVarInfo() && local)
         log->trprintf("            Node [%p] local [%p] idx %d\n", node, local, localIndex);
   ...
   while (liveSymRefIt.hasMoreElements())
      ...
      while (it.hasMoreElements())
         ...
         if (local && liveLocalIndex != INVALID_LIVENESS_INDEX)
            {
            if (comp()->getOption(TR_TraceOSR))
               log->trprintf("set liveLocalIndex %d for definingSymbol %p definingSymRefNumber %d\n",local->getLiveLocalIndex(), symbol, definingSymRefNumber);
