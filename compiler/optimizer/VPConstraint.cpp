...
TR::VPConstraint * TR::VPConstraint::intersect(TR::VPConstraint *other, OMR::ValuePropagation *vp)
   ...
   if (!other)
      {
      trprintf(vp->trace(), log, "setIntersectionFailed to true because NULL constraint found this = 0x%p, other = 0x%p\n", this, other);
...
TR::VPConstraint *TR::VPNotEqual::propagateRelativeConstraint(TR::VPRelation *other, int32_t relative, int32_t otherRelative, OMR::ValuePropagation *vp)
   ...
   if (vp->trace())
      {
      vp->comp()->log()->printf("      Propagating V != value %d %+d and V == value %d %+d", relative, increment(), otherRelative, other->increment());
