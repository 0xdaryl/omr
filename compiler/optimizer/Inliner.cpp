...
TR_InlinerBase::performInlining(TR::ResolvedMethodSymbol * callerSymbol)
   ...
   if (inlinedSite)
      trprintf(comp()->trace(OMR::inlining), comp()->log(), "inlined some calls for method %s\n", comp()->signature());
...
TR_DumbInliner::tryToInline(const char *message, TR_CallTarget *calltarget)
   ...
   if (getPolicy()->tryToInline(calltarget, NULL, true))
      {
      trprintf(comp()->trace(OMR::inlining), comp()->log(), "tryToInline pattern matched; %s for %s\n", message, method->signature(comp()->trMemory()));
      return true;
      }
...
TR_InlineCall::inlineCall(TR::TreeTop * callNodeTreeTop, TR_OpaqueClassBlock * thisClass, bool recursiveInlining, TR_PrexArgInfo *argInfo, int32_t initialMaxSize)
   ...
   for(int32_t i = 0 ; i < callsite->numTargets() ; i++)
      ...
      trprintf(comp()->trace(OMR::inlining), comp()->log(), "inliner: Setting current inline depth=%d\n", currentInlineDepth);
         ...
         if (node->getOpCode().isCall() && (!node->getOpCode().isJumpWithMultipleTargets()) && node->getVisitCount() != _visitCount && node->getInlinedSiteIndex() == thisCallSite)
            ...
            if ((isCold
               ...
               if (inlineCount >= MAX_INLINE_COUNT)
                  {
                  trprintf(comp()->trace(OMR::inlining), comp()->log(), "inliner: stopping inlining as max inline count of %d reached\n", MAX_INLINE_COUNT);
                  break;
                  }
               }
   ...
   for(int32_t i=0;i<callsite->numTargets();i++)
      ...
      if ((byteCodeSize > maxBCSize))
         ...
         if (tryToInline("overriding getMaxBytecodeIndex check", calltarget))
            {
            trprints(trace, log, "inliner: overriding getMaxBytecodeIndex check\n");
            }
         else if (alwaysWorthInlining(calltarget->_calleeSymbol->getResolvedMethod(), callNode))
            {
            trprints(trace, log, "inliner: overriding getMaxBytecodeIndex check because it's always worth inlining\n");
            }
         else
            {
            trprintf(trace, log, "inliner: failed: getInitialBytecodeSize(%d) > %d for %s\n",
                  byteCodeSize, callStack->_maxCallSize, tracer()->traceSignature(calltarget->_calleeSymbol));
            trprintf(comp()->cg()->traceBCDCodeGen(), log, "q^q : failing to inline %s into %s (callNode %p on line_no=%d) due to wcode size\n",
...
TR_InlinerBase::addGuardForVirtual(
   ...
   if (!disableHCRGuards && comp()->getHCRMode() != TR::none && guard->_kind != TR_HCRGuard && !skipHCRGuardForCallee)
      ...
      trprintf(comp()->trace(OMR::inlining), log, "HCR guard method class is %p\n", methodClass);
      ...
      if (virtualGuard &&
         ...
         if (guardNode)
            ...
            if (virtualGuard &&
               ...
               trprintf(comp()->trace(OMR::inlining), log, "Merge HCR guard with virtual guard %p n%dn\n", guardNode, guardNode->getGlobalIndex());
               }
            }
         }
...
bool TR_InlinerBase::heuristicForUsingOSR(TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol, TR::ResolvedMethodSymbol *callerSymbol, bool isIndirectCall)
   ...
   while (osrMethodData)
      ...
      trprintf(comp()->getOption(TR_TraceOSR), comp()->log(),
...
TR::TreeTop * OMR_InlinerUtil::storeValueInATemp(
   ...
   trprintf(comp->cg()->traceBCDCodeGen(), comp->log(), "\tcreate storeNode %p of tempSymRef #%d (possibly for node uncommoning during opcodeExpansion)\n",
...
void TR_InlinerBase::applyPolicyToTargets(TR_CallStack *callStack, TR_CallSite *callsite)
   ...
   for (int32_t i=0; i<callsite->numTargets(); i++)
      ...
      if (calltarget->_calleeSymbol && strstr(calltarget->_calleeSymbol->signature(trMemory()), "FloatingDecimal"))
         ...
         trprintf(trace, log, "Reducing bytecode size to %d because it's method of FloatingDecimal\n", bytecodeSize);
         }
      ...
      if (!forceInline(calltarget) &&
         ...
         if (toInline)
            {
            trprintf(trace, log, "tryToInline pattern matched.  Skipping size check for %s\n", calltarget->_calleeMethod->signature(comp()->trMemory()));
         ...
         if (toInline)
            {
            trprintf(trace, log, "tryToInline pattern matched.  Within the size check for %s\n", calltarget->_calleeMethod->signature(comp()->trMemory()));
...
bool TR_InlinerBase::inlineCallTarget2(TR_CallStack * callStack, TR_CallTarget *calltarget, TR::TreeTop** cursorTreeTop, bool inlinefromgraph, int32_t)
   ...
   if (innerPrexInfo->hasInnerAssumptions() && guard->_kind == TR_NoGuard)
      {
      trprintf(trace, log, "%sIPREX virtualize the non-virtual call %s because of inner preexistence for inner calls\n",
   ...
   trprintf(trace, log, "inlined %i calls so far,  maxInlinedCalls:%i\n", comp()->getInlinedCalls(), comp()->getOptions()->getMaxInlinedCalls());
...
OMR_InlinerUtil::clearArgInfoForNonInvariantArguments(TR_PrexArgInfo* argInfo, TR::ResolvedMethodSymbol* methodSymbol, TR_InlinerTracer* tracer)
   ...
   trprints(tracePrex, log, "Clearing arg info for non invariant arguments\n");
   ...
   if (!methodSymbol->getFirstTreeTop())
      {
      trprints(tracePrex, log, "IL trees are not generated for method, no arg info is cleared\n");
      return;
      }
   ...
   for (TR::TreeTop * tt = methodSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop())
      ...
      if (ordinal < argInfo->getNumArgs() && argInfo->get(ordinal))
         ...
         if (valueNode->getOpCode().hasSymbolReference() &&
            ...
            {
            trprintf(tracePrex, log, "ARGS PROPAGATION: arg %d holds the same value after store node n%dn, keep argInfo %p", ordinal, storeNode->getGlobalIndex(), argInfo);
            }
         else
            {
            trprintf(tracePrex, log, "ARGS PROPAGATION: unsetting an arg [%i] of argInfo %p", parmSymbol->getOrdinal(), argInfo);
