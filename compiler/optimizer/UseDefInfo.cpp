...
void TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals, bool cannotOmitTrivialDefs, bool conversionRegsOnly)
   ...
   if (requiresGlobals || prefersGlobals)
      ...
      if (requiresGlobals && !canBuild)
         ...
         if (trace())
            log->prints("Use/Def info: cannot build global use/def info as it failed in indexSymbolsAndNodes\n");
         return;
         }
      }
   ...
   if (!canBuild)
      ...
      if (trace())
         log->prints("Use/Def info: cannot build global nor local use/def info as it failed in indexSymbolsAndNodes\n");
      return;
      }
   ...
   // log->printf("Growing useDefInfo to %d\n",getNumUseNodes());
   ...
   if (trace())
      log->prints("completed initialization of use/def info\n\n");
   ...
   if (node->getOpCode().isStoreDirect())
      ...
      if (symRef->getSymbol()->isAutoOrParm())
         ...
         if (!aux._onceReadSymbolsIndices[symRefNum].IsNull())
            ...
            if (trace())
               comp()->getLogger()->trprintf("SETTING node %p symRefNum %d\n", node, symRefNum);
            }
         ...
         if (aux._neverWrittenSymbols.get(symRefNum))
            ...
            if (trace())
               comp()->getLogger()->trprintf("Resetting write bit %d at node %p\n", symRefNum, node);
            ...
            if (!aux._onceWrittenSymbolsIndices[symRefNum].IsNull())
               ...
               if (trace())
                  comp()->getLogger()->trprintf("Sym ref %d written once at node %p\n", symRefNum, treeTop->getNode());
               }
            }
      ...
      if (symRef->getSymbol()->isAutoOrParm())
         ...
         if (aux._neverReadSymbols.get(symRefNum))
            ...
            if (trace())
               comp()->getLogger()->trprintf("Resetting read bit %d at node %p\n", symRefNum, node);
            }
            ...
            if ((prevLoadNode->getByteCodeIndex() != node->getByteCodeIndex()) ||
               ...
               if (trace())
                  comp()->getLogger()->trprintf("KILLING bit %d at node %p\n", symRefNum, node);
               }
            }
         }
      }
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData &aux)
      ...
      if (symRef)
         ...
         if (sym)
            ...
            if (!shouldIndexVolatileSym(symRef,aux))
               {
               if (trace())
                  log->trprintf("Ignoring Symbol [%p] because it is volatile %d or aliased to a volatile %d\n",sym,sym->isVolatile(),aux._volatileOrAliasedToVolatileSymbols.get(symRefNumber));
               continue;
               }
         ...
         if (!sym->isStatic() &&
            ...
            //
            if (trace())
               log->trprintf("Symbol [%p] has index %d\n", sym, _numSymbols);
   ...
   while (refIter.hasMoreElements())
      ...
      if (trace())
         {
         log->trprintf("Non Trivial symbol Ref [%p:%d] isTrivial=%d isParm=%d _neverWritten=%d",
      ...
      if (symRef &&
         ...
         if (trace())
            {
               log->trprintf("   sym=%p localIndex=%d \n", sym, sym->getLocalIndex());
            }
         if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX)
            ...
            if (trace())
               {
                  log->trprintf("Non Trivial symbol [%p] has index %d\n", sym, _numSymbols);
               }
            _sideTableToSymRefNumMap[_numSymbols] = symRefNumber;
            sym->setLocalIndex(_numSymbols++);
            _numNonTrivialSymbols++;
   ...
   while (refIter.hasMoreElements())
      ...
      if (symRef &&
         ...
         if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX)
            ...
            //
            if (trace())
               log->trprintf("Trivial symbol [%p] has index %d\n", sym, _numSymbols);
            sym->setLocalIndex(_numSymbols++);
            }
         }
      }
...
void TR_UseDefInfo::buildUseDefs(void *vblockInfo, AuxiliaryData &aux)
   ...
   for (int32_t j = getFirstUseIndex(); j <= lastUseIndex; j++)
      ...
      if (trace())
         ...
         log->trprintf("   Use #%d[%p] is defined by:\n",j,getNode(j));
         TR_UseDefInfo::BitVector::Cursor cursor(info);
         for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne())
            {
            int32_t defIndex = cursor;
            if (defIndex >= getFirstUseIndex())
               log->trprintf("      Single defining load #%d[%p]",defIndex,getNode(defIndex));
            else
               log->trprintf("      Def #%d[%p]",defIndex,getNode(defIndex));
...
void TR_UseDefInfo::buildUseDefs(TR::Node *node, void *vanalysisInfo, TR_BitVector &nodesToBeDereferenced, TR::Node *parent, AuxiliaryData &aux)
   ...
   if (node->getVisitCount() == visitCount)
      return;
   if (trace())
      log->trprintf("looking at node %p\n", node);
   ...
   if (node->getOpCode().hasSymbolReference() &&
      ...
      if (node->getOpCode().isLoadVarDirect() ||
         ...
         if (trace())
            log->trprintf("For node %p index = %d and first use index = %d\n", node, nodeIndex, getFirstUseIndex());
         if (aux._neverWrittenSymbols.get(node->getSymbolReference()->getReferenceNumber()))
            ...
            if (trace())
               log->trprintf("Reached here (entry) for use node %p\n", node);
            }
         else if (!aux._onceReadSymbolsIndices[node->getSymbolReference()->getReferenceNumber()].IsNull())
            {
            if (trace())
               log->trprintf("Use node %p is of a symbol read only once\n", node);
            ...
            {
            if (trace())
               log->trprintf("Use node %p is of a symbol write only once\n", node);
   ...
   if (isExpandedUseIndex(expandedNodeIndex) ||
      ...
      if (!aux._defsForSymbol[symIndex]->isEmpty())
         ...
         if (trace())
            {
            log->trprintf("defs for symbol %d node:%p \n", symIndex, node);
         ...
         *defs = *(aux._defsForSymbol[symIndex]);
         ...
         while (cursor.hasMoreElements())
            ...
            if (i >= _numDefsOnEntry)
               ...
               if (trace())
                  log->trprintf("reached by expanded index %d [0x%p]\n", i, defNode);
