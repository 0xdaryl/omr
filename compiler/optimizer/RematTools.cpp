...
TR_YesNoMaybe RematTools::gatherNodesToCheck(TR::Compilation *comp,
   ...
   if (opCode.hasSymbolReference() && !opCode.isLoad())
      {
      if (trace)
         log->trprintf("  priv arg remat: Can't fully remat [%p] due to [%p] - non-load with a symref", privArg, currentNode);
      return TR_no;
      }
      ...
      if (comp->requiresSpineChecks() && currentNode->getSymbol()->isArrayShadowSymbol())
         {
         if (trace)
            log->trprintf("  priv arg remat: Can't fully remat [%p] due to [%p] - array access needs spine check", privArg, currentNode);
      ...
      for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i)
         ...
         if (childResult == TR_no)
            {
            if (trace)
               log->trprintf("  priv arg remat: Can't fully remat [%p] due to [%p] - unsafe arithmetic child %d", privArg, currentNode, i);
      ...
      if (trace)
         log->trprintf("  guarded call remat: Can't fully remat [%p] due to [%p] - unhandled case", privArg, currentNode);
      return TR_no;
      }
   }
...
bool RematTools::walkTreesCalculatingRematSafety(TR::Compilation *comp,
   ...
   while (start != end)
      ...
      if (!getNextTreeTop(start, blocks, firstBlock))
         {
         if (trace)
            comp->getLogger()->trprintf("  remat tools: failed to follow path for remat safety at [%p]\n", start->getNode());
         return false;
         }
      }
...
bool RematTools::walkTreeTopsCalculatingRematFailureAlternatives(TR::Compilation *comp,
   ...
   while (start != end)
      {
      if (trace)
         log->trprintf("  priv arg remat: visiting [%p]: isStore %d privArg %d failedTargetMatch %d", start->getNode(),
      ...
      if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() &&
         ...
         {
         if (trace)
            log->trprintf("  priv arg remat: considering partial remat with node [%p]", start->getNode());
         for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter)
            {
            if ((*iter) &&
                ...
                {
                if (trace)
                   log->trprintf("  priv arg remat: attempting partial remat with node [%p] for [%p]", start->getNode(), (*iter)->getNode());
      ...
      if (!getNextTreeTop(start, blocks, firstBlock))
         {
         if (trace)
            log->trprintf("  remat tools: failed to follow path for failure alternatives at [%p]\n", start->getNode());
         return false;
         }
      }
