...
void TR_PrexArgInfo::dumpTrace()
   ...
   log->trprintf("<argInfo address = %p numArgs = %d>\n", this, getNumArgs());
   for (int i = 0 ; i < getNumArgs(); i++)
      ...
      if (arg && arg->getClass())
         ...
         log->trprintf("<Argument no=%d address=%p classIsFixed=%d classIsPreexistent=%d argIsKnownObject=%d koi=%d class=%p className= %s/>\n",
         i, arg, arg->classIsFixed(), arg->classIsPreexistent(), arg->hasKnownObjectIndex(), arg->getKnownObjectIndex(), arg->getClass(), className);
         }
      else
         {
         log->trprintf("<Argument no=%d address=%p classIsFixed=%d classIsPreexistent=%d/>\n", i, arg, arg ? arg->classIsFixed() : 0, arg ? arg->classIsPreexistent() : 0);
         }
      }
   log->prints("</argInfo>\n");
   }
