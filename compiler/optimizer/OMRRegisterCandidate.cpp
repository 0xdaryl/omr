...
OMR::RegisterCandidate::removeAssignedCandidateFromLoop(TR::Compilation *comp, TR_Structure *loop, int32_t regNum, TR_BitVector *liveOnEntryUsage, TR_BitVector *liveOnExitUsage, bool trace)
   ...
   for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock=blocksIt.getNext())
      ...
      trprintf(trace, comp()->log(), "Removed candidate #%d from block_%d entry and exit\n", getSymbolReference()->getReferenceNumber(), blockNumber);
...
OMR::RegisterCandidate::processLiveOnEntryBlocks(TR::Block * * blocks, int32_t *blockStructureWeight, TR::Compilation *comp,
   ...
   //
   if (lookForBlocksToRemove)
      trprints(trace, log, "Trim live ranges upward:\n");
   ...
   while (lookForBlocksToRemove)
      ...
      while (bvi.hasMoreElements())
         ...
         trprintf(trace, log, "\tExamining live-on-entry block_%d with loads and stores = %d\n", blockNumber, _blocks.getNumberOfLoadsAndStores(blockNumber));
         ...
         if (_blocks.getNumberOfLoadsAndStores(blockNumber) == 0)
            ...
            if (!liveOnExit)
               {
               trprintf(trace, log, "\tNot referenced and not live on exit, re-setting block %d \n", blockNumber);
               _liveOnEntry.reset(blockNumber);
               lookForBlocksToRemove = true;
               }
            }
   ...
   if ((maxFrequency > 1) &&
      ...
      while (lookForBlocksToRemove)
         ...
         trprintf(trace, log, "At frequency %d for %d numBlocks %d numUnreferencedBlocks %d\n",
                        ...
                        if (parent &&
                           ...
                           if (shouldResetThisLoop)
                              ...
                              if (trace)
                                 ...
                                 comp->getOptimizer()->getResetExitsGRA()->print(log, comp);
                                 }
...
//             || prevBlockTooRegisterConstrained(comp,block,blockGPRCount,blockFPRCount,blockVRFCount)
   ...
   while (bvi.hasMoreElements())
      ...
      if (!_liveOnEntry.get(blockNumber) &&
         ...
         trprintf(trace, log, "Increased weight of candidate %d by %d (new weight %d) because of live on exit from block_%d\n",
               ...
               if (((*succ)->getTo() != block->getNextBlock()) &&
                  ...
                  if (!_liveOnEntry.get((*succ)->getTo()->getNumber()))
                     ...
                     if (!exclude && blockStructure && (/* !dontAssignInColdBlocks(comp) || */ (!block->isCold() && !(*succ)->getTo()->asBlock()->isCold())) && !comp->getOptimizer()->getSeenBlocksGRA()->get(block->getNumber()) &&
                        ...
                        trprintf(trace, log, "Reduced weight of candidate %d by %d (new weight %d) because of end in live range at block_%d succ %d\n",
                   ...
                   if ((*succ)->getTo() != block->getNextBlock())
                      ...
                      if (_liveOnEntry.get((*succ)->getTo()->getNumber()))
                         ...
                         if (!exclude && blockStructure && (/* !dontAssignInColdBlocks(comp) || */ !block->isCold()) && !comp->getOptimizer()->getSeenBlocksGRA()->get(extBlockNumber))
                            ...
                            loadsAndStores = loadsAndStores - blockWeight;
                            reduction = reduction + blockWeight;
                            trprintf(trace, log, "Reduced weight of candidate %d by %d (new weight %d) because of start of live range at block_%d succ %d\n",
                                  getSymbolReference()->getReferenceNumber(), blockWeight, loadsAndStores, blockNumber, (*succ)->getTo()->getNumber());
                            foundEndOfRange = true;
                            break;
                            }
   ...
   if (numberOfBlocks && isPPS)
     ...
     if (trace)
      ...
      if (!comp->allocateAtThisOptLevel() &&
         ...
         {
         trprints(trace, log, "handle negative weight\n");
...
OMR::RegisterCandidate::extendLiveRangesForLiveOnExit(TR::Compilation *comp, TR::Block **blocks, TR_Array<TR::Block *>& startOfExtendedBBForBB)
   ...
   bool trace = comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
   trprints(trace, log, "Extending live ranges due to live on exits\n");
         ...
         for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e)
            ...
            do
               ...
               if (!_liveOnEntry.get(currBlock->getNumber()) &&  // not set already
                  ...
                  {
                  trprintf(trace, log, "\tFor candidate #%d, set live on block_%d entry due to live on exit\n", getSymbolReference()->getReferenceNumber(), currBlock->getNumber());
...
OMR::RegisterCandidates::candidatesOverlap(
   ...
   for (TR::TreeTop * tt = block->getFirstRealTreeTop(); tt; tt = tt->getNextTreeTop())
      ...
      if (entrySymbolInTree && seenExitSymbol)
         {
         trprintf(trace, comp()->log(), "Returning true in block_%d node %p entry cand %d exit cand %d\n",
               block->getNumber(), node, candidateOnEntry->getSymbolReference()->getReferenceNumber(),
...
scanPressureSimulatorCacheForConflicts(TR::RegisterCandidate *rc, TR_BitVector &blocks, TR_Array<TR_BitVector> &liveOnEntryConflicts, TR_Array<TR_BitVector> &liveOnExitConflicts, TR_Array<TR_BitVector> &entryExitConflicts, int32_t firstRegister, int32_t lastRegister, TR_BitVector *spilledRegs, TR::Compilation *comp, TR::Block * * all_blocks)
   ...
   if (trace)
     {
     log->printf("\tscanPressureSimulatorCacheForConflits: Candidate #%d\n", rc->getSymbolReference()->getReferenceNumber());
     log->prints("\t blocks candidate is live: ");
     blocks.print(log, comp);
     log->println();
     log->prints("\t   adding conflicts as reg#: { [Entry|Exit](block#) [Entry|Exit](block#) ...}\n");
     }
   ...
   while (blockIterator.hasMoreElements())
      ...
      if (trace)
        {
        log->printf("\t  Adding conflicts in block_%d\n",blockNum);
        log->prints("\t   spilled regs: ");
      ...
      while (regIterator.hasMoreElements())
         ...
         if (firstRegister <= regNum && regNum <= lastRegister)
            ...
            trprintf(trace, log, "\t    reg %d: {", regNum);
            ...
            while (rc->getBlocksLiveOnEntry().get(currBlockNum))
               ...
               trprintf(trace, log, " Entry(%d)", currBlockNum);
               ...
               trprintf(trace, log, " Exit(%d)", currBlockNum);
               }
   ...
   trprints(trace, log, "\t Update conflicts on entry due to conflicts on predecessor exits:\n");

   int32_t i;
   for (i = firstRegister; i <= lastRegister; ++i)
      {
      trprintf(trace, log, "\t    reg %d: {", i);
         ...
         for (auto e = block->getSuccessors().begin(); e != block->getSuccessors().end(); ++e)
            ...
            if (trace)
               ...
               if (!liveOnEntryConflicts[i].isSet(succ->getNumber()))
                  log->printf(" Entry(%d)<-Exit(%d)", succ->getNumber(), block->getNumber());
               }
      ...
      while (bvi.hasMoreElements())
         ...
         for (auto e = block->getSuccessors().begin(); e != block->getSuccessors().end(); ++e)
            ...
            if (trace)
               if (!liveOnEntryConflicts[i].isSet(succ->getNumber()))
                  log->printf(" Entry(%d)<-EntryExit(%d)", succ->getNumber(), block->getNumber());
   ...
   if (trace)
      {
      log->prints("\tDone scanning for conflicts\n");
      TR_BitVectorIterator regIterator(*spilledRegs);
      while (regIterator.hasMoreElements())
         {
         int32_t regNum = regIterator.getNextElement();
         if (firstRegister <= regNum && regNum <= lastRegister)
            {
            log->printf("\tFor candidate #%d, on entry block conflicts with register %d ", rc->getSymbolReference()->getReferenceNumber(), regNum);
            liveOnEntryConflicts[regNum].print(log, comp);
            log->println();
            log->printf("\tFor candidate #%d, on exit block conflicts with register %d ", rc->getSymbolReference()->getReferenceNumber(), regNum);
            liveOnExitConflicts[regNum].print(log, comp);
            log->println();
            }
         }
      }
...
OMR::RegisterCandidates::assign(TR::Block ** cfgBlocks, int32_t numberOfBlocks, int32_t & lowestNumber, int32_t & highestNumber)
   ...
   for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next)
      ...
      if (comp()->getOption(TR_EnableGRACostBenefitModel))
         ...
         if ((rc->getWeight() < unsigned(weightThreshold)) && (((float) numAssigns) > (float) (numCandsThresholdFactor * (float) numCands)))
            {
            trprints(trace, log, "Leaving candidate because the compile time cost is not worth it\n");
      ...
      if (type.isInt64() && cg->getDisableLongGRA())
         {
         trprints(trace, log, "Leaving candidate because LongGRA is disabled and candidate is 64 bit\n");
         continue;
         }

      if (dt == TR::Aggregate && (1 || rc->getSymbolReference()->getSymbol()->getSize() > 8))
         {
         trprints(trace, log, "Leaving candidate because its an aggregate and > 64 bits\n");
         continue;
         }
      ...
      if ((!rc->getSymbolReference()->getSymbol()->isAutoOrParm())
         ...
         {
         trprints(trace, log, "Leaving candidate because it holdsMonitoredObject\n");
      ...
      if (aliasesPreventAllocation(comp(),rc->getSymbolReference()))
         {
         trprints(trace, log, "Leaving candidate because it has use_def_aliases\n");
         continue;
         }

      if (dt.isVector() && !comp()->cg()->hasGlobalVRF())
         {
         trprints(trace, log, "Leaving candidate because it has vector type but no global vector registers provided\n");
      ...
      if (trace) // Feel this would be useful by default
         ...
         rc->getBlocksLiveOnEntry().print(log, comp());
         dumpOptDetails(comp(), "\n");
         dumpOptDetails(comp(), "live on exit : ");
         rc->getBlocksLiveOnExit().print(log, comp());
         dumpOptDetails(comp(), "\n");
         }
      //
      ...
      if (trace)
         {
         log->prints("available registers : ");
         availableRegisters.print(log, comp());
         log->println();
         }
      TR::CodeGenerator * cg = comp()->cg();
      ...
      if (enableVectorGRA)
         ...
         for (int i = firstRegister; i <= lastRegister; ++i)
            ...
            if ((cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i) &&
               ...
               trprintf(trace, log, "removing %s (%d) from available list since it is aliased with %s (%d)\n",
     ...
     if ((registerNumber == -1) ||
        ...
        if (!rc->canBeReprioritized()) // only reprioritize a certain number of times based on hotness
           {
           trprints(trace, log, "Can't reprioritize anymore\n");
        ...
        //
           ...
           for (i = firstRegister; i <= lastRegister; ++i)
              {
              trprintf(trace, log, "Searching for blocks/structures with max frequency for reg: %d\n", i);
        ...
        // This code section moved to scanPressureSimulatorCacheForConflicts
              ...
              if (trace)
                 {
                 log->printf("Final live on entry conflicts with register : %d\n", i);
                 _liveOnEntryConflicts[i].print(log, comp());
                 log->println();
                 }
              ...
              if (!needs2Regs)
                 ...
                 trprintf(trace, log, "block_%d has max freq (%d)\n", maxBlock, maxFrequency);
              ...
              if (needs2Regs)
                 ...
                 if (trace)
                    ...
                    log->printf("max freq %d; smallest max freq %d\n", maxFrequency, smallestMaxFrequency);
                    log->printf("num clear structures %d; max clear structures %d\n", numClearStructures, maxClearStructures);
                    log->printf("num blocks at max freq %d;  min num conflicts %d\n", numBlocksAtMaxFrequency, numberOfConflicts);
                    }
           ...
           if (conflictingRegister != -1)
              ...
              if (!blocksToBeFreed.isEmpty())
                 ...
                 if (trace)
                    ...
                    blocksToBeFreed.print(log, comp());
                    log->println();
                    }
   ...
   while (entryBlocksIt.hasMoreElements())
     ...
     while (bvi.hasMoreElements())
        ...
        if (b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() != rc)
           ...
           if ((b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() != NULL) &&
              ...
              {
              trprintf(trace, log, "Candidate %d instead of candidate %d is required in register %d on exit out of block_%d\n",
        ...
        if (needs2Regs)
           ...
           if (b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit() != rc)
             ...
             if ((b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit() != NULL) &&
                 ...
                 {
                 trprintf(trace, log, "Candidate %d instead of candidate %d is required in register %d on exit out of block_%d\n",
                       b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit()->getSymbolReference()->getReferenceNumber(),
     ...
     if (trace)
        {
        log->printf("After assigning candidate %d real register %d, entry usage : \n", rc->getSymbolReference()->getReferenceNumber(), registerNumber);
        _liveOnEntryUsage[registerNumber].print(log, comp());
        log->println();
        }
     ...
     if (trace)
        {
        log->printf("After assigning candidate %d real register %d, exit usage : \n", rc->getSymbolReference()->getReferenceNumber(), registerNumber);
        _liveOnExitUsage[registerNumber].print(log, comp());
        log->println();
        }
     ...
     if (needs2Regs)
        ...
        if (trace)
           ...
           _liveOnEntryUsage[highRegisterNumber].print(log, comp());
           log->println();
           }
...
OMR::RegisterCandidates::computeAvailableRegisters(TR::RegisterCandidate *rc, int32_t firstRegister, int32_t lastRegister, TR::Block * * blocks, TR_BitVector *availableRegisters)
   ...
   for (i = firstRegister; i <= lastRegister; ++i)
      ...
      if (trace)
         ...
         liveOnExitConflicts.print(log, comp());
         log->prints("\nentry exit conflicts : ");
         entryExitConflicts.print(log, comp());
         log->prints("\nexit entry conflicts : ");
         exitEntryConflicts.print(log, comp());
         log->println();
         }
