    ...
    , TR_LocalAnalysis(info, t)
{
    ...
    logprints(trace(), log, "Starting LocalAnticipatability\n");
    ...
    *_seenCallSymbolReferences |= methodsBV;
    ...
    for (block = comp()->getStartBlock(); block != NULL; block = block->getNextBlock()) {
        ...
        if (trace()) {
            ...
            binfo->_analysisInfo->print(log, comp());
            binfo->_downwardExposedAnalysisInfo->print(log, comp());
            binfo->_downwardExposedStoreAnalysisInfo->print(log, comp());
        }
    }
    ...
    ContainerType *storeNodes, vcount_t visitCount)
{
    ...
    if (visitCount <= node->getVisitCount()) {
        if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)
            ...
            if ((opCode.isLoad() || node->getOpCodeValue() == TR::loadaddr) && !node->isDataAddrPointer()) {
                ...
                if (!adjustInfoForAddressAdd(node, node->getFirstChild(), seenDefinedSymbolReferences,
                        seenStoredSymbolReferences, killedExpressions, storeNodes, block))
                    return false;
            } else
                return false;
        }
    ...
    if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)
        ...
        if ((opCode.isLoad() || opCode.getOpCodeValue() == TR::loadaddr) && !node->isDataAddrPointer()) {
            if (opCode.hasSymbolReference() && (loadaddrAsLoad() || opCode.getOpCodeValue() != TR::loadaddr)) {
                if (seenDefinedSymbolReferences->get(node->getSymbolReference()->getReferenceNumber()))
                    return false;
                if (seenStoredSymbolReferences->get(node->getSymbolReference()->getReferenceNumber())) {
    ...
    ContainerType *killedExpressions, ContainerType *storeNodes, TR::Block *block)
{
    ...
    if (childHasSupportedOpCode) {
        ...
        if ((childOpCode.isLoad() || child->getOpCodeValue() == TR::loadaddr) && !child->isDataAddrPointer()) {
