...
/*
   for (treeTop = comp()->getStartTree(); treeTop != NULL; treeTop = treeTop->getNextTreeTop())
      ...
      if (node->getOpCodeValue() == TR::BBStart)
         ...
         if (trace())
            {
            comp()->getLogger()->trprintf("analysisInfo at node %p \n", node);
...
*/
   if (trace())
      log->prints("Just before buildOSRDefs\n");
...
void TR_OSRDefInfo::buildOSRDefs(TR::Node *node, void *vanalysisInfo, TR_OSRPoint *osrPoint, TR_OSRPoint *osrPoint2, TR::Node *parent, AuxiliaryData &aux)
   ...
   if (parent == NULL)
      ...
      if (trace())
         {
         log->trprintf("analysisInfo at node %p \n", node);
      ...
      if (osrPoint != NULL)
         ...
         *aux._defsForOSR[osrIndex] |= *analysisInfo;
         if (trace())
            {
            log->trprintf("_defsForOSR[%d] at node %p \n", osrIndex, node);
      ...
      if (osrPoint2 != NULL)
         ...
         *aux._defsForOSR[osrIndex] |= *analysisInfo;
         if (trace())
            {
            log->trprintf("_defsForOSR[%d] after node %p \n", osrIndex, node);
            aux._defsForOSR[osrIndex]->print(log, comp());
            log->println();
            }
         }
...
int32_t TR_OSRDefAnalysis::perform()
   ...
   if (comp()->getOption(TR_EnableOSR))
      ...
      if (comp()->getOption(TR_DisableOSRSharedSlots))
         {
         if (trace())
            log->prints("OSR is enabled but OSR def analysis is not.\n");
         return 0;
         }
      ...
      if (!comp()->canAffordOSRControlFlow())
         {
         if (trace())
            log->prints("OSR is enabled but no longer in use for this compilation.\n");
         return 0;
         }
      }
   else
      {
      if (trace())
         log->prints("Options is not enabled -- returning from OSR reaching definitions analysis.\n");
      return 0;
      }
   ...
   if (trace())
      {
      log->printf("%s OSR reaching definition analysis is required\n",
         optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
      }
...
bool TR_OSRLiveRangeAnalysis::shouldPerformAnalysis()
   ...
   if (!comp()->getOption(TR_EnableOSR))
      {
      if (traceOSR)
         log->prints("Should not perform OSRLiveRangeAnalysis -- OSR Option not enabled\n");
      return false;
      }
   else if (comp()->isPeekingMethod())
      {
      if (traceOSR)
         log->prints("Should not perform OSRLiveRangeAnalysis -- Not required because we are peeking\n");
      return false;
      }
   else if (!comp()->supportsInduceOSR())
      {
      if (traceOSR)
         log->prints("Should not perform OSRLiveRangeAnlysis -- OSR is not supported under the current configuration\n");
      return false;
      }
      ...
      if (comp()->getOption(TR_MimicInterpreterFrameShape))
         {
         if (traceOSR)
            log->prints("No need to perform OSRLiveRangeAnlysis under mimic interpreter frame shape\n");
         return false;
         }
      else if (disableOSRPointDeadslotsBookKeeping) // save some compile time
         {
         if (traceOSR)
            log->prints("Dead slots bookkeeping is disabled and therefore OSRLiveRangeAnlysis is not needed\n");
         return false;
         }
      }
   ...
   if (methodSymbol->getNumOSRPoints() == 0)
      {
      if (traceOSR)
         log->prints("No OSR points, skip liveness\n");
      return false;
      }
...
int32_t TR_OSRLiveRangeAnalysis::partialAnalysis()
   ...
   if (traceOSR)
      log->prints("Starting partial OSRLiveRangeAnalysis\n");
   ...
   for (TR::TreeTop *tt = comp()->getMethodSymbol()->getLastTreeTop(); tt; tt = tt->getPrevTreeTop())
      ...
      if (isPotentialOSRPoint && comp()->isOSRTransitionTarget(TR::postExecutionOSR))
         {
         if (traceOSR)
            log->printf("Analysing post OSR point for n%dn at %d:%d offset by %d\n", node->getGlobalIndex(),
         ...
         if (traceOSR)
            log->trprintf("Cannot find a post OSR point for node %p", node);
      ...
      if (isPotentialOSRPoint && (comp()->isOSRTransitionTarget(TR::preExecutionOSR) ||
         ...
         {
         if (traceOSR)
            log->printf("Analysing pre OSR point for n%dn at %d:%d\n", node->getGlobalIndex(),
   ...
   if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr)
      {
      if (traceOSR)
         log->prints("Analysing OSR point at method entry\n");
...
void TR_OSRLiveRangeAnalysis::pendingPushLiveRangeInfo(TR::Node *node, TR_BitVector *liveSymRefs,
   ...
   *_workBitVector |= *allPendingPushSymRefs;
   ...
   if (comp()->getOption(TR_TraceOSR))
      {
      TR::Logger *log = comp()->getLogger();
      log->trprintf("Live PP variables at OSR point %p of %p bytecode offset %d\n",
...
void TR_OSRLiveRangeAnalysis::pendingPushSlotSharingInfo(TR::Node *node, TR_BitVector *liveSymRefs,
   ...
   if (liveSymRefs && !liveSymRefs->isEmpty())
      ...
      if (comp()->getOption(TR_TraceOSR))
         comp()->getLogger()->trprintf("Shared PP slots at OSR point [%p] at %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
...
void TR_OSRLiveRangeAnalysis::buildDeadPendingPushSlotsInfo(TR::Node *node, TR_BitVector *livePendingPushSymRefs, TR_OSRPoint *osrPoint)
   ...
   TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
   if (trace())
      log->trprintf("buildDeadPendingPushSlotsInfo: OSR point [%p] at %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
   ...
   if (livePendingPushSymRefs && !livePendingPushSymRefs->isEmpty())
      ...
      while (bvi.hasMoreElements())
         ...
         List<TR::SymbolReference> *list = NULL;
         if (trace())
            log->printf("pending push slot %d is live with symref %d takesTwoSlots %d\n", slot, symRefNum, takesTwoSlots);
   ...
   if (deadPPSSlots && trace())
      {
      TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
      log->trprintf("deadppslots at node %p %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
...
void TR_OSRLiveRangeAnalysis::buildDeadSlotsInfo(TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint, int32_t *liveLocalIndexToSymRefNumberMap, bool containsPendingPushes)
   ...
   TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
   if (trace())
      log->trprintf("buildOSRSlotSharingInfoForDeadSlots: OSR point [%p] at %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
   ...
   if (!liveVars->isEmpty())
      ...
      while (bvi.hasMoreElements())
         ...
         List<TR::SymbolReference> *list = NULL;
         if (trace())
            log->printf("slot %d is live with symref %d takesTwoSlots %d\n", slot, symRefNum, takesTwoSlots);
   ...
   if (deadPPSSlots && !deadPPSSlots->isEmpty())
      ...
      while (bvi.hasMoreElements())
         ...
         int32_t slot =  -bvi.getNextElement()-1;
         if (trace())
            log->trprintf("ppslot %d is dead at %d:%d\n", slot, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
         //symRefNum = -1 and symRefOrder = -1 indicate those slots should be zeroed in prepareForOSR
         comp()->getOSRCompilationData()->addSlotSharingInfo(osrPoint->getByteCodeInfo(),
            slot, -1 /* symRefNum */, -1 /* symRefOrder */, static_cast<int32_t>(TR::Compiler->om.sizeofReferenceAddress()) /*symSize*/ , false /*takesTwoSlots*/);
         }
   ...
   if (deadAutoSlots && trace())
      {
      TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
      log->trprintf("deadAutoSlots at node %p %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
      deadAutoSlots->print(log, comp());
      log->println();
      }
      ...
      for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop())
         ...
         if (tt->getNode()->getOpCodeValue() == TR::BBEnd)
            ...
            if (edge == block->getExceptionSuccessors().end())
               {
               if (comp()->getOption(TR_TraceOSR))
                  log->printf("skipping block_%d without OSR points", block->getNumber());
               break;
               }
...
void TR_OSRLiveRangeAnalysis::maintainLiveness(TR::Node *node,
   ...
   if (comp()->getOption(TR_TraceOSR))
      {
      comp()->getLogger()->trprintf("---> visiting node %p\n", node);
      }
...
void TR_OSRLiveRangeAnalysis::buildOSRSlotSharingInfo(TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
   ...
   if (!liveVars->isEmpty())
      ...
      if (trace())
         comp()->getLogger()->trprintf("Shared slots at OSR point [%p] at %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
...
void TR_OSRLiveRangeAnalysis::buildOSRLiveRangeInfo(TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
   ...
   *_workBitVector -= *liveVars;
   ...
   if (comp()->getOption(TR_TraceOSR))
      {
      TR::Logger *log = comp()->getLogger();
      log->trprintf("Dead variables at OSR point %p of %p bytecode offset %d\n", node, osrMethodData, osrPoint->getByteCodeInfo().getByteCodeIndex());
      if (deadSymRefs)
...
int32_t TR_OSRExceptionEdgeRemoval::perform()
   ...
   if (comp()->getOption(TR_EnableOSR))
      ...
      if (false)
         {
         if (traceOSR)
            log->prints("OSR is enabled but OSR exception edge removal is not.\n");
         return 0;
         }
      }
   else
      {
      if (traceOSR)
         log->prints("OSR Option not enabled -- returning from OSR exception edge removal analysis.\n");
      return 0;
      }
   ...
   if (comp()->getOSRMode() == TR::involuntaryOSR)
      {
      if (traceOSR)
         log->prints("Involuntary OSR enabled -- returning from OSR exception edge removal analysis since we implement involuntary OSR with exception edges.\n");
      return 0;
      }
   ...
   if (comp()->osrInfrastructureRemoved())
      {
      if (traceOSR)
         log->prints("OSR infrastructure removed -- returning from OSR exception edge removal analysis since we have already removed OSR infrastructure.\n");
      return 0;
      }
   ...
   if (comp()->isPeekingMethod())
      ...
      if (traceOSR)
         {
         log->printf("%s OSR exception edge analysis can be done\n",
            optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
         }
      }
