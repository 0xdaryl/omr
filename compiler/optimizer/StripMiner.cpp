...
bool TR_StripMiner::shouldPerform()
   ...
   if (!comp()->generateArraylets())
      {
      trprints(trace(), log, "Not enabled in non-rtj mode.\n");
      return false;
      }
   else if (comp()->getOption(TR_DisableStripMining))
      {
      trprints(trace(), log, "Option is not enabled -- returning from strip mining.\n");
      return false;
      }

   if (!comp()->mayHaveLoops())
      {
      trprints(trace(), log, "Method does not have loops -- returning from strip mining.\n");
      return false;
      }
...
int32_t TR_StripMiner::perform()
   {
   TR::Logger *log = comp()->log();
   trprintf(trace(), log, "Processing method: %s\n", comp()->signature());
...
void TR_StripMiner::collectLoops(TR_Structure *str)
   ...
   if (region->isNaturalLoop())
      {
      trprintf(trace(), log, "<analyzeLoops loop=%d addr=%p>\n", region->getNumber(), region);
      // Check for loop pre-header
      //
      TR::Block *preHeader = getLoopPreHeader(str);
      if (preHeader == NULL)
         {
         trprintf(trace(), log, "\tReject loop %d ==> no pre-header\n", region->getNumber());
         return;
         }
      ...
      if (loopTest == NULL)
         {
         trprintf(trace(), log, "\tReject loop %d ==> no loop test block\n", region->getNumber());
         return;
         }
      ...
      if (piv == NULL)
         {
         trprintf(trace(), log, "\tReject loop %d ==> no primary induction variable\n", region->getNumber());
         return;
         }
      ...
      if (region->getEntryBlock()->getPredecessors().size() != 2)
         {
         trprintf(trace(), log, "\tReject loop %d ==> more than 1 back edge\n", region->getNumber());
         return;
         }
      ...
      for (TR::Block *b = bIt.getFirst(); b; b = bIt.getNext())
         ...
         if (b->hasExceptionPredecessors()) // catch
            {
            trprintf(trace(), log, "\tReject loop %d ==> block_%d has exception predecessors\n", region->getNumber(), b->getNumber());
            return;
            }
         if (b->hasExceptionSuccessors()) // try
            {
            trprintf(trace(), log, "\tReject loop %d ==> block_%d has exception successors\n", region->getNumber(), b->getNumber());
            return;
            }
         }
      ...
      if (checkIfIncrementalIncreasesOfPIV(li))
         {
         trprintf(trace(), log, "\tReject loop %d ==> multiple store of induction variable were found\n", region->getNumber());
         return;
      ...
      if ( (li->_offset==-1 ) ||
         ...
         {
         trprintf(trace(), log, "\tReject loop %d ==> needs a offsetLoop - cannot deal with this now\n", region->getNumber());
         return;
         }
      ...
      trprintf(trace(), log, "\titerationCount = %d stripLength = %d\n", iterCount, li->_stripLen);
      if (iterCount != -1)
         {
         if (iterCount < li->_stripLen)
            {
            trprintf(trace(), log, "\tReject loop %d ==> iteration count is less than the strip length\n", region->getNumber());
            return;
            }
         }
      ...
      if ((li->_preOffset!=0) ||
          (li->_postOffset !=0))
         {
         trprintf(trace(), log, "\tReject loop %d ==> pre offset = %d, post offsets = %d\n", region->getNumber(), li->_preOffset, li->_postOffset);
         return;
         }
      ...
      if (li->_preOffset + li->_postOffset >= li->_stripLen)
         {
         trprintf(trace(), log, "\tReject loop %d ==> pre offset = %d, post offsets = %d, strip length = %d\n", region->getNumber(), li->_preOffset, li->_postOffset, li->_stripLen);
         return;
         }
      ...
      if (!li->_canMoveAsyncCheck)
         {
         trprintf(trace(), log, "\tReject loop %d ==> calls present\n", region->getNumber());
         return;
         }
      ...
      {
      trprintf(trace(), log, "\tReject region %d ==> not a natural loop\n", region->getNumber());
      }
   }
...
void TR_StripMiner::transformLoops()
   ...
   for (LoopInfo *li = it.getFirst(); li; li = it.getNext())
      ...
      if (li != NULL &&
         ...
         trprintf(trace(), comp()->log(), "Done transforming loop %d\n", li->_regionNum);
         }
      }
   }
...
void TR_StripMiner::duplicateLoop(LoopInfo *li, TR_ClonedLoopType type)
   ...
   switch (type)
      ...
      case offsetLoop:
         ...
         trprintf(trace(), log, "\tcreating start offset loop: loop %d...\n", li->_regionNum);
      ...
      case preLoop:
         ...
         trprintf(trace(), log, "\tcreating pre loop: loop %d...\n", li->_regionNum);
         isMainOrResidual = false;
         break;
      case mainLoop:
         blockMapper = _mainBlockMapper;
         trprintf(trace(), log, "\tcreating main loop: loop %d...\n", li->_regionNum);
         break;
      case postLoop:
         if (li->_postOffset <= 0)
            return;
         blockMapper = _postBlockMapper;
         trprintf(trace(), log, "\tcreating post loop: loop %d...\n", li->_regionNum);
         isMainOrResidual = false;
         break;
      case residualLoop:
         blockMapper = _residualBlockMapper;
         trprintf(trace(), log, "\tcreating residual loop: loop %d...\n", li->_regionNum);
         break;
      default:
         trprintf(trace(), log, "\tcannot recognize loop type: %d, abort duplication...\n", type);
         return;
      }
   ...
   for (block = bIt.getFirst(); block; block = bIt.getNext())
      ...
      trprintf(trace(), log, "\tprocessing edges for: block [%d] => cloned block [%d]\n", block->getNumber(), clonedBlock->getNumber());

      if (block == li->_loopTest && !isMainOrResidual)
         {
         trprintf(trace(), log, "\tskipping edge from non-main non-residual loop test block [%d]\n", block->getNumber());
         continue;
         }
      ...
      for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge)
         ...
         trprintf(trace(), log, "\t   for edge: [%d] => [%d]\n", clonedBlock->getNumber(), clonedDest ? clonedDest->getNumber() : dest->getNumber());

         if (clonedDest)
            redirect(clonedBlock, dest, clonedDest);
         else
            trprintf(trace(), log, "\t   skipping exit edge to [%d]\n", dest->getNumber());
         }
      }
...
void TR_StripMiner::transformLoop(LoopInfo *li)
   ...
   if (limitNode->getOpCode().isLoadConst())
      ...
      if ((li->_increasing && limit < limit + li->_stripLen) ||
          ...
          {
          trprintf(trace(), log, "No overflow test for %s loop %d: limit = %ld, stripLength = %ld\n",
   ...
   if (testOverflow)
      ...
      trprintf(trace(), log, "\tcreating overflow test [%p] in block [%d]\n", ifOverNode, ifOverBlock->getNumber());

      // Point the original loop pre-header's predecessor to the if over block
      trprintf(trace(), log, "\tfixing predecessor edges of original loop pre-header [%d] :\n", ph->getNumber());
      ...
      //
      trprintf(trace(), log, "\t   adding edge: overflow test block [%d] => original pre-header [%d]\n", ifOverBlock->getNumber(), ph->getNumber());
   ...
   if (li->_needOffsetLoop)
      ...
      if (testOverflow)
         ...
         trprintf(trace(), log, "\tfixing edges: preds of pre-header [%d] -> offset loop [%d]\n", ph->getNumber(), offsetLoopPh->getNumber());
   ...
   trprintf(trace(), log, "<stripMining loop=%d outer header=%d>\n", li->_regionNum, outerHeader->getNumber());
   ...
   if (li->_preOffset > 0)
      ...
      /* Fix the edges */
      trprints(trace(), log, "\t   fixing preloop loop test successor edges:\n");
   ...
   if (li->_postOffset > 0)
      ...
      /* Fix the edges */
      trprints(trace(), log, "\t   fixing postloop loop test successor edges:\n");
...
TR::Block *TR_StripMiner::createStartOffsetLoop(LoopInfo *li, TR::Block *outerHeader)
   ...
   //
   trprintf(trace(), log, "\t   adding edge: test block [%d] => exit [%d]\n", ltBlock->getNumber(), origExit->getNumber());
   ...
   trprintf(trace(), log, "\t   adding edge: new test block [%d] => outer pre-header [%d]\n", newLtBlock->getNumber(), outerHeader->getNumber());
   _cfg->addEdge(newLtBlock, outerHeader);
   trprintf(trace(), log, "\t created a new block [%d] to position at arraylet with test [%p]\n", newLtBlock->getNumber(), newLtNode);
   return newLtBlock;
   }
...
TR::Block *TR_StripMiner::stripMineLoop(LoopInfo *li, TR::Block *outerHeader)
   ...
   trprintf(trace(), log, "\tcreating striplength test [%p] in outer loop block [%d]\n", ifNode, branchBlock->getNumber());

   /* Add an edge: if -> main pre-header */
   trprintf(trace(), log, "\t   adding edge: branch [%d] => main pre-header [%d]\n", branchBlock->getNumber(), mainPh->getNumber());
   ...
   trprintf(trace(), log, "\t   created a new block [%d] to store i = i + j and test if i < N\n", ifContinueBlock->getNumber());
   ...
   trprintf(trace(), log, "\t   changed main loop test [%p] to j < (strip length - pre - post)\n", mainLtTree->getNode());




   /* For both main and residual loops, replace old induction variable in the inner loop */
   trprintf(trace(), log, "\t   replacing original induction variable symRef [%d]\n", li->_piv->getSymRef()->getReferenceNumber());
   ...
   trprintf(trace(), log, "\t   changed residual loop test [%p] to j < (N - i)\n", residualLtTree->getNode());
   ...
   if (li->_canMoveAsyncCheck)
      ...
      if (li->_asyncTree != NULL)
         {
         trprintf(trace(), log, "\t   moved asynccheck tree [%p] to block [%d]\n", li->_asyncTree, outerHeader->getNumber());
...
void TR_StripMiner::redirect(TR::Block *source, TR::Block *oldDest, TR::Block *newDest)
   ...
   if (branchNode->getOpCode().isSwitch())
      ...
      for (auto i = branchNode->getCaseIndexUpperBound() - 1; i > 0; --i)
         ...
         if (branchNode->getChild(i)->getBranchDestination()->getNode()->getBlock() == oldDest)
            {
            trprintf(trace(), log, "\t      fixing switch statement: [%d] => [%d]\n", source->getNumber(), newDest->getNumber());
   ...
   if (found)
      ...
      trprintf(trace(), log, "\t      adding edge: [%d] => [%d]\n", source->getNumber(), newDest->getNumber());
      }
   else
      {
      /* Create a goto block to the new destination */
      TR::Block *gotoBlock = createGotoBlock(source, newDest);
      trprintf(trace(), log, "\t      adding edges: source [%d] => goto [%d]; goto [%d] => new dest [%d]\n",
...
TR::Block *TR_StripMiner::createLoopTest(LoopInfo *li, TR_ClonedLoopType type)
   ...
   trprintf(trace(), log, "\t Adding temp = i + %s-offset in block [%d]\n", type == preLoop ? "pre" : "post", clonedPh->getNumber());
   ...
   trprintf(trace(), log, "\t created a new block [%d] to test if i < temp [%p]\n", ifBlock->getNumber(), ifTree->getNode());
   ...
   for (block = bIt.getFirst(); block; block = bIt.getNext())
      ...
      while (currentTree != exitTree)
         ...
         if (checkClone && li->_canMoveAsyncCheck && node->getOpCodeValue() == TR::asynccheck &&
             li->_asyncTree == NULL)
            {
            trprintf(trace(), log, "      found asynccheck [%p] in block [%d]\n", currentTree, block->getNumber());
...
void TR_StripMiner::examineNode(LoopInfo *li, TR::Node *parent, TR::Node *node,
   ...
   if (checkClone && node->getSymbolReference() == oldSymRef)
      ...
      if (node->getOpCode().isLoad())
         ...
         if (foundLoad)
            {
            trprintf(trace(), log, "      adding node [%p] to load list parent: [%p], childNum: %d\n", node, parent, childNum);
         ...
         {
         trprintf(trace(), log, "      adding node [%p] store list parent: [%p]\n", node, parent);
                     ...
                     if (secondChild->getFirstChild()->getOpCode().isMul() ||
                        ...
                        if (firstChild->getOpCode().isRightShift() ||
                           ...
                           if ((accessNode->getOpCodeValue() == TR::iload) &&
                              ...
                              trprintf(trace(), log, "Node %p accesses array with no pre/post offset\n", node);
                              }
                           else
                              {
                              if (accessNode->getOpCodeValue() != TR::iload)
                                 ...
                                 if ((accessNode->getOpCode().isAdd() ||
                                    ...
                                    trprintf(trace(), log, "Node %p has pre-offset: %d, post-offset: %d\n", node, li->_preOffset, li->_postOffset);
                                    }
                                 else
                                    {
                                    ...
                                    if (foundPiv)
                                       ...
                                       li->_preOffset = 0;
                                       li->_postOffset = 0;
                                       trprintf(trace(), log, "Found an iload/iadd/isub tree for node %p\n", node);
                                       }
                                    else
                                       {
                                       li->_preOffset = -1;
                                       li->_postOffset = -1;
                                       trprintf(trace(), log, "No iload/iadd/isub tree found for node %p\n", node);
                                       }
                                    }
                                 }
                                 ...
                                 {
                                 trprintf(trace(), log, "Found iload %p which is not piv ... skipping it\n", node);
                                 }
                              }
                           }
                        else
                           {
                           trprintf(trace(), log, "Node %p does not have an ishr/idiv tree\n", node);
                           }
                        }
                     }
                  }
...
void TR_StripMiner::replaceLoopPivs(LoopInfo *li, TR::ILOpCodes newOpCode, TR::Node *newConst,
   ...
   trprintf(trace(), log, "   replacing Pivs in %s loop %d...\n", type == mainLoop ? "main" : "residual", li->_regionNum);
   ...
   for (parent = it.getFirst(); parent != NULL; parent = it.getNext())
      ...
      if ((parentNode->getOpCode().isRightShift() || parentNode->getOpCode().isDiv()) && skipNode)
         {
         trprintf(trace(), log, "      replacing load [%p] with new load [%p]: parent [%p], childNum %d\n", parentNode->getChild(childNum), iNode, parentNode, childNum);
         ...
         {
         trprintf(trace(), log, "    changing and node parent  [%p] with new load [%p]\n", parentNode, parentNode->getChild(0));
         }
      else
         {
...
/* Process list of stores */
   ...
   for (parent = it.getFirst(); parent != NULL; parent = it.getNext())
      ...
      trprintf(trace(), log, "      replacing store [%p] with new store [%p]: new symref [%d]\n", node, storeNode, newSymRef->getReferenceNumber());
...
TR::Block *TR_StripMiner::getLoopTest(TR_Structure *str, TR::Block *preHeader)
   ...
   if (loopTest &&
      ...
      {
      trprintf(trace(), log, "loop %d: no loop test found on backedge\n", region->getNumber());
      loopTest = NULL;
      }
   ...
   if (loopTest)
      ...
      if (TR::ILOpCode::isNotEqualCmp(loopTest->getLastRealTreeTop()->getNode()->getOpCodeValue()) ||
         ...
         {
         trprintf(trace(), log, "loop %d: found loop with eq/ne test condition\n", region->getNumber());
         loopTest = NULL;
...
bool TR_StripMiner::checkIfIncrementalIncreasesOfPIV(LoopInfo *li)
   ...
   trprintf(trace(), log, "   looking for stores in original loop %d...\n",  li->_regionNum);
         ...
         if (node->getOpCode().isStore() && node->getSymbolReference()==oldCounter)
            ...
            if (((node->getFirstChild()->getOpCode().isSub()) || (node->getFirstChild()->getOpCode().isAdd())) &&
               ...
               if (pivIncInStore  != li->_piv->getDeltaOnBackEdge())
                  {
                  trprintf(trace(), log, "\t loop %d ==> Found a store to induction variable with increment different than deltaObBackEdge\n", li->_region->getNumber());
                  return true;
                  }
               }
            }
