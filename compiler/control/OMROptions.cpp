...
TR::OptionTable OMR::Options::_jitOptions[] = {
   ...
   {"disableVectorAPIBoxing",             "M\tdisable boxing/unboxing during VectorAPIExpansion", SET_OPTION_BIT(TR_DisableVectorAPIBoxing), "F"},
   ...
   {"dontDisclaimMemoryOnSwap",           "M\tIf memory disclaiming is enabled, prevent disclaiming on swap. Takes precedence over -Xjit:disclaimMemoryOnSwap", SET_OPTION_BIT(TR_DontDisclaimMemoryOnSwap), "F", NOT_IN_SUBSET},
   ...
   {"traceFileLength=",                 "L\tmax trace file length in MiB (best effort). 0 for unlimited (default)", TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_traceFileLengthInMiB, 0, "F%d", NOT_IN_SUBSET},
   ...
   {"traceNodeRefCountVerification",    "L\ttrace verification of node reference counts",  SET_OPTION_BIT(TR_TraceNodeRefCountVerification), "F"},
...
OMR::Options::jitLatePostProcess(TR::OptionSet *optionSet, void * jitConfig)
   ...
   if (!optionSet)
      ...
      if (_logFileName)
         ...
         if (_debug)
            {
            _logFile = _debug->findLogFile(TR::Options::getAOTCmdLineOptions(), TR::Options::getJITCmdLineOptions(), optionSet, _logFileName, _logger);
            if (_logFile == NULL)
               self()->openLogFile();
            else
...
OMR::Options::shutdown(TR_FrontEnd * fe)
   ...
   if (TR::Options::isFullyInitialized())
      ...
      if (TR::Options::getAOTCmdLineOptions() && TR::Options::getAOTCmdLineOptions()->getLogFile())
         TR::Options::closeLogFile(fe, TR::Options::getAOTCmdLineOptions()->getLogFile(), TR::Options::getAOTCmdLineOptions()->getLogger());
      ...
      if (TR::Options::getAOTCmdLineOptions())
         ...
         for (optionSet = TR::Options::getAOTCmdLineOptions()->_optionSets; optionSet; optionSet = optionSet->getNext())
            ...
            TR::Logger *logger = optionSet->getOptions()->getLogger();
      ...
      if (TR::Options::getJITCmdLineOptions())
         ...
         if (TR::Options::getJITCmdLineOptions()->getLogFile())
            ...
            TR::Logger *logger = TR::Options::getJITCmdLineOptions()->getLogger();
            if (TR::Options::getAOTCmdLineOptions())
               {
...
OMR::Options::requiresLogFile()
   ...
   if (self()->getAnyOption(TR_TraceAll) ||
       ...
       self()->getAnyOption(TR_TraceRegisterPressureDetails) ||
       self()->getAnyOption(TR_TraceOSR) ||
       ...
       self()->getAnyOption(TR_TraceCFGVerification) ||
       self()->getAnyOption(TR_TraceTreeVerification) ||
       self()->getAnyOption(TR_TraceNodeRefCountVerification) ||
       self()->getAnyOption(TR_TraceBlockVerification) ||
       self()->getAnyOption(TR_DebugInliner)
       )
      return true;
...
TR::Logger *
OMR::Options::createLoggerForLogFile(TR::FILE *file)
   {
   TR::Logger *logger = TR::CStdIOStreamLogger::create((::FILE *)file);

   if (_traceFileLengthInMiB > 0)
      {
      // Wrap the logger in a CircularLogger if requested
      //
      logger = TR::CircularLogger::create(logger, _traceFileLengthInMiB << 20);
      }
...
#undef FN_BUF_SIZE
   ...
   /**
    * Prepare a TR::Logger wrapper for the newly opened log file and add the
    * boilerplate header.
    */
   if (_logFile)
      {
...
void OMR::Options::closeLogFile(TR_FrontEnd *fe, TR::FILE *file, TR::Logger *log)
   {
   if (file != NULL)
      log->prints("</jitlog>\n");
