...
bool OMR::Options::jitLatePostProcess(TR::OptionSet *optionSet, void *jitConfig)
{
    ...
    } else // option set processing
    {
        ...
        if (getLogFileNameBase()) {
            ...
            if (_debug) {
                OMR::Logger *logger = self()->findLoggerForLogFileName(TR::Options::getAOTCmdLineOptions(), TR::Options::getJITCmdLineOptions(),
                    optionSet, getLogFileNameBase());
    ...
    if (TR::Options::isFullyInitialized()) {
        ...
        if (aotCmdLineOptions) {
            for (TR::OptionSet *optionSet = aotCmdLineOptions->_optionSets; optionSet;
                ...
                for (TR::OptionSet *prev = aotCmdLineOptions->_optionSets; prev != optionSet;
                     prev = prev->getNext()) {
        ...
        if (jitCmdLineOptions) {
            ...
            for (TR::OptionSet *optionSet = jitCmdLineOptions->_optionSets; optionSet;
                ...
                for (TR::OptionSet *prev = jitCmdLineOptions->_optionSets; prev != optionSet;
                     prev = prev->getNext()) {
                    if (prev->getOptions()->getLogger() == logger) {
                        logger = NULL;
                        break;
...
OMR::Logger *OMR::Options::findLoggerForLogFileName(TR::Options *cmdLineOptions, TR::OptionSet *optSet, char *logFileName)
{
    OMR::Logger *logger = NULL;
...
{
    if (aotCmdLineOptions) {
        OMR::Logger *logger = self()->findLoggerForLogFileName(aotCmdLineOptions, optSet, logFileName);
        if (logger)
            return logger;
    }

    if (jitCmdLineOptions) {
        OMR::Logger *logger = self()->findLoggerForLogFileName(jitCmdLineOptions, optSet, logFileName);
        if (logger)
            return logger;
    }
...
{
    OMR::Logger *logger = self()->findLoggerForLogFileNameOnCompilationThreadInner(logFileName, aotCmdLineOptions, compThreadID);
...
void OMR::Options::setLogForCompilationThread(int32_t compThreadID, TR::Options *mainOptions)
{
    ...
    if (OMR::Options::_dualLogging) {
        ...
        _logger = self()->findLoggerForLogFileNameOnCompilationThread(getLogFileNameBase(), OMR::Options::getAOTCmdLineOptions(),
            OMR::Options::getJITCmdLineOptions(), compThreadID);
