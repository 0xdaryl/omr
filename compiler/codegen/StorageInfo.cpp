...
TR_StorageInfo::print()
   {
   comp()->getLogger()->trprintf("\t\t\t%s (%p) len %d: addr %s (%p) symRef #%d, offset %d, class %s\n",
...
TR_StorageInfo::mayOverlapWith(TR_StorageInfo *info)
   ...
   if (symbolMismatch &&
      ...
      {
      if (traceBCDCodeGen)
         log->trprintf("\t\toverlap=false : autoDirectMapped and diff symbols (#%d (%p) and #%d (%p))\n",
      ...
      {
      if (traceBCDCodeGen)
         log->trprintf("\t\toverlap=false : staticDirectMapped and diff symbols (#%d (%p) and #%d (%p))\n",
            _symRef->getReferenceNumber(),_symRef->getSymbol(),info->_symRef->getReferenceNumber(),info->_symRef->getSymbol());
   ...
   if (!compareRanges &&
      ...
      {
      if (traceBCDCodeGen)
         log->trprintf("\t\t\tcompareRanges : nodes match case (%s (%p) and %s (%p))\n",
