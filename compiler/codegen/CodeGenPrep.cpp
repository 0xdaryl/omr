...
OMR::CodeGenerator::addCountersToEdges(TR::Block *b)
   ...
   if (lastNode->getOpCode().isBranch() && !found)
      ...
      if (c && comp->getOptions()->dynamicDebugCounterIsEnabled(c) && !(dest->getPredecessors().size() == 1) /* && !toBlock(relevantEdge->getFrom())->getSuccessors().isSingleton() */)
         {
      ...
      TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "block_%d TAKEN", b->getNumber()), dest->getEntry()->getNextTreeTop());

      if (lastNode->getOpCode().isIf())
         {
         TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "block_%d NOT TAKEN", b->getNumber()), fallthrough->getEntry()->getNextTreeTop());
         }
      }
   }
...
OMR::CodeGenerator::insertDebugCounters()
   ...
   for (tt = comp->getStartTree(); tt; tt = tt->getNextTreeTop())
      ...
      if (node->getOpCodeValue() == TR::BBStart)
         ...
         if (block->isCold())
            {
            TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "blocks/%sCompiles/coldBlocks/=%d", comp->getHotnessName(comp->getMethodHotness()), block->getNumber()), tt->getNextTreeTop(), 1, TR::DebugCounter::Free);
            TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "coldBlocks/byJittedBody/(%s)/%s/=%d", comp->signature(), comp->getHotnessName(comp->getMethodHotness()), block->getNumber()), tt->getNextTreeTop(), 1, TR::DebugCounter::Free);
            }
         else
            {
            TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "blocks/%sCompiles/warmBlocks/=%d", comp->getHotnessName(comp->getMethodHotness()), block->getNumber()), tt->getNextTreeTop());
            TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "warmBlocks/byJittedBody/(%s)/%s/=%d", comp->signature(), comp->getHotnessName(comp->getMethodHotness()), block->getNumber()), tt->getNextTreeTop());
            }
            ...
            if (predBlock->getEntry() && predBlock->getExit())
               ...
               if (predNode->getOpCode().isSwitch())
                  ...
                  TR::DebugCounter::prependDebugCounter
                     (comp, TR::DebugCounter::debugCounterName(comp, "branchtargets/%s/(%s)/%s/%d/%d",
                  opName,
                  comp->signature(),
      ...
      if (node->getOpCode().isBranch() && !node->getOpCode().isSwitch())
         ...
         TR::DebugCounter::prependDebugCounter
            (comp, TR::DebugCounter::debugCounterName(comp, "branches/%s/(%s)/%s/%d",
               opName,
               comp->signature(),
               comp->getHotnessName(comp->getMethodHotness()),
               tt->getNode()->getByteCodeIndex()),
      ...
      if (node->getOpCode().isNew())
         ...
         if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::anewarray)
            ...
            if (classChild->getOpCodeValue() != TR::loadaddr)
               ...
               {
               int32_t length;
               const char *className = TR::Compiler->cls.classNameChars(comp, (TR_OpaqueClassBlock*)classChild->getSymbol()->castToStaticSymbol()->getStaticAddress(), length);
               TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp,
            ...
            TR::DebugCounter::prependDebugCounter(comp, TR::DebugCounter::debugCounterName(comp, "allocations/%s/%s", opName, typeName), tt);
            }
         }
      }
   }
