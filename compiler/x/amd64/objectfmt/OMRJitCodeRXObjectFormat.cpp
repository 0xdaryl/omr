...
TR::Instruction *OMR::X86::AMD64::JitCodeRXObjectFormat::emitFunctionCall(TR::FunctionCallData &data)
{
    ...
    if (comp->isRecursiveMethodTarget(methodSymRef->getSymbol()) && !comp->couldBeRecompiled()) {
        ...
        if (data.prevInstr) {
            callImmSymInstr = INST_ImmSym(data.prevInstr, OP::CALLImm4, 0, data.methodSymRef, data.regDeps, data.cg);
        } else {
            callImmSymInstr = INST_ImmSym(OP::CALLImm4, data.callNode, 0, data.methodSymRef, data.regDeps, data.cg);
        }
    ...
    TR::StaticSymbol *functionDataSymbol = TR::StaticSymbol::createWithAddress(comp->trHeapMemory(), TR::Address,
        reinterpret_cast<void *>(ccFunctionDataAddress));
    ...
    if (data.prevInstr) {
        callInstr = INST_CallMem(data.prevInstr, OP::CALLMem, callMR, data.regDeps, data.cg);
    } else {
        callInstr = INST_CallMem(OP::CALLMem, data.callNode, callMR, data.regDeps, data.cg);
    }
...
void OMR::X86::AMD64::JitCodeRXObjectFormat::printEncodedFunctionCall(OMR::Logger *log, TR::FunctionCallData &data,
    uint8_t *bufferPos)
{
