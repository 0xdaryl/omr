...
TR::Instruction *OMR::X86::AMD64::JitCodeRXObjectFormat::emitFunctionCall(TR::FunctionCallData &data)
{
    ...
    if (comp->isRecursiveMethodTarget(methodSymRef->getSymbol()) && !comp->couldBeRecompiled()) {
        ...
        if (data.prevInstr) {
            callImmSymInstr = INST_ImmSym(data.prevInstr, TR::InstOpCode::CALLImm4, 0, data.methodSymRef,
                data.regDeps, data.cg);
        } else {
            callImmSymInstr = INST_ImmSym(TR::InstOpCode::CALLImm4, data.callNode, 0, data.methodSymRef,
                data.regDeps, data.cg);
        }
