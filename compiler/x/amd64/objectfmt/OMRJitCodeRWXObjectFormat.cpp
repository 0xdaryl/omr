...
TR::Instruction *OMR::X86::AMD64::JitCodeRWXObjectFormat::emitFunctionCall(TR::FunctionCallData &data)
{
    ...
    if (data.runtimeHelperIndex || methodSymRef->getSymbol()->castToMethodSymbol()->isHelper()) {
        ...
        if (targetAddress == 0) {
             ...
             */
            if (data.prevInstr) {
                callInstr = INST_ImmSym(data.prevInstr, OP::CALLImm4, 0, data.methodSymRef,
                    data.regDeps, data.cg);
            } else {
                callInstr = INST_ImmSym(OP::CALLImm4, data.callNode, 0, data.methodSymRef,
                    data.regDeps, data.cg);
            }
        } else {
            /**
            ...
            if (data.useSymInstruction) {
                ...
                if (data.prevInstr) {
                    loadInstr = INST_RegImm64Sym(data.prevInstr, OP::MOV8RegImm64,
                        data.scratchReg, targetAddress, data.methodSymRef, data.cg);
                } else {
                    loadInstr = INST_RegImm64Sym(OP::MOV8RegImm64, data.callNode,
                        data.scratchReg, targetAddress, data.methodSymRef, data.cg);
                }
                ...
                if (data.prevInstr) {
                    loadInstr = INST_RegImm64(data.prevInstr, OP::MOV8RegImm64,
                        data.scratchReg, targetAddress, data.cg, data.reloKind);
                } else {
                    loadInstr = INST_RegImm64(OP::MOV8RegImm64, data.callNode,
                        data.scratchReg, targetAddress, data.cg, data.reloKind);
                }
