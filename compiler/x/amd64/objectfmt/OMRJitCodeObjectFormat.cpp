...
TR::Instruction *
OMR::X86::AMD64::JitCodeObjectFormat::emitGlobalFunctionCall(TR::FunctionCallData &data)
   {
   ...
   if (data.runtimeHelperIndex > 0)
      ...
      {
      globalMethodSymRef = data.methodSymRef;
      }
...
uint8_t *
OMR::X86::AMD64::JitCodeObjectFormat::encodeGlobalFunctionCall(TR::FunctionCallData &data)
   {
   uint8_t op = data.isCall ? 0xe8 : 0xe9;   // CALL rel32 | JMP rel32
   *data.bufferAddress++ = op;

   int32_t disp32 = data.cg->branchDisplacementToHelperOrTrampoline(data.bufferAddress+4, data.methodSymRef);
...
TR::Instruction *
OMR::X86::AMD64::JitCodeObjectFormat::emitCodeCacheFunctionCall(TR::FunctionCallData &data)
   {
   return emitGlobalFunctionCall(data);
   }


uint8_t *
OMR::X86::AMD64::JitCodeObjectFormat::encodeCodeCacheFunctionCall(TR::FunctionCallData &data)
   {
   return encodeCodeCacheFunctionCall(data);
   }
