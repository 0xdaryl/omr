        ...
        if (valueReg) {
            ...
            if (isVolatile) {
                if (performTransformation(comp, "O^O Using SSE for volatile store %s\n",
                    ...
                    instr = INST_MemReg(TR::InstOpCode::S4MemReg, node, stackLow, valueReg->getLowOrder(),
                        cg);
    ...
    if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL
        ...
        if (lowValue >= -128 && lowValue <= 127) {
            if (isMemOp)
                ...
                instr = INST_RegImm(TR::InstOpCode::ADD4RegImms, node, targetRegister->getLowOrder(),
                    lowValue, cg);
        } else if (lowValue == 128) {
            if (isMemOp)
                instr = INST_MemImm(TR::InstOpCode::SUB4MemImms, node, lowMR, -128, cg);
            else
                instr = INST_RegImm(TR::InstOpCode::SUB4RegImms, node, targetRegister->getLowOrder(),
                    -128, cg);
            ...
            if (isMemOp)
                ...
                instr = INST_RegImm(TR::InstOpCode::ADD4RegImm4, node, targetRegister->getLowOrder(),
                    lowValue, cg);
        }
    ...
    if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL
        ...
        if (lowValue >= -128 && lowValue <= 127) {
            ...
            if (isMemOp)
                ...
                instr = INST_RegImm(TR::InstOpCode::SUB4RegImm4, node, targetRegister->getLowOrder(),
                    lowValue, cg);
        }
        ...
        if (lowValue == 0 && highValue == 0) {
            ...
            if (!firstIU2L && !intMul) {
                ...
                if (tempRegister == 0) // decomposition failed
                {
                    ...
                    INST_RegRegImm(opCode, node, tempRegister, multiplierRegister->getHighOrder(),
                        lowValue, cg);
                }
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairShlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        if (value == 0) {
            ...
            INST_RegReg(TR::InstOpCode::MOV4RegReg, node, targetRegister->getHighOrder(),
            ...
            if (value < 32) {
                ...
                if (value != 32) {
                    value -= 32;
                    INST_RegImm(TR::InstOpCode::SHL4RegImm1, node, targetRegister->getLowOrder(), value,
                        cg);
                }
        ...
        INST_RegReg(TR::InstOpCode::SHLD4RegRegCL, node, targetRegister->getHighOrder(),
        ...
        INST_RegReg(TR::InstOpCode::CMOVNE4RegReg, node, targetRegister->getHighOrder(),
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (value != 0) {
            ...
            if (value != 0) {
                ...
                INST_RegReg(TR::InstOpCode::MOV4RegReg, node, tempHighReg, targetRegister->getHighOrder(),
                    cg);
                ...
                INST_RegRegImm(TR::InstOpCode::SHLD4RegRegImm1, node, targetRegister->getLowOrder(),
        ...
        INST_RegReg(TR::InstOpCode::SHLD4RegRegCL, node, targetRegister->getHighOrder(),
            targetRegister->getLowOrder(), shiftDependencies, cg);
        INST_RegReg(TR::InstOpCode::SHLD4RegRegCL, node, targetRegister->getLowOrder(), scratchReg,
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairShrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        if (value == 0)
            ...
            INST_RegReg(TR::InstOpCode::MOV4RegReg, node, targetRegister->getLowOrder(),
        ...
        TR::Register *scratchReg = cg->allocateRegister();
        INST_RegReg(TR::InstOpCode::SHRD4RegRegCL, node, targetRegister->getLowOrder(),
        ...
        INST_RegImm(TR::InstOpCode::TEST1RegImm1, node, shiftAmountReg, 32, cg);
        INST_RegReg(TR::InstOpCode::CMOVNE4RegReg, node, targetRegister->getLowOrder(),
            targetRegister->getHighOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getReferenceCount() != 1) {
                targetRegister = cg->allocateRegister();
                INST_RegReg(TR::InstOpCode::MOV4RegReg, node, targetRegister, cmpRegister->getLowOrder(),
                    cg);
            }
...
TR::Register *OMR::X86::I386::TreeEvaluator::b2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        INST_RegReg(TR::InstOpCode::MOVSXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::bu2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        INST_RegReg(TR::InstOpCode::MOVZXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::s2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        node->setRegister(longReg);
        INST_RegReg(TR::InstOpCode::MOVSXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::su2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        node->setRegister(longReg);
        INST_RegReg(TR::InstOpCode::MOVZXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::dstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if ((valueChild->getOpCodeValue() == TR::lbits2d) && !valueChild->getRegister()) {
        ...
        if (valueChild->getOpCode().isLoadConst()) {
            ...
            INST_RegMem(TR::InstOpCode::L4RegMem, node, tempRegister,
                generateX86MemoryReference(*loadLowMR, 4, cg), cg);
            instr = INST_MemReg(TR::InstOpCode::S4MemReg, node,
...
TR::Register *OMR::X86::I386::TreeEvaluator::l2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        INST_MemReg(TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(*lowMR, 4, cg),
...
TR::Register *OMR::X86::I386::TreeEvaluator::lbits2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && child->getReferenceCount() == 1) {
        ...
        INST_MemReg(TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(*tempMR, 4, cg),
            longReg->getHighOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::dbits2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && (child->getReferenceCount() == 1)) {
        ...
        INST_RegMem(TR::InstOpCode::L4RegMem, node, highReg, generateX86MemoryReference(*tempMR, 4, cg),
            cg);
        ...
        INST_RegMem(TR::InstOpCode::L4RegMem, node, lowReg, generateX86MemoryReference(*tempMR, 0, cg),
            cg);
        INST_RegMem(TR::InstOpCode::L4RegMem, node, highReg, generateX86MemoryReference(*tempMR, 4, cg),
            cg);
    }
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getOpCodeValue() == TR::land && firstChild->getReferenceCount() == 1
                ...
                if (firstChild->getReferenceCount() != 1) {
                    targetRegister = cg->allocateRegister();
                    INST_RegReg(TR::InstOpCode::MOV4RegReg, node, targetRegister,
                        cmpRegister->getLowOrder(), cg);
                    targetNeedsToBeExplicitlyStopped = true;
                }
                INST_RegReg(TR::InstOpCode::OR4RegReg, node, targetRegister, cmpRegister->getHighOrder(),
                    cg);
            }
