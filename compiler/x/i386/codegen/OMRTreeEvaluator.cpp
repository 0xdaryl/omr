...
TR::Register *OMR::X86::I386::TreeEvaluator::longArithmeticCompareRegisterWithImmediate(TR::Node *node,
    TR::Register *cmpRegister, TR::Node *immedChild, OP::Mnemonic firstBranchOpCode,
    OP::Mnemonic secondBranchOpCode, TR::CodeGenerator *cg)
{
...
TR::Register *OMR::X86::I386::TreeEvaluator::compareLongAndSetOrderedBoolean(TR::Node *node,
    OP::Mnemonic highSetOpCode, OP::Mnemonic lowSetOpCode, TR::CodeGenerator *cg)
{
...
void OMR::X86::I386::TreeEvaluator::compareLongsForOrder(TR::Node *node, OP::Mnemonic highOrderBranchOp,
    OP::Mnemonic highOrderReversedBranchOp, OP::Mnemonic lowOrderBranchOp,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR::Node *secondChild = node->getSecondChild();
        ...
        if (lowValue <= highValue) {
            ...
            if (lowValue == highValue) {
                ...
                INST_RegMem(OP::LEA4RegMem, node, lowRegister,
                    MREF_Bdisp32(highRegister, lowValue - highValue, cg), cg);
            }
        } else {
            ...
            INST_RegMem(OP::LEA4RegMem, node, highRegister,
                MREF_Bdisp32(lowRegister, highValue - lowValue, cg), cg);
        }
    } else {
        lowRegister = loadConstant(node, lowValue, TR_RematerializableInt, cg);
        highRegister = loadConstant(node, highValue, TR_RematerializableInt, cg);
        ...
        if (valueReg) {
            ...
            if (isVolatile) {
                if (performTransformation(comp, "O^O Using SSE for volatile store %s\n",
                    ...
                    instr = INST_MemReg(OP::S4MemReg, node, stackLow, valueReg->getLowOrder(),
                        cg);
                    ...
                    INST_Mem(cg->comp()->target().isSMP() ? OP::LCMPXCHG8BMem
                                                                        : OP::CMPXCHG8BMem,
    ...
    if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL
        ...
        if (lowValue >= -128 && lowValue <= 127) {
            if (isMemOp)
                ...
                instr = INST_RegImm(OP::ADD4RegImms, node, targetRegister->getLowOrder(),
                    lowValue, cg);
        } else if (lowValue == 128) {
            if (isMemOp)
                instr = INST_MemImm(OP::SUB4MemImms, node, lowMR, -128, cg);
            else
                instr = INST_RegImm(OP::SUB4RegImms, node, targetRegister->getLowOrder(),
                    -128, cg);
            ...
            if (isMemOp)
                ...
                instr = INST_RegImm(OP::ADD4RegImm4, node, targetRegister->getLowOrder(),
                    lowValue, cg);
        }
    ...
    if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL
        ...
        if (lowValue >= -128 && lowValue <= 127) {
            ...
            if (isMemOp)
                ...
                instr = INST_RegImm(OP::SUB4RegImm4, node, targetRegister->getLowOrder(),
                    lowValue, cg);
        }
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairShlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        if (value == 0) {
            ...
            INST_RegReg(OP::MOV4RegReg, node, targetRegister->getHighOrder(),
            ...
            INST_RegRegImm(OP::SHLD4RegRegImm1, node, targetRegister->getHighOrder(),
            ...
            if (value < 32) {
                ...
                if (value != 32) {
                    value -= 32;
                    INST_RegImm(OP::SHL4RegImm1, node, targetRegister->getLowOrder(), value,
                        cg);
                }
        ...
        INST_RegReg(OP::SHLD4RegRegCL, node, targetRegister->getHighOrder(),
        ...
        INST_RegReg(OP::CMOVNE4RegReg, node, targetRegister->getHighOrder(),
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (value != 0) {
            ...
            if (value != 0) {
                ...
                INST_RegReg(OP::MOV4RegReg, node, tempHighReg, targetRegister->getHighOrder(),
                    cg);
                ...
                INST_RegRegImm(OP::SHLD4RegRegImm1, node, targetRegister->getLowOrder(),
        ...
        INST_RegReg(OP::SHLD4RegRegCL, node, targetRegister->getHighOrder(),
            targetRegister->getLowOrder(), shiftDependencies, cg);
        INST_RegReg(OP::SHLD4RegRegCL, node, targetRegister->getLowOrder(), scratchReg,
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairShrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        if (value == 0)
            ...
            INST_RegRegImm(OP::SHRD4RegRegImm1, node, targetRegister->getLowOrder(),
            ...
            INST_RegReg(OP::MOV4RegReg, node, targetRegister->getLowOrder(),
        ...
        TR::Register *scratchReg = cg->allocateRegister();
        INST_RegReg(OP::SHRD4RegRegCL, node, targetRegister->getLowOrder(),
        ...
        INST_RegImm(OP::TEST1RegImm1, node, shiftAmountReg, 32, cg);
        INST_RegReg(OP::CMOVNE4RegReg, node, targetRegister->getLowOrder(),
            targetRegister->getHighOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::landEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (lowValue == -1)
            ...
            if (isMemOp) {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? OP::AND4MemImms : OP::AND4MemImm4;
                lowInstr = INST_MemImm(opCode, node, lowMR, lowValue, cg);
            } else {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? OP::AND4RegImms : OP::AND4RegImm4;
        ...
        if (highValue == -1)
            ...
            if (isMemOp) {
                opCode = (highValue >= -128 && highValue <= 127) ? OP::AND4MemImms
                                                                 : OP::AND4MemImm4;
                highInstr = INST_MemImm(opCode, node, highMR, highValue, cg);
            } else {
                opCode = (highValue >= -128 && highValue <= 127) ? OP::AND4RegImms
                                                                 : OP::AND4RegImm4;
        ...
        temp.genericLongAnalyser(node, OP::AND4RegReg, OP::AND4RegReg,
            OP::AND4RegMem, OP::AND2RegMem, OP::AND1RegMem,
...
TR::Register *OMR::X86::I386::TreeEvaluator::lorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (lowValue == 0)
            ...
            if (isMemOp) {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? OP::OR4MemImms : OP::OR4MemImm4;
                lowInstr = INST_MemImm(opCode, node, lowMR, lowValue, cg);
            } else {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? OP::OR4RegImms : OP::OR4RegImm4;
                lowInstr = INST_RegImm(opCode, node, lowReg, lowValue, cg);
            }
        }
        ...
        if (highValue == 0)
            ...
            if (isMemOp) {
                opCode
                    = (highValue >= -128 && highValue <= 127) ? OP::OR4MemImms : OP::OR4MemImm4;
                highInstr = INST_MemImm(opCode, node, highMR, highValue, cg);
            } else {
                opCode
                    = (highValue >= -128 && highValue <= 127) ? OP::OR4RegImms : OP::OR4RegImm4;
                highInstr = INST_RegImm(opCode, node, highReg, highValue, cg);
            }
        }
        ...
        temp.genericLongAnalyser(node, OP::OR4RegReg, OP::OR4RegReg, OP::OR4RegMem,
            OP::OR2RegMem, OP::OR1RegMem, OP::OR4RegMem,
            OP::MOV4RegReg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lxorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (lowValue == 0)
            ...
            if (isMemOp) {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? OP::XOR4MemImms : OP::XOR4MemImm4;
                lowInstr = INST_MemImm(opCode, node, lowMR, lowValue, cg);
            } else {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? OP::XOR4RegImms : OP::XOR4RegImm4;
                lowInstr = INST_RegImm(opCode, node, lowReg, lowValue, cg);
            }
        }
        ...
        if (highValue == 0)
            ...
            if (isMemOp) {
                opCode = (highValue >= -128 && highValue <= 127) ? OP::XOR4MemImms
                                                                 : OP::XOR4MemImm4;
                highInstr = INST_MemImm(opCode, node, highMR, highValue, cg);
            } else {
                opCode = (highValue >= -128 && highValue <= 127) ? OP::XOR4RegImms
                                                                 : OP::XOR4RegImm4;
                highInstr = INST_RegImm(opCode, node, highReg, highValue, cg);
            }
        }
        ...
        temp.genericLongAnalyser(node, OP::XOR4RegReg, OP::XOR4RegReg,
            OP::XOR4RegMem, OP::XOR2RegMem, OP::XOR1RegMem,
...
TR::Register *OMR::X86::I386::TreeEvaluator::lcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getReferenceCount() != 1) {
                targetRegister = cg->allocateRegister();
                INST_RegReg(OP::MOV4RegReg, node, targetRegister, cmpRegister->getLowOrder(),
                    cg);
            }
...
TR::Register *OMR::X86::I386::TreeEvaluator::b2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        INST_RegReg(OP::MOVSXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::bu2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        INST_RegReg(OP::MOVZXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::s2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        node->setRegister(longReg);
        INST_RegReg(OP::MOVSXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::su2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        node->setRegister(longReg);
        INST_RegReg(OP::MOVZXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(),
            cg);
    }
    ...
    if ((valueChild->getOpCodeValue() == TR::lbits2d) && !valueChild->getRegister()) {
        ...
        if (valueChild->getOpCode().isLoadConst()) {
            instr = INST_MemImm(OP::S4MemImm4, node,
                MREF_MREFoff(*storeLowMR, 4, cg), valueChild->getLongIntHigh(), cg);
            ...
            INST_RegMem(OP::L4RegMem, node, tempRegister,
                MREF_MREFoff(*loadLowMR, 4, cg), cg);
            instr = INST_MemReg(OP::S4MemReg, node,
                MREF_MREFoff(*storeLowMR, 4, cg), tempRegister, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::l2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        INST_MemReg(OP::S4MemReg, node, MREF_MREFoff(*lowMR, 4, cg),
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (isVolatile || (symRef && symRef->isUnresolved())) {
        ...
        highRegister
            = loadMemory(node, MREF_MREFoff(*sourceMR, 4, cg), TR_RematerializableInt, false, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lbits2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && child->getReferenceCount() == 1) {
        ...
        INST_MemReg(OP::S4MemReg, node, MREF_MREFoff(*tempMR, 4, cg),
            longReg->getHighOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::dbits2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && (child->getReferenceCount() == 1)) {
        ...
        INST_RegMem(OP::L4RegMem, node, highReg, MREF_MREFoff(*tempMR, 4, cg),
            cg);
        ...
        INST_RegMem(OP::L4RegMem, node, lowReg, MREF_MREFoff(*tempMR, 0, cg),
            cg);
        INST_RegMem(OP::L4RegMem, node, highReg, MREF_MREFoff(*tempMR, 4, cg),
            cg);
    }
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getOpCodeValue() == TR::land && firstChild->getReferenceCount() == 1
                ...
                if (firstChild->getReferenceCount() != 1) {
                    targetRegister = cg->allocateRegister();
                    INST_RegReg(OP::MOV4RegReg, node, targetRegister,
                        cmpRegister->getLowOrder(), cg);
                    targetNeedsToBeExplicitlyStopped = true;
                }
                INST_RegReg(OP::OR4RegReg, node, targetRegister, cmpRegister->getHighOrder(),
                    cg);
            }
...
TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    if (generateLAddOrSubForOverflowCheck(node, cg)) {
        ...
        OP::Mnemonic reverseCompareOp
            = node->getOpCode().isUnsigned() ? OP::JA4 : OP::JG4;
...
TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    if (generateLAddOrSubForOverflowCheck(node, cg)) {
        ...
        OP::Mnemonic reverseCompareOp
            = node->getOpCode().isUnsigned() ? OP::JB4 : OP::JL4;
...
TR::Register *OMR::X86::I386::TreeEvaluator::lcmpsetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_Mem(cg->comp()->target().isSMP() ? OP::LCMPXCHG8BMem : OP::CMPXCHG8BMem,
        node, memRef, deps, cg);
