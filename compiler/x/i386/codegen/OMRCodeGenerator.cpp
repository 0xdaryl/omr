...
OMR::X86::I386::CodeGenerator::pickRegister(
   ...
   if (!comp->getOption(TR_DisableRegisterPressureSimulation))
      ...
      if (comp->getOption(TR_AssignEveryGlobalRegister))
         ...
         //
         availableRegisters -= *self()->getGlobalRegisters(TR_vmThreadSpill, comp->getMethodSymbol()->getLinkageConvention());
         }
   ...
   if (!_assignedGlobalRegisters)
      _assignedGlobalRegisters = new (self()->trStackMemory()) TR_BitVector(comp->getSymRefCount(), self()->trMemory(), stackAlloc, growable);
