...
enum OpCodePrefixes : OpCodePropertiesWidth_t {
    ...
    // For VEX encoded instructions only. VEX instructions may re-use the Prefix
    // field and require their own unique encoding to avoid enum name clashes.
    // These flags are mutually exclusive.
    //
    opc_VEX_PP_NP = 0,
    opc_VEX_PP_66 = 1,
...
enum OpCodeVEX_PP : OpCodePropertiesWidth_t {
...
enum OpCodeFlags : OpCodePropertiesWidth_t {
    ...
    // Some opcodes have custom overrides of EVEX and ModRM fields that require
    // special handling during binary encoding
    //
    opc_Flag_SpecializedEncoding = 0x08,
};
...
struct OpCodeProperties {
    ...
    OpCodeEEVEX_ND          opc_eevex_ND : 1;
    OpCodeEEVEX_NF          opc_eevex_NF : 1;
    ...
    //
    OpCodeVEX_PP            opc_promoted2evex_pp : 2;
