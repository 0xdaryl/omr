...
enum OpCodePrefixes : OpCodePropertiesWidth_t {
    ...
    // For VEX/EVEX encoded instructions only. VEXi/EVEX instructions may re-use
    // the Prefix field and require their own unique encoding to avoid enum name
    // clashes.
    // These flags are mutually exclusive.
    //
    opc_PP_NP = 0,
...
enum OpCodeEVEX_PP : OpCodePropertiesWidth_t {
    // For EVEX encoded instructions. These prefixes are in a separate enum than
    // the other prefixes to enable a smaller 2-bit bitfield.
    // These flags are mutually exclusive.
    //
    opc_EVEX_PP_NP = 0,
    opc_EVEX_PP_66 = 1,
...
enum OpCodeStatusRFlags : OpCodePropertiesWidth_t {
    // These flags are NOT mutually exclusive
    //
    opc_NoRFlags = 0x000, ///< Does not test or set any RFlag status flags
...
#define OPC_RFLAGS(x) static_cast<OpCodeStatusRFlags>(x)
...
enum OpCodeVectorLength : OpCodePropertiesWidth_t {
    // These flags are mutually exclusive
    //
    opc_LLZ       = 0, ///< LL must be zero
    opc_LL_Scalar = 0,
    opc_LL_128    = 0,
    opc_LL_256    = 1,
    opc_LL_512    = 2,
...
enum OpCode_b_or_ND : OpCodePropertiesWidth_t {
    // These flags are mutually exclusive
    //
    opc_b_or_ND_None = 0, ///< Default, b nor ND applies
    opc_ND0          = 0, ///< ND = 0
    opc_ND1          = 1, ///< ND = 1
    opc_b0           = 0, ///< b = 0
    opc_b1           = 1, ///< b = 1
...
enum OpCode_NF : OpCodePropertiesWidth_t {
    // These flags are mutually exclusive
    //
    opc_NF_None = 0, ///< Default, NF does not apply
    opc_NF0     = 0, ///< NF = 0
    opc_NF1     = 1, ///< NF = 1
};

enum OpCode_z : OpCodePropertiesWidth_t {
    // These flags are mutually exclusive
    //
    opc_z_None = 0, ///< Default, z does not apply
    opc_z0     = 0, ///< z = 0
    opc_z1     = 1, ///< z = 1
};

enum OpCodeFlags : OpCodePropertiesWidth_t {
    // These flags are NOT mutually exclusive
    //
    opc_NoFlags                  = 0x00,

    // Aggregate 3 bits for OpCode kind
    opc_Kind_Branch              = 0x01,
    opc_Kind_StackPush           = 0x02,
    opc_Kind_StackPop            = 0x03,
    opc_Kind_Shift               = 0x04,
    opc_Kind_Rotate              = 0x05,
    opc_Kind_Call                = 0x06,
    opc_Kind_Pseudo              = 0x07,
    opc_Kind_Mask                = 0x07,

    // One of the register operands is encoded in the opcode byte
    opc_RegInOpCode              = 0x08,
};

#define OPC_FLAGS(x) static_cast<OpCodeFlags>(x)

enum OpCodeTupleType : OpCodePropertiesWidth_t {
    // These flags are mutually exclusive
    //

    // Compressed displacement (disp8 * N) for instructions not affected by
    // embedded broadcast
    //
    opc_TupleType_NoScale         = 0,
    opc_TupleType_FullMem         = 1,
    opc_TupleType_Tuple1Scalar_8  = 2,
    opc_TupleType_Tuple1Scalar_16 = 3,
    opc_TupleType_Tuple1Scalar_32 = 4,
    opc_TupleType_Tuple1Scalar_64 = 5,
    opc_TupleType_Tuple1Fixed_32  = 6,
    opc_TupleType_Tuple1Fixed_64  = 7,
    opc_TupleType_Tuple2_32       = 8,
    opc_TupleType_Tuple2_64       = 9,
    opc_TupleType_Tuple4_32       = 10,
    opc_TupleType_Tuple4_64       = 11,
    opc_TupleType_Tuple8_32       = 12,
    opc_TupleType_HalfMem         = 13,
    opc_TupleType_QuarterMem      = 14,
    opc_TupleType_EighthMem       = 15,
    opc_TupleType_Mem128          = 16,
    opc_TupleType_MOVDDUP         = 17,

    // Compressed displacement (disp8 * N) for instructions affected by
    // embedded broadcast
    //
    opc_TupleType_Full_b0_32      = 18,
    opc_TupleType_Full_b1_32      = 19,
    opc_TupleType_Full_b0_64      = 20,
    opc_TupleType_Full_b1_64      = 21,
    opc_TupleType_Half_b0_32      = 22,
    opc_TupleType_Half_b1_32      = 23,
};
...
struct OpCodeProperties {
    ...
    OpCode_W                opc_W : 1;
    OpCodeEncodingPrefix    opc_validEncPrefixes : 8;
    OpCodeStatusRFlags      opc_statusRFlags : 12;

    // The remaining fields are deliberately last to avoid explicit initialization
    // for less common properties in the OpCodeTable
    //
    OpCodeFlags             opc_flags : 4;

    OpCodeVectorLength      opc_LL : 2;
    OpCodeTupleType         opc_tupleType : 5;
    OpCode_b_or_ND          opc_b_or_ND : 1;
    OpCode_NF               opc_NF : 1;
    OpCode_z                opc_z : 1;

    // For legacy encodings promoted to EVEX. Only 2 bits are required for the
    // promoted prefix because only the opc_VEX_ family of prefixes is allowed.
    //
    OpCodeEVEX_PP           opc_promoted2evex_pp : 2;
    OpCodePropertiesWidth_t opc_promoted2evex_byte : 8;
    // 64 used; 0 avail
};
...
enum OperandKind : OperandPropertiesWidth_t {
    ...
    opnd_RM  = 2, ///< Register/Memory operand
    opnd_Imm = 3, ///< Immediate operand
};
...
enum OperandDataType : OperandPropertiesWidth_t {
    ...
    opnd_Mask  = 2, ///< Operand is an opmask
};
