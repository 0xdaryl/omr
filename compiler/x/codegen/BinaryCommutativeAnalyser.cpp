    ...
    bool nonClobberingDestination) // false by default
{
    ...
    TR::Register *tempReg = genericAnalyserImpl(root, firstChild, secondChild, regRegOpCode, regMemOpCode, copyOpCode,
        nonClobberingDestination);
    ...
    bool nonClobberingDestination) // false by default
{
    ...
    targetRegister = genericAnalyserImpl(root, firstChild, secondChild, regRegOpCode, regMemOpCode, copyOpCode,
        nonClobberingDestination);
    ...
    bool nonClobberingDestination)
{
    ...
    if (getOpReg1Reg2()) {
        ...
        INST_RegReg(regRegOpCode, root, secondRegister, firstRegister, _cg);
        targetRegister = secondRegister;
        notReversedOperands();
    } else if (getCopyReg1()) {
        TR::Register *tempReg = _cg->allocateRegister();
        targetRegister = tempReg;
        INST_RegReg(copyOpCode, root, tempReg, firstRegister, _cg);
        INST_RegReg(regRegOpCode, root, tempReg, secondRegister, _cg);
    } else if (getCopyReg2()) {
        TR::Register *tempReg = _cg->allocateRegister();
        targetRegister = tempReg;
        INST_RegReg(copyOpCode, root, tempReg, secondRegister, _cg);
...
void TR_X86BinaryCommutativeAnalyser::genericLongAnalyser(TR::Node *root, OP::Mnemonic lowRegRegOpCode,
    OP::Mnemonic highRegRegOpCode, OP::Mnemonic lowRegMemOpCode, OP::Mnemonic lowRegMemOpCode2Byte,
    OP::Mnemonic lowRegMemOpCode1Byte, OP::Mnemonic highRegMemOpCode, OP::Mnemonic copyOpCode)
{
    TR::Node *firstChild;
    TR::Node *secondChild;
    if (_cg->whichChildToEvaluate(root) == 0) {
    ...
    if (getEvalChild2()) {
        ...
        if (bypassedSecondUnsignedConversionChild)
            zeroExtendTo32BitRegister(bypassedSecondUnsignedConversionChild, secondRegister, secondChild->getSize(),
                _cg);
    }
    ...
    if (getOpReg1Reg2()) {
        ...
        INST_RegReg(lowRegRegOpCode, root, oneLow, twoLow, _cg);
            ...
            if (secondHighZero) {
                ...
                if (root->getOpCodeValue() == TR::land) {
                    ...
                    if (secondChild->getReferenceCount() == 1) {
                        ...
                        oneHigh = _cg->allocateRegister();
                        INST_RegReg(copyOpCode, root, oneHigh, twoHigh, _cg);
                    }
                }
            }
        ...
        INST_RegReg(lowRegRegOpCode, root, twoLow, oneLow, _cg);
        ...
        if (firstHighZero) {
            ...
            INST_RegReg(highRegRegOpCode, root, twoHigh, oneHigh, _cg);
        }
        ...
        INST_RegReg(copyOpCode, root, copyLow, temp, _cg);
        ...
        INST_RegReg(lowRegRegOpCode, root, copyLow, sourceLow, _cg);
        ...
        if (copyHighZero) {
            ...
            INST_RegReg(copyOpCode, root, copyHigh, copyReg->getHighOrder(), _cg);
            INST_RegReg(highRegRegOpCode, root, copyHigh, sourceHigh, _cg);
        }
        ...
        INST_RegMem(lowRegMemOpCode, root, targetLow, lowMR, _cg);
...
TR::Register *TR_X86BinaryCommutativeAnalyser::integerAddAnalyserImpl(TR::Node *root, TR::Node *firstChild,
    TR::Node *secondChild, OP::Mnemonic regRegOpCode, OP::Mnemonic regMemOpCode, bool needsEflags, TR::Node *carry)
{
    TR::Register *targetRegister;
 ...
 *     the operands for the add operation
 */
void TR_X86BinaryCommutativeAnalyser::longAddAnalyserWithExplicitOperands(TR::Node *root, TR::Node *firstChild,
    TR::Node *secondChild)
{
    TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
 ...
 * directly
 */
TR::Register *TR_X86BinaryCommutativeAnalyser::longAddAnalyserImpl(TR::Node *root, TR::Node *&firstChild,
    TR::Node *&secondChild)
{
    ...
    if (getOpReg1Reg2()) {
        ...
        INST_RegReg(regRegOpCode, root, twoLow, oneLow, _cg);
        if (firstHighZero) {
            if (secondHighZero) {
                twoHigh = _cg->allocateRegister();
                // xor would reset the carry we need for the subsequent adc instruction
...
void TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser(TR::Node *root)
{
    ...
    if (getOpReg1Reg2()) {
        ...
        if (!secondHighZero) {
            ...
            if (getOpReg2Reg1() == false) {
                twoHigh = _cg->allocateRegister();
                INST_RegReg(OP::MOV4RegReg, root, twoHigh, secondRegister->getHighOrder(), _cg);
            } else {
                twoHigh = secondRegister->getHighOrder();
            }
        } else {
