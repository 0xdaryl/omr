...
TR::Register *OMR::X86::TreeEvaluator::fconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getFloatBits() == 0) {
        ...
        TR::MemoryReference *tempMR
            = MREF_const(cg->findOrCreate4ByteConstant(node, node->getFloatBits()), cg);
        TR::Instruction *instr
            = INST_RegMem(OP::MOVSSRegMem, node, targetRegister, tempMR, cg);
...
TR::Register *OMR::X86::TreeEvaluator::dconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getLongInt() == 0) {
        ...
        TR::MemoryReference *tempMR
            = MREF_const(cg->findOrCreate8ByteConstant(node, node->getLongInt()), cg);
...
TR::Register *OMR::X86::TreeEvaluator::floatingPointStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (valueChild->getOpCode().isLoadConst()) {
        if (nodeIs64Bit) {
            if (cg->comp()->target().is64Bit()) {
                ...
                if (valueChild->getLongInt() == 0) {
                    ...
                    INST_RegImm64(OP::MOV8RegImm64, node, floatConstReg,
                ...
                exceptionPoint = INST_MemImm(OP::S4MemImm4, node, tempMR,
                    valueChild->getLongIntLow(), cg);
                INST_MemImm(OP::S4MemImm4, node, MREF_MREFoff(*tempMR, 4, cg),
                    valueChild->getLongIntHigh(), cg);
            }
        } else {
            exceptionPoint
                = INST_MemImm(OP::S4MemImm4, node, tempMR, valueChild->getFloatBits(), cg);
        }
        ...
        exceptionPoint
            = INST_MemReg(OP::SMemReg(nodeIs64Bit), node, tempMR, tempRegister, cg);
        ...
        if (cg->comp()->target().is64Bit() && tempMR->getSymbolReference().isUnresolved()) {
            if (!tempMR->getSymbolReference().getSymbol()->isShadow()
                ...
                //
                exceptionPoint = INST_MemReg(OP::MOVSMemReg(nodeIs64Bit), node, tempMR,
                    sourceRegister, cg);
            } else {
                ...
                exceptionPoint
                    = INST_MemReg(OP::MOVSMemReg(nodeIs64Bit), node, mr, sourceRegister, cg);

                tempMR->setProcessAsFPVolatile();

                if (cg->comp()->getOption(TR_X86UseMFENCE))
                    insertUnresolvedReferenceInstructionMemoryBarrier(cg, OP::MFENCE, exceptionPoint,
            ...
            TR_ASSERT(nodeIs64Bit != sourceRegister->isSinglePrecision(),
                "Wrong operand type to floating point store\n");
            exceptionPoint
                = INST_MemReg(OP::MOVSMemReg(nodeIs64Bit), node, tempMR, sourceRegister, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::fpSqrtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_RegReg(value->isSinglePrecision() ? OP::SQRTSSRegReg : OP::SQRTSDRegReg,
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (child->getOpCode().isDouble()) {
        ...
        //
        loadHighInstr = INST_RegMem(OP::L4RegMem, node, highReg,
            MREF_MREFoff(*convertedLongMR, 4, cg), cg);

        loadLowInstr = INST_RegMem(OP::L4RegMem, node, lowReg,
...
TR::Register *OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...

    {
        ...
        INST_RegMem(doubleSource ? OP::UCOMISDRegMem : OP::UCOMISSRegMem, node,
            sourceRegister,
            MREF_const(
...
TR::Register *OMR::X86::TreeEvaluator::fbits2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && (child->getReferenceCount() == 1)) {
        ...
        INST_MemReg(OP::MOVSSMemReg, node, tempMR, floatReg, cg);
        INST_RegMem(OP::L4RegMem, node, target, MREF_MREFoff(*tempMR, 0, cg),
            cg);
    }
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu || cmpOp == TR::ifdcmpneu || cmpOp == TR::dcmpneu) {
        if (generateBranch) {
            ...
            INST_Label(OP::JNE4, node, node->getBranchDestination()->getNode()->getLabel(),
                deps1, cg);
            INST_Label(OP::JPE4, node, node->getBranchDestination()->getNode()->getLabel(),
                deps, cg);
        } else {
        ...
        if (generateBranch) {
            ...
            INST_Label(OP::JE4, node, node->getBranchDestination()->getNode()->getLabel(),
                cg);
...
{
    TR::TreeEvaluator::compareFloatOrDoubleForOrder(node, OP::UCOMISSRegReg, OP::UCOMISSRegMem,
        cg);
...
{
    TR::TreeEvaluator::compareFloatOrDoubleForOrder(node, OP::UCOMISDRegReg, OP::UCOMISDRegMem,
        cg);
    return TR::TreeEvaluator::generateBranchOrSetOnFPCompare(node, true, cg);
}
