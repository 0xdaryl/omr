...
TR::Register *OMR::X86::TreeEvaluator::fconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getFloatBits() == 0) {
        ...
        TR::Instruction *instr = Inst_RegMem(TR::InstOpCode::MOVSSRegMem, node, targetRegister, tempMR, cg);
...
TR::Register *OMR::X86::TreeEvaluator::floatingPointStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (valueChild->getOpCode().isLoadConst()) {
        if (nodeIs64Bit) {
            if (cg->comp()->target().is64Bit()) {
                ...
                if (valueChild->getLongInt() == 0) {
                    ...
                    Inst_RegImm64(TR::InstOpCode::MOV8RegImm64, node, floatConstReg, valueChild->getLongInt(), cg);
                }
                exceptionPoint = Inst_MemReg(TR::InstOpCode::S8MemReg, node, tempMR, floatConstReg, cg);
                cg->stopUsingRegister(floatConstReg);
            } else {
                exceptionPoint = Inst_MemImm(TR::InstOpCode::S4MemImm4, node, tempMR, valueChild->getLongIntLow(), cg);
                Inst_MemImm(TR::InstOpCode::S4MemImm4, node, generateX86MemoryReference(*tempMR, 4, cg),
                    valueChild->getLongIntHigh(), cg);
            }
        } else {
            exceptionPoint = Inst_MemImm(TR::InstOpCode::S4MemImm4, node, tempMR, valueChild->getFloatBits(), cg);
        }
        ...
        exceptionPoint = Inst_MemReg(TR::InstOpCode::SMemReg(nodeIs64Bit), node, tempMR, tempRegister, cg);
        ...
        if (cg->comp()->target().is64Bit() && tempMR->getSymbolReference().isUnresolved()) {
            if (!tempMR->getSymbolReference().getSymbol()->isShadow()
                ...
                //
                exceptionPoint = Inst_MemReg(TR::InstOpCode::MOVSMemReg(nodeIs64Bit), node, tempMR, sourceRegister, cg);
            } else {
                ...
                exceptionPoint = Inst_MemReg(TR::InstOpCode::MOVSMemReg(nodeIs64Bit), node, mr, sourceRegister, cg);
...
TR::Register *OMR::X86::TreeEvaluator::fpSqrtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    Inst_RegReg(value->isSinglePrecision() ? TR::InstOpCode::SQRTSSRegReg : TR::InstOpCode::SQRTSDRegReg, node, result,
        value, cg);
...
TR::Register *OMR::X86::TreeEvaluator::fbits2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && (child->getReferenceCount() == 1)) {
        ...
        Inst_MemReg(TR::InstOpCode::MOVSSMemReg, node, tempMR, floatReg, cg);
        Inst_RegMem(TR::InstOpCode::L4RegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
    }
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu || cmpOp == TR::ifdcmpneu || cmpOp == TR::dcmpneu) {
        if (generateBranch) {
            ...
            Inst_Label(TR::InstOpCode::JNE4, node, node->getBranchDestination()->getNode()->getLabel(), deps1, cg);
            Inst_Label(TR::InstOpCode::JPE4, node, node->getBranchDestination()->getNode()->getLabel(), deps, cg);
        } else {
        ...
        if (generateBranch) {
            ...
            Inst_Label(TR::InstOpCode::JE4, node, node->getBranchDestination()->getNode()->getLabel(), cg);
