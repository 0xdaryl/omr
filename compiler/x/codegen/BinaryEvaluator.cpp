    ...
    if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
        ...
        if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL)
            ...
            if (targetRegister
                ...
                if (computesCarry) {
                    INST_RegReg(TR::InstOpCode::MOVRegReg(nodeIs64Bit), node, targetRegister, tempReg,
                        cg);
                    INST_RegImm(TR::InstOpCode::AddRegImm4(nodeIs64Bit, isWithCarry), node,
                ...
                if (constValue >= -128 && constValue <= 127) {
                    if (computesCarry) {
                        if (isMemOp)
                            instr = INST_MemImm(TR::InstOpCode::AddMemImms(nodeIs64Bit, isWithCarry),
                                node, tempMR, static_cast<int32_t>(constValue), cg);
                        else
                            instr = INST_RegImm(TR::InstOpCode::AddRegImms(nodeIs64Bit, isWithCarry),
                        ...
                        if (isMemOp)
                            ...
                            instr = INST_RegImm(TR::InstOpCode::ADDRegImms(nodeIs64Bit), node,
                                targetRegister, 1, cg);
                    } else if (constValue == -1) {
                        if (isMemOp)
                            instr = INST_Mem(TR::InstOpCode::DECMem(nodeIs64Bit), node, tempMR, cg);
                        else
                            instr = INST_RegImm(TR::InstOpCode::SUBRegImms(nodeIs64Bit), node,
                                targetRegister, 1, cg);
                    } else {
                        if (isMemOp)
                    ...
                    if (isMemOp)
                        instr = INST_MemImm(TR::InstOpCode::AddMemImm4(nodeIs64Bit, isWithCarry), node,
                            tempMR, static_cast<int32_t>(constValue), cg);
                    else
                        instr = INST_RegImm(TR::InstOpCode::AddRegImm4(nodeIs64Bit, isWithCarry), node,
                            targetRegister, static_cast<int32_t>(constValue), cg);
                }
                if (debug("traceMemOp"))
...
TR::Register *OMR::X86::TreeEvaluator::baddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, TR::InstOpCode::ADD1RegReg,
            TR::InstOpCode::ADD1RegMem, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
...
TR::Register *OMR::X86::TreeEvaluator::saddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, TR::InstOpCode::ADD2RegReg,
            TR::InstOpCode::ADD2RegMem, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
        ...
        if (targetRegister && firstChild->getReferenceCount() > 1) {
            ...
            if (value >= -128 && value <= 127) {
                ...
                if (isMemOp)
                    ...
                    instr = INST_RegImm(TR::InstOpCode::SUB4RegImms, node, targetRegister, (uint32_t)-128,
                        cg);
            } else {
                if (isMemOp)
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
        ...
        if (targetRegister && !computesCarry && firstChild->getReferenceCount() > 1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                if (computesCarry) {
                    if (isMemOp)
                        instr = INST_MemImm(TR::InstOpCode::SubMemImms(nodeIs64Bit, isWithBorrow), node,
                            tempMR, static_cast<int32_t>(constValue), cg);
                    else
                        instr = INST_RegImm(TR::InstOpCode::SubRegImms(nodeIs64Bit, isWithBorrow), node,
                            targetRegister, static_cast<int32_t>(constValue), cg);
                } else if (constValue == 1) {
                    if (isMemOp)
                        instr = INST_Mem(TR::InstOpCode::DECMem(nodeIs64Bit), node, tempMR, cg);
                ...
                if (isMemOp)
                    instr = INST_MemImm(TR::InstOpCode::SubMemImm4(nodeIs64Bit, isWithBorrow), node,
                        tempMR, static_cast<int32_t>(constValue), cg);
                else
                    instr = INST_RegImm(TR::InstOpCode::SubRegImm4(nodeIs64Bit, isWithBorrow), node,
                        targetRegister, static_cast<int32_t>(constValue), cg);
            }
...
TR::Register *OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
        ...
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, TR::InstOpCode::SUB1RegReg, TR::InstOpCode::SUB1RegMem,
...
TR::Register *OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDualCyclic() || needsHighMulOnly) {
        ...
        INST_RegReg(TR::InstOpCode::MUL8AccReg, node, lmulTargetRegister, lumulhTargetRegister,
            multDependencies, cg);
...
TR::Register *OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!targetRegister && dataType == TR::Int8) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isMemoryReference()) {
            ...
            INST_RegReg(TR::InstOpCode::IMUL1AccReg, node, targetRegister, tempRegister, multDependencies,
                cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_RegReg(TR::InstOpCode::IMULAccReg(nodeIs64Bit), node, eaxRegister, targetRegister,
        multDependencies, cg);
    ...
    if (isPowerOf2(dvalue)) {
        ...
        if (rootOpCode.isRem()) {
            ...
            if (isMinInt) {
                ...
                // is negative.
                //
                INST_RegReg(TR::InstOpCode::TESTRegReg(nodeIs64Bit), node, tempRegister, tempRegister,
                    cg);
            ...
            if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
                ...
                INST_RegImm(TR::InstOpCode::ORRegImm4(nodeIs64Bit), node, tempRegister, (uint32_t)-dvalue,
                    cg);
        ...
        if (!isMinInt) {
            ...
            if ((dividend->isNonNegative() == false) && (dvalue > 0)) {
                ...
                {
                    INST_RegReg(TR::InstOpCode::SUBRegReg(nodeIs64Bit), node, tempRegister, edxRegister,
                        cg);
                    ...
                    TR_ASSERT(mask >= 0,
                        ...
                        "of tempRegister");
                    INST_RegImm(TR::InstOpCode::ANDRegImm4(nodeIs64Bit), node, edxRegister,
                        (uint32_t)dvalue - 1, cg);
                    INST_RegReg(TR::InstOpCode::ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister,
                        cg);
                } else {
            ...
            INST_RegImm(TR::InstOpCode::SARRegImm1(nodeIs64Bit), node, tempRegister,
        ...
        if (!nodeIs64Bit || IS_32BIT_SIGNED(m))
            INST_RegImm(TR::InstOpCode::MOVRegImm4(nodeIs64Bit), node, eaxRegister,
                static_cast<int32_t>(m), cg);
        else
            INST_RegMem(TR::InstOpCode::LEA8RegMem, node, eaxRegister, generateX86MemoryReference(m, cg),
                cg);
        INST_RegReg(TR::InstOpCode::IMULAccReg(nodeIs64Bit), node, eaxRegister, dividendRegister,
            multDependencies, cg);
        ...
        // Do the magic shift
        //
        INST_RegImm(TR::InstOpCode::SARRegImm1(nodeIs64Bit), node, edxRegister, static_cast<int32_t>(s),
            cg);
        ...
        if (!(dividend->isNonNegative() && (dvalue > 0))) {
            ...
            INST_RegImm(TR::InstOpCode::SHRRegImm1(nodeIs64Bit), node, eaxRegister, nodeIs64Bit ? 63 : 31,
                cg);
        ...
        if (rootOpCode.isRem()) {
            ...
            INST_RegReg(TR::InstOpCode::SUBRegReg(nodeIs64Bit), node, dividendRegister, edxRegister,
                multDependencies, cg);
            cg->stopUsingRegister(edxRegister);
    ...
    if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
        ...
        if (!nodeIs64Bit && node->isUnsigned()) {
            ...
            if (divisorRegister) {
                ...
                divideInstr = INST_RegMem(TR::InstOpCode::DIVAccMem(nodeIs64Bit), node, eaxRegister,
            ...
            if (divisorRegister) {
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = INST_RegMem(TR::InstOpCode::IDIVAccMem(nodeIs64Bit), node, eaxRegister,
                        tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        }
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (shiftAmount == 0) {
            ...
            if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift()
                ...
                targetRegister = cg->allocateRegister();
                instr = INST_RegReg(TR::InstOpCode::MOVRegReg(nodeIs64Bit), node, targetRegister,
...
TR::Register *OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(node, TR::InstOpCode::SHLMemImm1(nodeIs64Bit),
            TR::InstOpCode::SHLMemCL(nodeIs64Bit), cg);
        ...
        TR::TreeEvaluator::generateRegisterShift(node, TR::InstOpCode::SHLRegImm1(nodeIs64Bit),
            TR::InstOpCode::SHLRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register *OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        INST_RegReg(TR::InstOpCode::ROLRegCL(nodeIs64Bit), node, targetRegister, rotateAmountReg,
...
TR::Register *OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(node, TR::InstOpCode::SARMemImm1(nodeIs64Bit),
            TR::InstOpCode::SARMemCL(nodeIs64Bit), cg);
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, TR::InstOpCode::SARRegImm1(nodeIs64Bit),
            TR::InstOpCode::SARRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register *OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::Instruction *shiftInstruction = TR::TreeEvaluator::generateMemoryShift(node,
            TR::InstOpCode::SHRMemImm1(nodeIs64Bit), TR::InstOpCode::SHRMemCL(nodeIs64Bit), cg);
        if (shiftInstruction)
            faultingInstruction = shiftInstruction;
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, TR::InstOpCode::SHRRegImm1(nodeIs64Bit),
            TR::InstOpCode::SHRRegCL(nodeIs64Bit), cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpCode == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(TR::InstOpCode::SHL1MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = INST_RegReg(TR::InstOpCode::SHL1RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(TR::InstOpCode::SHL2MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = INST_RegReg(TR::InstOpCode::SHL4RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(TR::InstOpCode::SAR1MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = INST_RegReg(TR::InstOpCode::SAR1RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(TR::InstOpCode::SAR2MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = INST_RegReg(TR::InstOpCode::SAR2RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode2 == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(TR::InstOpCode::SHR1MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = INST_RegReg(TR::InstOpCode::SHR1RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(TR::InstOpCode::SHR2MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = INST_RegReg(TR::InstOpCode::SHR2RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic package[],
    TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    TR::Register *targetRegister = NULL;
