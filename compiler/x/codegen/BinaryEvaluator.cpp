...
static void forceSize(TR::Node *node, TR::Register *reg, bool is64Bit, TR::CodeGenerator *cg)
{
    if (is64Bit && node->getSize() <= 4 && !node->isNonNegative()) {
        ...
        // already been done.
        INST_RegReg(OP::MOVSXReg8Reg4, node, reg, reg, cg);
    }
}
...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (indexNode) {
        ...
        targetRegister = TR::TreeEvaluator::generateLEAForLoadAddr(firstChild, leaMR, firstChild->getSymbolReference(),
            cg, isInternal);
        cg->decReferenceCount(firstChild);
        node->setRegister(targetRegister);
        return true;
...
TR::Register *OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node) || (opCode == TR::luaddc) || (opCode == TR::iuaddc)) {
        ...
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, OP::AddRegReg(nodeIs64Bit, isWithCarry),
            OP::AddRegMem(nodeIs64Bit, isWithCarry), true, /* produce eflags */
    ...
    if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
        ...
        if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL)
            ...
            if (targetRegister
                ...
                if (computesCarry) {
                    INST_RegReg(OP::MOVRegReg(nodeIs64Bit), node, targetRegister, tempReg, cg);
                ...
                if (constValue >= -128 && constValue <= 127) {
                    if (computesCarry) {
                        if (isMemOp)
                            instr = INST_MemImm(OP::AddMemImms(nodeIs64Bit, isWithCarry), node, tempMR,
                                static_cast<int32_t>(constValue), cg);
                        else
                        ...
                        if (isMemOp)
                            instr = INST_Mem(OP::DECMem(nodeIs64Bit), node, tempMR, cg);
                        else
                            instr = INST_RegImm(OP::SUBRegImms(nodeIs64Bit), node, targetRegister, 1, cg);
                    } else {
                        if (isMemOp)
                            instr = INST_MemImm(OP::ADDMemImms(nodeIs64Bit), node, tempMR,
                                static_cast<int32_t>(constValue), cg);
                        else
                            instr = INST_RegImm(OP::ADDRegImms(nodeIs64Bit), node, targetRegister,
                                static_cast<int32_t>(constValue), cg);
                    }
                } else if ((constValue == 128) && !computesCarry) {
                    if (isMemOp)
                        instr = INST_MemImm(OP::SUBMemImms(nodeIs64Bit), node, tempMR, (uint32_t)-128, cg);
                    else
                        instr = INST_RegImm(OP::SUBRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
                } else {
                    // comp()->useCompressedPointers
                    // ladd
                    //    iu2l
                    ...
                    if (isMemOp)
                        instr = INST_MemImm(OP::AddMemImm4(nodeIs64Bit, isWithCarry), node, tempMR,
                            static_cast<int32_t>(constValue), cg);
                    else
                        instr = INST_RegImm(OP::AddRegImm4(nodeIs64Bit, isWithCarry), node, targetRegister,
                            static_cast<int32_t>(constValue), cg);
                }
                if (debug("traceMemOp"))
            ...
            instr = INST_MemReg(OP::AddMemReg(nodeIs64Bit, isWithCarry), node, tempMR, cg->evaluate(secondChild), cg);
            if (debug("traceMemOp"))
        ...
        if (isMemOp) {
            ...
            temp.integerAddAnalyser(node, OP::AddRegReg(nodeIs64Bit, isWithCarry),
                OP::AddRegMem(nodeIs64Bit, isWithCarry), computesCarry, 0);
            targetRegister = node->getRegister();
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::baddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, OP::ADD1RegReg, OP::ADD1RegMem,
            true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
        ...
        if (targetRegister && firstChild->getReferenceCount() > 1) {
            ...
            if (value == 1) {
                if (isMemOp)
                    instr = INST_Mem(OP::INC1Mem, node, tempMR, cg);
                else
                    instr = INST_Reg(OP::INC1Reg, node, targetRegister, cg);
            } else if (value == -1) {
                if (isMemOp)
                    instr = INST_Mem(OP::DEC1Mem, node, tempMR, cg);
                else
                    instr = INST_Reg(OP::DEC1Reg, node, targetRegister, cg);
            } else {
                if (isMemOp)
                    instr = INST_MemImm(OP::ADD1MemImm1, node, tempMR, value, cg);
                else
                    instr = INST_RegImm(OP::ADD1RegImm1, node, targetRegister, value, cg);
            }
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d", node, value);
        }
    } else if (isMemOp) {
        instr = INST_MemReg(OP::ADD1MemReg, node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by var", node);
    } else {
        TR_X86BinaryCommutativeAnalyser temp(cg);
        temp.integerAddAnalyser(node, OP::ADD1RegReg, OP::ADD1RegMem);
        targetRegister = node->getRegister();
...
TR::Register *OMR::X86::TreeEvaluator::saddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, OP::ADD2RegReg, OP::ADD2RegMem,
            true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
        ...
        if (targetRegister && firstChild->getReferenceCount() > 1) {
            ...
            if (value >= -128 && value <= 127) {
                if (value == 1) {
                    if (isMemOp)
                        instr = INST_Mem(OP::INC2Mem, node, tempMR, cg);
                    else
                        instr = INST_Reg(OP::INC4Reg, node, targetRegister, cg);
                } else if (value == -1) {
                    if (isMemOp)
                        instr = INST_Mem(OP::DEC2Mem, node, tempMR, cg);
                    else
                        instr = INST_Reg(OP::DEC4Reg, node, targetRegister, cg);
                } else {
                    if (isMemOp)
                        instr = INST_MemImm(OP::ADD2MemImms, node, tempMR, value, cg);
                    else
                        instr = INST_RegImm(OP::ADD4RegImms, node, targetRegister, value, cg);
                }
            } else if (value == 128) {
                if (isMemOp)
                    instr = INST_MemImm(OP::SUB2MemImms, node, tempMR, (uint32_t)-128, cg);
                else
                    instr = INST_RegImm(OP::SUB4RegImms, node, targetRegister, (uint32_t)-128, cg);
            } else {
                if (isMemOp)
                    instr = INST_MemImm(OP::ADD2MemImm2, node, tempMR, value, cg);
                else
                    instr = INST_RegImm(OP::ADD2RegImm2, node, targetRegister, value, cg);
            }
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d", node, value);
        }
    } else if (isMemOp) {
        instr = INST_MemReg(OP::ADD2MemReg, node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by var", node);
    } else {
        TR_X86BinaryCommutativeAnalyser temp(cg);
        temp.integerAddAnalyser(node, OP::ADD4RegReg, OP::ADD2RegMem);
        return node->getRegister();
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb) || (node->getOpCodeValue() == TR::iusubb)) {
        ...
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, OP::SubRegReg(nodeIs64Bit, isWithBorrow),
            OP::SubRegMem(nodeIs64Bit, isWithBorrow), OP::MOVRegReg(nodeIs64Bit), true,
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
        ...
        if (targetRegister && !computesCarry && firstChild->getReferenceCount() > 1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                if (computesCarry) {
                    if (isMemOp)
                        instr = INST_MemImm(OP::SubMemImms(nodeIs64Bit, isWithBorrow), node, tempMR,
                            static_cast<int32_t>(constValue), cg);
                    else
                        instr = INST_RegImm(OP::SubRegImms(nodeIs64Bit, isWithBorrow), node, targetRegister,
                            static_cast<int32_t>(constValue), cg);
                } else if (constValue == 1) {
                    if (isMemOp)
                        instr = INST_Mem(OP::DECMem(nodeIs64Bit), node, tempMR, cg);
                    ...
                    if (isMemOp)
                        instr = INST_MemImm(OP::SUBMemImms(nodeIs64Bit), node, tempMR, static_cast<int32_t>(constValue),
                            cg);
                    else
                        instr = INST_RegImm(OP::SUBRegImms(nodeIs64Bit), node, targetRegister,
                            static_cast<int32_t>(constValue), cg);
                }
            } else if ((constValue == 128) && !computesCarry) {
                if (isMemOp)
                    instr = INST_MemImm(OP::ADDMemImms(nodeIs64Bit), node, tempMR, (uint32_t)-128, cg);
                else
                    instr = INST_RegImm(OP::ADDRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
            } else {
                if (isMemOp)
                    instr = INST_MemImm(OP::SubMemImm4(nodeIs64Bit, isWithBorrow), node, tempMR,
                        static_cast<int32_t>(constValue), cg);
                else
                    instr = INST_RegImm(OP::SubRegImm4(nodeIs64Bit, isWithBorrow), node, targetRegister,
                        static_cast<int32_t>(constValue), cg);
            }
        ...
        instr = INST_MemReg(OP::SubMemReg(nodeIs64Bit, isWithBorrow), node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
    } else {
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, OP::SubRegReg(nodeIs64Bit, isWithBorrow),
            OP::SubRegMem(nodeIs64Bit, isWithBorrow), OP::MOVRegReg(nodeIs64Bit), computesCarry, 0);
...
TR::Register *OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, OP::SUB1RegReg, OP::SUB1RegMem, OP::MOV1RegReg,
            true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
        ...
        if (targetRegister && firstChild->getReferenceCount() > 1) {
            ...
            if (value == 1) {
                ...
                if (isMemOp)
                    instr = INST_MemImm(OP::SUB1MemImm1, node, tempMR, value, cg);
                else
                    instr = INST_RegImm(OP::SUB1RegImm1, node, targetRegister, value, cg);
            }
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d", node, value);
        }
    } else if (isMemOp) {
        instr = INST_MemReg(OP::SUB1MemReg, node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
    } else {
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, OP::SUB1RegReg, OP::SUB1RegMem, OP::MOV1RegReg);
        targetRegister = node->getRegister();
        countsAreDecremented = true;
    }
...
TR::Register *OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, OP::SUB2RegReg, OP::SUB2RegMem, OP::MOV4RegReg,
            true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
        ...
        if (firstChild->getReferenceCount() > 1) {
            ...
            if (value >= -128 && value <= 127) {
                if (value == 1) {
                    ...
                    if (isMemOp)
                        instr = INST_MemImm(OP::SUB2MemImms, node, tempMR, value, cg);
                    else
                        instr = INST_RegImm(OP::SUB4RegImms, node, targetRegister, value, cg);
                }
            } else {
                if (isMemOp)
                    instr = INST_MemImm(OP::SUB2MemImm2, node, tempMR, value, cg);
                else
                    instr = INST_RegImm(OP::SUB2RegImm2, node, targetRegister, value, cg);
            }
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d", node, value);
        }
    } else if (isMemOp) {
        instr = INST_MemReg(OP::SUB2MemReg, node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
    } else {
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, OP::SUB4RegReg, OP::SUB2RegMem,
            OP::MOV4RegReg); // Use MOV4 to avoid unnecessary size prefix
        return node->getRegister();
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDualCyclic() || needsHighMulOnly) {
        ...
        INST_RegReg(OP::MUL8AccReg, node, lmulTargetRegister, lumulhTargetRegister, multDependencies, cg);
...
TR::Register *OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (value = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg))) {
        if (value == 0) {
            ...
            INST_RegReg(OP::XOR4RegReg, node, targetRegister, targetRegister, cg);
        } else {
            ...
            {
                OP::Mnemonic opCode = OP::bad;

                if (firstChild->getReferenceCount() > 1 || firstChild->getRegister() != 0) {
                    if (value >= -128 && value <= 127) {
                        opCode = OP::IMulRegRegImms(node->getSize());
                    } else {
                        opCode = OP::IMulRegRegImm4(node->getSize());
                    }
                    targetRegister = cg->allocateRegister();
                    ...
                    if (firstChild->getOpCode().isMemoryReference()) {
                        if (value >= -128 && value <= 127) {
                            opCode = OP::IMulRegMemImms(node->getSize());
                        } else {
    ...
    if (!targetRegister && dataType == TR::Int8) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isMemoryReference()) {
            ...
            INST_RegReg(OP::IMUL1AccReg, node, targetRegister, tempRegister, multDependencies, cg);
        }
    }
    ...
    if (targetRegister) {
        ...
        TR_X86BinaryCommutativeAnalyser temp(cg);
        temp.genericAnalyser(node, OP::IMulRegReg(node->getSize()), OP::IMulRegMem(node->getSize()),
            OP::MovRegReg(node->getSize()));
        targetRegister = node->getRegister();
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR_ASSERT(node->getOpCodeValue() == TR::imulh, "OP::IMULAccReg only works for imulh");
    INST_RegReg(OP::IMULAccReg(nodeIs64Bit), node, eaxRegister, targetRegister, multDependencies, cg);
    ...
    if (isPowerOf2(dvalue)) {
        ...
        if (rootOpCode.isRem()) {
            ...
            if (isMinInt) {
                ...
                // is negative.
                //
                INST_RegReg(OP::TESTRegReg(nodeIs64Bit), node, tempRegister, tempRegister, cg);
                INST_Label(OP::JNS4, node, doneLabel, cg);
            } else if (nodeIs64Bit) {
                if (1 <= dvalue && dvalue <= 0x40000000) {
                    ...
                    //
                    INST_RegImm(OP::ROL8RegImm1, node, tempRegister, 1, cg);
                    INST_RegImm(OP::AND8RegImm4, node, tempRegister, (uint32_t)(2 * dvalue - 1), cg);
                    INST_RegImm(OP::ROR8RegImm1, node, tempRegister, 1, cg);
                } else if (dvalue == CONSTANT64(0x80000000)) {
                    // Keep only the low 31 bits plus the sign bit
                    //
                    INST_RegImm(OP::ROL8RegImm1, node, tempRegister, 1, cg);
                    INST_RegReg(OP::MOVZXReg8Reg4, node, tempRegister, tempRegister, cg);
                    INST_RegImm(OP::ROR8RegImm1, node, tempRegister, 1, cg);
                } else {
                    ...
                    //
                    INST_RegImm(OP::ROL8RegImm1, node, tempRegister, 32, cg);
                    INST_RegImm(OP::AND8RegImm4, node, tempRegister, SIGN32 | (uint32_t)((int64_t)(dvalue - 1) >> 32),
                        cg);
                    INST_RegImm(OP::ROR8RegImm1, node, tempRegister, 32, cg);
                }
                ...
                INST_RegImm(OP::AND4RegImm4, node, tempRegister, static_cast<int32_t>((uint32_t)SIGN32 + dvalue - 1),
                    cg);
                INST_Label(OP::JNS4, node, doneLabel, cg);
            }
            ...
            if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
                INST_Reg(OP::DECReg(nodeIs64Bit), node, tempRegister, cg);
                INST_RegImm(OP::ORRegImm4(nodeIs64Bit), node, tempRegister, (uint32_t)-dvalue, cg);
                INST_Reg(OP::INCReg(nodeIs64Bit), node, tempRegister, cg);
                INST_Label(OP::label, node, doneLabel, deps, cg);
            } else {
                TR_ASSERT(nodeIs64Bit, "assertion failure");
                INST_Reg(OP::DEC8Reg, node, tempRegister, cg);
                if (dvalue == CONSTANT64(0x100000000)) {
                    ...
                    INST_RegImm(OP::XOR4RegImms, node, tempRegister, (unsigned)-1, cg);
                    // Flip all 64 bits
                    INST_RegImm(OP::XOR8RegImms, node, tempRegister, (unsigned)-1, cg);
                    // Now the top 32 bits are set, and bottom 32 bits have their original values
                } else {
                    // Rotate so sign bit is at bit 31
                    INST_RegImm(OP::ROR8RegImm1, node, tempRegister, 33, cg);
                    // Set the appropriate bits, making sure the Imm4 has its sign bit clear to leave the top 33 bits
                    // alone
                    INST_RegImm(OP::OR8RegImm4, node, tempRegister, (~SIGN32) & (uint32_t)((int64_t)(-dvalue) >> 33),
                        cg);
                    // Un-rotate
                    INST_RegImm(OP::ROL8RegImm1, node, tempRegister, 33, cg);
                }
                INST_Reg(OP::INC8Reg, node, tempRegister, cg);
                INST_Label(OP::label, node, doneLabel, deps, cg);
            }
        ...
        if (!isMinInt) {
            ...
            if ((dividend->isNonNegative() == false) && (dvalue > 0)) {
                ...
                {
                    INST_RegReg(OP::SUBRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
                    ...
                    TR_ASSERT(mask >= 0,
                        ...
                        "of tempRegister");
                    INST_RegImm(OP::ANDRegImm4(nodeIs64Bit), node, edxRegister, (uint32_t)dvalue - 1, cg);
                    INST_RegReg(OP::ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
                } else {
                    // dvalue-1 is too big for an Imm4, so shift off the bits instead.
                    //
                    int32_t shiftAmount = leadingZeroes(dvalue) + 1;
                    INST_RegImm(OP::SHL8RegImm1, node, edxRegister, shiftAmount, cg);
                    INST_RegImm(OP::SHR8RegImm1, node, edxRegister, shiftAmount, cg);
                    INST_RegReg(OP::ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
                }
            }

            INST_RegImm(OP::SARRegImm1(nodeIs64Bit), node, tempRegister, trailingZeroes(dvalue), cdqDependencies, cg);

            if (isNegative) {
                INST_Reg(OP::NEGReg(nodeIs64Bit), node, tempRegister, cdqDependencies, cg);
            }
        } else {
            // MIN_INT or MIN_LONG divisor
            //
            TR::Register *quotientRegister = cg->allocateRegister();
            INST_RegReg(OP::XOR4RegReg, node, quotientRegister, quotientRegister, cg);

            if (nodeIs64Bit) {
                INST_RegImm64(OP::MOV8RegImm64, node, edxRegister, dvalue, cg);
                INST_RegReg(OP::CMP8RegReg, node, edxRegister, tempRegister, cg);
            } else {
                INST_RegImm(OP::CMP4RegImm4, node, tempRegister, (int32_t)dvalue, cg);
            }

            INST_Reg(OP::SETE1Reg, node, quotientRegister, cg);
        ...
        //
        if ((dvalue > 0) && (m < 0)) {
            INST_RegReg(OP::ADDRegReg(nodeIs64Bit), node, edxRegister, dividendRegister, cg);
        } else if ((dvalue < 0) && (m > 0)) {
            INST_RegReg(OP::SUBRegReg(nodeIs64Bit), node, edxRegister, dividendRegister, cg);
        }

        // Do the magic shift
        //
        INST_RegImm(OP::SARRegImm1(nodeIs64Bit), node, edxRegister, static_cast<int32_t>(s), cg);
        ...
        if (!(dividend->isNonNegative() && (dvalue > 0))) {
            ...
            INST_RegReg(OP::MOVRegReg(nodeIs64Bit), node, eaxRegister, edxRegister, cg);
            INST_RegImm(OP::SHRRegImm1(nodeIs64Bit), node, eaxRegister, nodeIs64Bit ? 63 : 31, cg);
            INST_RegReg(OP::ADDRegReg(nodeIs64Bit), node, edxRegister, eaxRegister, cg);
        ...
        if (rootOpCode.isRem()) {
            if (nodeIs64Bit && !IS_32BIT_SIGNED(dvalue)) {
                ...
                INST_RegImm64(OP::MOV8RegImm64, node, scratchReg, dvalue, cg);
                INST_RegReg(OP::IMULRegReg(nodeIs64Bit), node, edxRegister, scratchReg, cg);
                cg->stopUsingRegister(scratchReg);
            } else {
                OP::Mnemonic opCode;
                if (dvalue >= -128 && dvalue <= 127) {
                    opCode = OP::IMULRegRegImms(nodeIs64Bit);
                } else {
                    opCode = OP::IMULRegRegImm4(nodeIs64Bit);
                }
                INST_RegRegImm(opCode, node, edxRegister, edxRegister, static_cast<int32_t>(dvalue), cg);
            }

            INST_RegReg(OP::SUBRegReg(nodeIs64Bit), node, dividendRegister, edxRegister, multDependencies, cg);
            cg->stopUsingRegister(edxRegister);
...
TR::Register *OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
        ...
        if (needsExplicitOverflowCheck) {
            ...
            //
            INST_Label(OP::label, node, startLabel, cg);
            INST_RegImm(OP::CMPRegImms(nodeIs64Bit), node, eaxRegister, 1, cg);
            INST_Label(OP::JO4, node, overflowSnippetLabel, cg);
        ...
        if (!nodeIs64Bit && node->isUnsigned()) {
            ...
            if (divisorRegister) {
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = INST_RegMem(OP::IDIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        }
        ...
        //
        if (needsExplicitOverflowCheck) {
            INST_Label(OP::label, node, restartLabel, allDeps, cg);
...
TR::X86RegInstruction *OMR::X86::TreeEvaluator::generateRegisterShift(TR::Node *node, OP::Mnemonic immShiftOpCode,
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (shiftAmount == 0) {
            ...
            if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift()
                ...
                targetRegister = cg->allocateRegister();
                instr = INST_RegReg(OP::MOVRegReg(nodeIs64Bit), node, targetRegister, origTargetRegister, cg);
            } else {
                targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild,
...
TR::X86MemInstruction *OMR::X86::TreeEvaluator::generateMemoryShift(TR::Node *node, OP::Mnemonic immShiftOpCode,
    OP::Mnemonic regShiftOpCode, TR::CodeGenerator *cg)
{
...
TR::Register *OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(node, OP::SHLMemImm1(nodeIs64Bit), OP::SHLMemCL(nodeIs64Bit), cg);
        ...
        TR::Register *targetRegister = cg->allocateRegister();
        INST_RegMem(OP::LEARegMem(nodeIs64Bit), node, targetRegister, memRef, cg);
        node->setRegister(targetRegister);
        cg->decReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, OP::SHLRegImm1(nodeIs64Bit), OP::SHLRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register *OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (rotateAmount == 0) {
            ...
            targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
            INST_RegImm(OP::ROLRegImm1(nodeIs64Bit), node, targetRegister, static_cast<int32_t>(rotateAmount), cg);
        }
    } else {
        targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
        ...
        INST_RegReg(OP::ROLRegCL(nodeIs64Bit), node, targetRegister, rotateAmountReg, rotateDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(node, OP::SARMemImm1(nodeIs64Bit), OP::SARMemCL(nodeIs64Bit), cg);
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, OP::SARRegImm1(nodeIs64Bit), OP::SARRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register *OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::Instruction *shiftInstruction
            = TR::TreeEvaluator::generateMemoryShift(node, OP::SHRMemImm1(nodeIs64Bit), OP::SHRMemCL(nodeIs64Bit), cg);
        if (shiftInstruction)
            faultingInstruction = shiftInstruction;
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, OP::SHRRegImm1(nodeIs64Bit), OP::SHRRegCL(nodeIs64Bit), cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpCode == TR::bconst
        ...
        if (isMemOp) {
            if (value != 0) {
                instr = INST_MemImm(OP::SHL1MemImm1, node, tempMR, value, cg);
                if (debug("traceMemOp"))
            ...
            if (value == 0) {
                ...
                targetRegister = cg->allocateRegister();
                instr = INST_RegMem(OP::LEA4RegMem, node, targetRegister, tempMR, cg);
            } else {
                targetRegister = cg->intClobberEvaluate(firstChild);
                instr = INST_RegImm(OP::SHL1RegImm1, node, targetRegister, value, cg);
            }
        }
    } else {
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHL1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = INST_RegReg(OP::SHL1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        && performTransformation(comp, "O^O SSHLEvaluator: second child is not a 16-bit integer constant: %x\n",
            testOpcode)) {
        int32_t value = secondChild->getInt();
        if (isMemOp) {
            if (value != 0) {
                instr = INST_MemImm(OP::SHL2MemImm1, node, tempMR, value, cg);
                if (debug("traceMemOp"))
                    diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by const %d", node, value);
            }
        } else {
            if (value == 0) {
                ...
                targetRegister = cg->intClobberEvaluate(firstChild);
                instr = INST_RegImm(OP::SHL4RegImm1, node, targetRegister, value, cg);
            }
        }
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHL2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = INST_RegReg(OP::SHL4RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::bconst
        && performTransformation(comp,
            "O^O BSHREvaluator: second child is not an 8-bit signed Two's complement opcode %x\n", testOpcode)) {
        int32_t value = secondChild->getByte();
        if (value != 0) {
            if (isMemOp) {
                instr = INST_MemImm(OP::SAR1MemImm1, node, tempMR, value, cg);
                if (debug("traceMemOp"))
                    diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by const %d", node, value);
            } else
                instr = INST_RegImm(OP::SAR1RegImm1, node, targetRegister, value, cg);
        }
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SAR1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = INST_RegReg(OP::SAR1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        && performTransformation(comp,
            "O^O SSHREvaluator: second child is not a 16-bit signed two's complement number %x\n", testOpcode)) {
        int32_t value = secondChild->getInt();
        if (value != 0) {
            if (isMemOp) {
                instr = INST_MemImm(OP::SAR2MemImm1, node, tempMR, value, cg);
                if (debug("traceMemOp"))
                    diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by const %d", node, value);
            } else
                instr = INST_RegImm(OP::SAR2RegImm1, node, targetRegister, value, cg);
        }
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SAR2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = INST_RegReg(OP::SAR2RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (isMemOp) {
        ...
        INST_RegImm(OP::MOV1RegImm1, node, targetRegister, value, cg);
    } else {
        targetRegister = cg->intClobberEvaluate(firstChild);
    }
    ...
    if (testOpcode2 == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemImm(OP::SHR1MemImm1, node, tempMR, value, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by const %d", node, value);
        } else
            instr = INST_RegImm(OP::SHR1RegImm1, node, targetRegister, value, cg);
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHR1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = INST_RegReg(OP::SHR1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::ILOpCodes testOpcode = secondChild->getOpCodeValue();
    if (testOpcode == TR::iconst
        && performTransformation(comp, "O^O SUSHREvaluator: opcode is not a 16-bit signed two's complement %x\n",
            testOpcode)) {
        int32_t value = secondChild->getInt();
        if (isMemOp) {
            instr = INST_MemImm(OP::SHR2MemImm1, node, tempMR, value, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by const %d", node, value);
        } else
            instr = INST_RegImm(OP::SHR2RegImm1, node, targetRegister, value, cg);
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHR2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = INST_RegReg(OP::SHR2RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
OP::Mnemonic OMR::X86::TreeEvaluator::_logicalOpPackage[numLogicalOpPackages][numLogicalOpForms] = {
    // band
    { OP::AND1RegReg, OP::AND1RegMem, OP::MOV1RegReg, OP::AND1RegImm1,         OP::UD2, OP::AND1MemImm1,OP::AND1MemImm1,
     OP::AND1MemReg, OP::NOT1Mem                 },
    // cand/sand
    { OP::AND2RegReg, OP::AND2RegMem, OP::MOV4RegReg, OP::AND2RegImms, OP::AND2RegImm2, OP::AND2MemImms,
     OP::AND2MemImm2, OP::AND2MemReg, OP::NOT2Mem },
    // iand
    { OP::AND4RegReg, OP::AND4RegMem, OP::MOV4RegReg, OP::AND4RegImms, OP::AND4RegImm4, OP::AND4MemImms,
     OP::AND4MemImm4, OP::AND4MemReg, OP::NOT4Mem },
    // land
    { OP::AND8RegReg, OP::AND8RegMem, OP::MOV8RegReg, OP::AND8RegImms, OP::AND8RegImm4, OP::AND8MemImms,
     OP::AND8MemImm4, OP::AND8MemReg, OP::NOT8Mem },
    // bor
    {  OP::OR1RegReg,  OP::OR1RegMem, OP::MOV1RegReg,  OP::OR1RegImm1,         OP::UD2,  OP::OR1MemImm1,  OP::OR1MemImm1,
     OP::OR1MemReg, OP::NOT1Mem                  },
    // cor, sor
    {  OP::OR2RegReg,  OP::OR2RegMem, OP::MOV4RegReg,  OP::OR2RegImms,  OP::OR2RegImm2,  OP::OR2MemImms,  OP::OR2MemImm2,
     OP::OR2MemReg, OP::NOT2Mem                  },
    // ior
    {  OP::OR4RegReg,  OP::OR4RegMem, OP::MOV4RegReg,  OP::OR4RegImms,  OP::OR4RegImm4,  OP::OR4MemImms,  OP::OR4MemImm4,
     OP::OR4MemReg, OP::NOT4Mem                  },
    // lor
    {  OP::OR8RegReg,  OP::OR8RegMem, OP::MOV8RegReg,  OP::OR8RegImms,  OP::OR8RegImm4,  OP::OR8MemImms,  OP::OR8MemImm4,
     OP::OR8MemReg, OP::NOT8Mem                  },
    // bxor
    { OP::XOR1RegReg, OP::XOR1RegMem, OP::MOV1RegReg, OP::XOR1RegImm1,         OP::UD2, OP::XOR1MemImm1, OP::XOR1MemImm1,
     OP::XOR1MemReg, OP::NOT1Mem                 },
    // cxor, sxor
    { OP::XOR2RegReg, OP::XOR2RegMem, OP::MOV4RegReg, OP::XOR2RegImms, OP::XOR2RegImm2, OP::XOR2MemImms,
     OP::XOR2MemImm2, OP::XOR2MemReg, OP::NOT2Mem },
    // ixor
    { OP::XOR4RegReg, OP::XOR4RegMem, OP::MOV4RegReg, OP::XOR4RegImms, OP::XOR4RegImm4, OP::XOR4MemImms,
     OP::XOR4MemImm4, OP::XOR4MemReg, OP::NOT4Mem },
    // lxor
    { OP::XOR8RegReg, OP::XOR8RegMem, OP::MOV8RegReg, OP::XOR8RegImms, OP::XOR8RegImm4, OP::XOR8MemImms,
     OP::XOR8MemImm4, OP::XOR8MemReg, OP::NOT8Mem },
};

TR::Register *OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node *node, OP::Mnemonic package[], TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    TR::Register *targetRegister = NULL;
    TR::Node *firstChild = node->getFirstChild();
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue)
        ...
        if (!isMemOp && node->getOpCode().isAnd() && !firstChild->getRegister()
            ...
            if (firstChild->getFirstChild()->getReferenceCount() > 1 || firstChild->getReferenceCount() > 1) {
                ...
                INST_RegReg(OP::MOVRegReg(nodeIs64Bit), node, temp, targetRegister, cg);
                targetRegister = temp;
            }
        ...
        if (node->getOpCode().isXor() && constValue == -1) {
            if (isMemOp) {
                ...
                instr = INST_Reg(OP::NOTReg(nodeIs64Bit), node, targetRegister, cg);
            }
        } else {
            if (constValue >= -128 && constValue <= 127) {
                if (isMemOp)
