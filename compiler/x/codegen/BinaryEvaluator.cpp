    ...
    if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
        ...
        if (firstOp.isAdd() && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &dummyConstValue, cg)) {
            ...
            if (llChild->getRegister() == NULL && llChild->getReferenceCount() == 1
                ...
                //    const       (secondChild)
                //
                leaMR = MREF_BISdisp32(cg->evaluate(lrChild), cg->evaluate(llChild->getFirstChild()),
                ...
                //    const       (secondChild)
                //
                leaMR = MREF_BISdisp32(cg->evaluate(llChild), cg->evaluate(lrChild->getFirstChild()),
                    stride, displacement, cg);
...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (indexNode) {
        ...
        if (firstChild->getOpCode().isSub())
            val = -val;
        leaMR = MREF_BISdisp32(cg->evaluate(firstChild->getFirstChild()), cg->evaluate(secondChild), 0, val,
            cg);
        targetRegister = cg->allocateRegister();
        INST_RegMem(OP::LEARegMem(nodeIs64Bit), node, targetRegister, leaMR, cg);
        node->setRegister(targetRegister);
...
TR::Register *OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node) || (opCode == TR::luaddc) || (opCode == TR::iuaddc)) {
        ...
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node,
            OP::AddRegReg(nodeIs64Bit, isWithCarry), OP::AddRegMem(nodeIs64Bit, isWithCarry),
            true, /* produce eflags */
    ...
    if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
        ...
        if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL)
            ...
            if (targetRegister
                ...
                if (computesCarry) {
                    INST_RegReg(OP::MOVRegReg(nodeIs64Bit), node, targetRegister, tempReg,
                        cg);
                    INST_RegImm(OP::AddRegImm4(nodeIs64Bit, isWithCarry), node,
                ...
                if (constValue >= -128 && constValue <= 127) {
                    if (computesCarry) {
                        if (isMemOp)
                            instr = INST_MemImm(OP::AddMemImms(nodeIs64Bit, isWithCarry),
                                node, tempMR, static_cast<int32_t>(constValue), cg);
                        else
                            instr = INST_RegImm(OP::AddRegImms(nodeIs64Bit, isWithCarry),
                        ...
                        if (isMemOp)
                            ...
                            instr = INST_RegImm(OP::ADDRegImms(nodeIs64Bit), node,
                                targetRegister, 1, cg);
                    } else if (constValue == -1) {
                        if (isMemOp)
                            instr = INST_Mem(OP::DECMem(nodeIs64Bit), node, tempMR, cg);
                        else
                            instr = INST_RegImm(OP::SUBRegImms(nodeIs64Bit), node,
                                targetRegister, 1, cg);
                    } else {
                        if (isMemOp)
                    ...
                    if (isMemOp)
                        instr = INST_MemImm(OP::SUBMemImms(nodeIs64Bit), node, tempMR,
                            (uint32_t)-128, cg);
                    else
                    ...
                    if (isMemOp)
                        instr = INST_MemImm(OP::AddMemImm4(nodeIs64Bit, isWithCarry), node,
                            tempMR, static_cast<int32_t>(constValue), cg);
                    else
                        instr = INST_RegImm(OP::AddRegImm4(nodeIs64Bit, isWithCarry), node,
                            targetRegister, static_cast<int32_t>(constValue), cg);
                }
                if (debug("traceMemOp"))
            ...
            instr = INST_MemReg(OP::AddMemReg(nodeIs64Bit, isWithCarry), node, tempMR,
...
TR::Register *OMR::X86::TreeEvaluator::baddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, OP::ADD1RegReg,
            OP::ADD1RegMem, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
...
TR::Register *OMR::X86::TreeEvaluator::saddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, OP::ADD2RegReg,
            OP::ADD2RegMem, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
        ...
        if (targetRegister && firstChild->getReferenceCount() > 1) {
            ...
            if (value >= -128 && value <= 127) {
                ...
                if (isMemOp)
                    ...
                    instr = INST_RegImm(OP::SUB4RegImms, node, targetRegister, (uint32_t)-128,
                        cg);
            } else {
                if (isMemOp)
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
        ...
        if (targetRegister && !computesCarry && firstChild->getReferenceCount() > 1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                if (computesCarry) {
                    if (isMemOp)
                        instr = INST_MemImm(OP::SubMemImms(nodeIs64Bit, isWithBorrow), node,
                            tempMR, static_cast<int32_t>(constValue), cg);
                    else
                        instr = INST_RegImm(OP::SubRegImms(nodeIs64Bit, isWithBorrow), node,
                            targetRegister, static_cast<int32_t>(constValue), cg);
                } else if (constValue == 1) {
                    if (isMemOp)
                        instr = INST_Mem(OP::DECMem(nodeIs64Bit), node, tempMR, cg);
                ...
                if (isMemOp)
                    instr = INST_MemImm(OP::SubMemImm4(nodeIs64Bit, isWithBorrow), node,
                        tempMR, static_cast<int32_t>(constValue), cg);
                else
                    instr = INST_RegImm(OP::SubRegImm4(nodeIs64Bit, isWithBorrow), node,
                        targetRegister, static_cast<int32_t>(constValue), cg);
            }
        ...
        instr = INST_MemReg(OP::SubMemReg(nodeIs64Bit, isWithBorrow), node, tempMR,
            cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
        ...
        temp.integerSubtractAnalyser(node, OP::SubRegReg(nodeIs64Bit, isWithBorrow),
            OP::SubRegMem(nodeIs64Bit, isWithBorrow), OP::MOVRegReg(nodeIs64Bit), computesCarry,
            0);
...
TR::Register *OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, OP::SUB1RegReg, OP::SUB1RegMem,
            OP::MOV1RegReg, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
        ...
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, OP::SUB1RegReg, OP::SUB1RegMem,
            OP::MOV1RegReg);
        targetRegister = node->getRegister();
        countsAreDecremented = true;
    }
...
TR::Register *OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node)) {
        ...
        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, OP::SUB2RegReg, OP::SUB2RegMem,
            OP::MOV4RegReg, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDualCyclic() || needsHighMulOnly) {
        ...
        INST_RegReg(OP::MUL8AccReg, node, lmulTargetRegister, lumulhTargetRegister,
            multDependencies, cg);
...
TR::Register *OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
        ...
        if (value == 0) {
            ...
            if (targetRegister == 0 && IS_32BIT_SIGNED(value) && (dataType != TR::Int8)) // decomposition failed
            {
                ...
                if (firstChild->getReferenceCount() > 1 || firstChild->getRegister() != 0) {
                    ...
                    targetRegister = cg->allocateRegister();
                    INST_RegRegImm(opCode, node, targetRegister, cg->evaluate(firstChild),
                    ...
                    if (firstChild->getOpCode().isMemoryReference()) {
                        ...
                        targetRegister = cg->allocateRegister();
                        INST_RegMemImm(opCode, node, targetRegister, tempMR, static_cast<int32_t>(value),
                            cg);
                        tempMR->decNodeReferenceCounts(cg);
                    } else {
                        if (value >= -128 && value <= 127) {
                        ...
                        targetRegister = cg->evaluate(firstChild);
                        INST_RegRegImm(opCode, node, targetRegister, targetRegister,
                            static_cast<int32_t>(value), cg);
                    }
                }
            }
    ...
    if (!targetRegister && dataType == TR::Int8) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isMemoryReference()) {
            ...
            INST_RegReg(OP::IMUL1AccReg, node, targetRegister, tempRegister, multDependencies,
                cg);
        }
    }
    ...
    if (targetRegister) {
        ...
        TR_X86BinaryCommutativeAnalyser temp(cg);
        temp.genericAnalyser(node, OP::IMulRegReg(node->getSize()),
            OP::IMulRegMem(node->getSize()), OP::MovRegReg(node->getSize()));
        targetRegister = node->getRegister();
    }
...
TR::Register *OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_RegReg(OP::IMULAccReg(nodeIs64Bit), node, eaxRegister, targetRegister,
        multDependencies, cg);
    ...
    if (isPowerOf2(dvalue)) {
        ...
        if (rootOpCode.isRem()) {
            ...
            if (isMinInt) {
                ...
                // is negative.
                //
                INST_RegReg(OP::TESTRegReg(nodeIs64Bit), node, tempRegister, tempRegister,
                    cg);
                ...
                if (1 <= dvalue && dvalue <= 0x40000000) {
                    ...
                    INST_RegImm(OP::AND8RegImm4, node, tempRegister,
                ...
                INST_RegImm(OP::AND4RegImm4, node, tempRegister,
            ...
            if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
                ...
                INST_RegImm(OP::ORRegImm4(nodeIs64Bit), node, tempRegister, (uint32_t)-dvalue,
                    cg);
                ...
                if (dvalue == CONSTANT64(0x100000000)) {
                    ...
                    // alone
                    INST_RegImm(OP::OR8RegImm4, node, tempRegister,
        ...
        if (!isMinInt) {
            ...
            if ((dividend->isNonNegative() == false) && (dvalue > 0)) {
                ...
                {
                    INST_RegReg(OP::SUBRegReg(nodeIs64Bit), node, tempRegister, edxRegister,
                        cg);
                    ...
                    TR_ASSERT(mask >= 0,
                        ...
                        "of tempRegister");
                    INST_RegImm(OP::ANDRegImm4(nodeIs64Bit), node, edxRegister,
                        (uint32_t)dvalue - 1, cg);
                    INST_RegReg(OP::ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister,
                        cg);
                } else {
            ...
            INST_RegImm(OP::SARRegImm1(nodeIs64Bit), node, tempRegister,
        ...
        if (!nodeIs64Bit || IS_32BIT_SIGNED(m))
            INST_RegImm(OP::MOVRegImm4(nodeIs64Bit), node, eaxRegister,
                static_cast<int32_t>(m), cg);
        else
            INST_RegMem(OP::LEA8RegMem, node, eaxRegister, MREF_abs(m, cg),
                cg);
        INST_RegReg(OP::IMULAccReg(nodeIs64Bit), node, eaxRegister, dividendRegister,
            multDependencies, cg);
        ...
        // Do the magic shift
        //
        INST_RegImm(OP::SARRegImm1(nodeIs64Bit), node, edxRegister, static_cast<int32_t>(s),
            cg);
        ...
        if (!(dividend->isNonNegative() && (dvalue > 0))) {
            ...
            INST_RegImm(OP::SHRRegImm1(nodeIs64Bit), node, eaxRegister, nodeIs64Bit ? 63 : 31,
                cg);
        ...
        if (rootOpCode.isRem()) {
            ...
            INST_RegReg(OP::SUBRegReg(nodeIs64Bit), node, dividendRegister, edxRegister,
                multDependencies, cg);
            cg->stopUsingRegister(edxRegister);
...
TR::Register *OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
        ...
        if (!nodeIs64Bit && node->isUnsigned()) {
            ...
            if (divisorRegister) {
                divideInstr = INST_RegReg(OP::DIVAccReg(nodeIs64Bit), node, eaxRegister,
                    divisorRegister, eaxEdxDeps, cg);
            } else {
                TR::MemoryReference *tempMR = MREF_node(secondChild, cg);
                divideInstr = INST_RegMem(OP::DIVAccMem(nodeIs64Bit), node, eaxRegister,
            ...
            if (divisorRegister) {
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = INST_RegReg(OP::IDIVAccReg(nodeIs64Bit), node, eaxRegister,
                        divisorRegister, eaxEdxDeps, cg);
            } else {
                TR::MemoryReference *tempMR = MREF_node(secondChild, cg);
                if (node->getFirstChild()->isNonNegative() || node->getOpCode().isUnsigned())
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = INST_RegMem(OP::IDIVAccMem(nodeIs64Bit), node, eaxRegister,
                        tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        }
...
TR::X86RegInstruction *OMR::X86::TreeEvaluator::generateRegisterShift(TR::Node *node,
    OP::Mnemonic immShiftOpCode, OP::Mnemonic regShiftOpCode, TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    TR::Register *targetRegister = NULL;
    TR::Node *firstChild = node->getFirstChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (shiftAmount == 0) {
            ...
            if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift()
                ...
                targetRegister = cg->allocateRegister();
                instr = INST_RegReg(OP::MOVRegReg(nodeIs64Bit), node, targetRegister,
            ...
            instr = INST_RegImm(immShiftOpCode, node, targetRegister, static_cast<int32_t>(shiftAmount),
                cg);
        }
    } else {
...
TR::Register *OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(node, OP::SHLMemImm1(nodeIs64Bit),
            OP::SHLMemCL(nodeIs64Bit), cg);
        ...
        TR::TreeEvaluator::generateRegisterShift(node, OP::SHLRegImm1(nodeIs64Bit),
            OP::SHLRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register *OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (rotateAmount == 0) {
            ...
            targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
            INST_RegImm(OP::ROLRegImm1(nodeIs64Bit), node, targetRegister,
        ...
        INST_RegReg(OP::ROLRegCL(nodeIs64Bit), node, targetRegister, rotateAmountReg,
...
TR::Register *OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(node, OP::SARMemImm1(nodeIs64Bit),
            OP::SARMemCL(nodeIs64Bit), cg);
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, OP::SARRegImm1(nodeIs64Bit),
            OP::SARRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register *OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::Instruction *shiftInstruction = TR::TreeEvaluator::generateMemoryShift(node,
            OP::SHRMemImm1(nodeIs64Bit), OP::SHRMemCL(nodeIs64Bit), cg);
        if (shiftInstruction)
            faultingInstruction = shiftInstruction;
    } else {
        TR::TreeEvaluator::generateRegisterShift(node, OP::SHRRegImm1(nodeIs64Bit),
            OP::SHRRegCL(nodeIs64Bit), cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpCode == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHL1MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = INST_RegReg(OP::SHL1RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHL2MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = INST_RegReg(OP::SHL4RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SAR1MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = INST_RegReg(OP::SAR1RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SAR2MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = INST_RegReg(OP::SAR2RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode2 == TR::bconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHR1MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = INST_RegReg(OP::SHR1RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (testOpcode == TR::iconst
        ...
        if (isMemOp) {
            instr = INST_MemReg(OP::SHR2MemCL, node, tempMR, shiftAmountReg,
                shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = INST_RegReg(OP::SHR2RegCL, node, targetRegister, shiftAmountReg,
                shiftDependencies, cg);
    }
...
OP::Mnemonic OMR::X86::TreeEvaluator::_logicalOpPackage[numLogicalOpPackages][numLogicalOpForms] = {
    // band
    { OP::AND1RegReg, OP::AND1RegMem, OP::MOV1RegReg, OP::AND1RegImm1,
     OP::UD2, OP::AND1MemImm1, OP::AND1MemImm1,OP::AND1MemReg,
     OP::NOT1Mem                            },
    // cand/sand
    { OP::AND2RegReg, OP::AND2RegMem, OP::MOV4RegReg, OP::AND2RegImms,
     OP::AND2RegImm2, OP::AND2MemImms, OP::AND2MemImm2,
     OP::AND2MemReg, OP::NOT2Mem },
    // iand
    { OP::AND4RegReg, OP::AND4RegMem, OP::MOV4RegReg, OP::AND4RegImms,
     OP::AND4RegImm4, OP::AND4MemImms, OP::AND4MemImm4,
     OP::AND4MemReg, OP::NOT4Mem },
    // land
    { OP::AND8RegReg, OP::AND8RegMem, OP::MOV8RegReg, OP::AND8RegImms,
     OP::AND8RegImm4, OP::AND8MemImms, OP::AND8MemImm4,
     OP::AND8MemReg, OP::NOT8Mem },
    // bor
    {  OP::OR1RegReg,  OP::OR1RegMem, OP::MOV1RegReg,  OP::OR1RegImm1,
     OP::UD2,  OP::OR1MemImm1,  OP::OR1MemImm1,  OP::OR1MemReg,
     OP::NOT1Mem                            },
    // cor, sor
    {  OP::OR2RegReg,  OP::OR2RegMem, OP::MOV4RegReg,  OP::OR2RegImms,
     OP::OR2RegImm2,  OP::OR2MemImms,  OP::OR2MemImm2,  OP::OR2MemReg,
     OP::NOT2Mem                            },
    // ior
    {  OP::OR4RegReg,  OP::OR4RegMem, OP::MOV4RegReg,  OP::OR4RegImms,
     OP::OR4RegImm4,  OP::OR4MemImms,  OP::OR4MemImm4,  OP::OR4MemReg,
     OP::NOT4Mem                            },
    // lor
    {  OP::OR8RegReg,  OP::OR8RegMem, OP::MOV8RegReg,  OP::OR8RegImms,
     OP::OR8RegImm4,  OP::OR8MemImms,  OP::OR8MemImm4,  OP::OR8MemReg,
     OP::NOT8Mem                            },
    // bxor
    { OP::XOR1RegReg, OP::XOR1RegMem, OP::MOV1RegReg, OP::XOR1RegImm1,
     OP::UD2, OP::XOR1MemImm1, OP::XOR1MemImm1, OP::XOR1MemReg,
     OP::NOT1Mem                            },
    // cxor, sxor
    { OP::XOR2RegReg, OP::XOR2RegMem, OP::MOV4RegReg, OP::XOR2RegImms,
     OP::XOR2RegImm2, OP::XOR2MemImms, OP::XOR2MemImm2,
     OP::XOR2MemReg, OP::NOT2Mem },
    // ixor
    { OP::XOR4RegReg, OP::XOR4RegMem, OP::MOV4RegReg, OP::XOR4RegImms,
     OP::XOR4RegImm4, OP::XOR4MemImms, OP::XOR4MemImm4,
     OP::XOR4MemReg, OP::NOT4Mem },
    // lxor
    { OP::XOR8RegReg, OP::XOR8RegMem, OP::MOV8RegReg, OP::XOR8RegImms,
     OP::XOR8RegImm4, OP::XOR8MemImms, OP::XOR8MemImm4,
     OP::XOR8MemReg, OP::NOT8Mem },
};

TR::Register *OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node *node, OP::Mnemonic package[],
    TR::CodeGenerator *cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    TR::Register *targetRegister = NULL;
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue)
        ...
        if (node->getOpCode().isXor() && constValue == -1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                if (isMemOp)
                    ...
                    instr = INST_RegImm(package[TR::TreeEvaluator::logicalByteImmOp], node,
                        targetRegister, static_cast<int32_t>(constValue), cg);
            } else {
                if (isMemOp)
        ...
        instr = INST_MemReg(package[TR::TreeEvaluator::logicalMemRegOp], node, tempMR,
            cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
