    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; i--) {
        ...
        if (registers.find(globalReg)) {
            if (globalReg->getKind() == TR_GPR) {
                ...
                OP::Mnemonic op = cg->comp()->target().cpu.supportsAVX() ? OP::VMOVDQUYmmYmm
                                                                                     : OP::MOVDQURegReg;
                op = cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F) ? OP::VMOVDQUZmmZmm
                                                                                       : op;
            ...
            if (cg->getUseNonLinearRegisterAssigner()) {
                if (virtReg->getAssignedRegister()) {
                    ...
                    if (assignedReg->getKind() == TR_FPR) {
                        op = (assignedReg->isSinglePrecision()) ? OP::MOVSSRegMem
                        ...
                        op = cg->comp()->target().cpu.supportsAVX() ? OP::VMOVDQUYmmMem
                                                                    : OP::MOVDQURegMem;
...
TR::RegisterDependencyConditions *REGDEPS(uint32_t numPreConds, uint32_t numPostConds,
    TR::CodeGenerator *cg)
{
    return new (cg->trHeapMemory()) TR::RegisterDependencyConditions(numPreConds, numPostConds, cg->trMemory());
}
