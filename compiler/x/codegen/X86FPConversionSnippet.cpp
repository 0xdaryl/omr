...
uint8_t *TR::X86FPConversionSnippet::emitCallToConversionHelper(uint8_t *buffer)
{
    ...
    *buffer++ = 0xe8; // CallImm4
    ...
    if (cg()->directCallRequiresTrampoline(helperAddress, callInstructionAddress)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(getHelperSymRef()->getReferenceNumber(),
            (void *)buffer);
    ...
    *(int32_t *)buffer = (int32_t)(helperAddress - nextInstructionAddress);
    cg()->addProjectSpecializedRelocation(buffer, (uint8_t *)getHelperSymRef(), NULL, TR_HelperAddress, __FILE__,
        __LINE__, getNode());
    buffer += 4;
    ...
    *buffer++ = 0xec;
    ...
    if (_convertInstruction->getIA32RegMemInstruction()) {
        ...
        *buffer++ = 0x24;
        ...
        TR_ASSERT(instr != NULL,
            "f2i conversion instruction must be either OP::L4RegMem or CVTTSS2SIRegReg\n");

        TR::RealRegister *sourceRegister = toRealRegister(instr->getSourceRegister());

        // MOVSS/OP::MOVSD [esp], source
        //
        if (opcode == TR::f2i)
            *buffer++ = 0xf3;
        else
