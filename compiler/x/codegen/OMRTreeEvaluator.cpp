    ...
    int32_t value, TR::CodeGenerator *cg)
{
    ...
    OP::Mnemonic cmpOp
        = (value >= -128 && value <= 127) ? OP::CMPMemImms(is64Bit) : OP::CMPMemImm4(is64Bit);
    ...
    TR::CodeGenerator *cg)
{
    ...
    bool is64Bit
        = cg->comp()->target().is64Bit() ? TR::TreeEvaluator::getNodeIs64Bit(node->getFirstChild(), cg) : false;
    OP::Mnemonic cmpOp
        = (value >= -128 && value <= 127) ? OP::CMPRegImms(is64Bit) : OP::CMPRegImm4(is64Bit);
    ...
    TR_RematerializableTypes type, TR::CodeGenerator *cg, TR::Instruction *currentInstruction)
{
    ...
    static const OP::Mnemonic ops[TR_NumRematerializableTypes + 1][3] =
        //    load 0      load -1     load c
        {
            ...
            { OP::XOR4RegReg, OP::OR4RegImms,
             OP::MOV4RegImm4                                                         }, // 32-bit address constant
    ...
    if (currentInstruction) {
        ...
        if (is64Bit) {
            if (cg->constantAddressesCanChangeSize(node) && node && node->getOpCodeValue() == TR::aconst
                && (node->isClassPointerConstant() || node->isMethodPointerConstant())) {
                movInstruction = INST_RegImm64(currentInstruction, OP::MOV8RegImm64, target,
                    value, cg, reloKind);
            } else if (IS_32BIT_UNSIGNED(value)) {
                // zero-extended 4-byte MOV
                movInstruction = INST_RegImm(currentInstruction, OP::MOV4RegImm4, target,
                    static_cast<int32_t>(value), cg, reloKind);
            } else if (IS_32BIT_SIGNED(value)) // TODO:AMD64: Is there some way we could get RIP too?
            {
                movInstruction = INST_RegImm(currentInstruction, OP::MOV8RegImm4, target,
                    static_cast<int32_t>(value), cg, reloKind);
            } else {
                movInstruction = INST_RegImm64(currentInstruction, OP::MOV8RegImm64, target,
                    value, cg, reloKind);
        ...
        if (value == 0 && canClobberEFLAGS) {
            ...
            if (is64Bit) {
                if (cg->constantAddressesCanChangeSize(node) && node && node->getOpCodeValue() == TR::aconst
                    && (node->isClassPointerConstant() || node->isMethodPointerConstant())) {
                    movInstruction
                        = INST_RegImm64(OP::MOV8RegImm64, node, target, value, cg, reloKind);
                } else if (IS_32BIT_UNSIGNED(value)) {
                    // zero-extended 4-byte MOV
                    movInstruction = INST_RegImm(OP::MOV4RegImm4, node, target,
                        static_cast<int32_t>(value), cg, reloKind);
                } else if (IS_32BIT_SIGNED(value)) // TODO:AMD64: Is there some way we could get RIP too?
                {
                    movInstruction = INST_RegImm(OP::MOV8RegImm4, node, target,
                        static_cast<int32_t>(value), cg, reloKind);
                } else {
                    movInstruction
                        = INST_RegImm64(OP::MOV8RegImm64, node, target, value, cg, reloKind);
...
TR::Register *OMR::X86::TreeEvaluator::aloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!node->getSymbolReference()->isUnresolved()
        ...
        if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
            INST_Mem(OP::PREFETCHT0, node, generateX86MemoryReference(reg, 0, cg), cg);
        }
    }
...
TR::Register *OMR::X86::TreeEvaluator::SSE2ArraycmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_RegMem(OP::MOVUPSRegMem, node, xmm2Reg, generateX86MemoryReference(s2Reg, 0, cg),
        cg);
    INST_RegMem(OP::MOVUPSRegMem, node, xmm1Reg,
    ...
    INST_RegMem(OP::L1RegMem, node, s2ByteVer1Reg, generateX86MemoryReference(s2Reg, 0, cg),
        cg);
    INST_MemReg(OP::CMP1MemReg, node, generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
        s2ByteVer1Reg, cg);
    ...
    INST_RegMem(OP::L1RegMem, node, s2ByteVer2Reg,
...
TR::Register *OMR::X86::TreeEvaluator::SSE2ArraycmpLenEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_RegMem(OP::MOVUPSRegMem, node, xmm2Reg,
    ...
    INST_Label(OP::label, node, byteLoop, cg);
    INST_RegMem(OP::L1RegMem, node, s2ByteReg,
        generateX86MemoryReference(s2Reg, resultReg, 0, cg), cg);
    INST_MemReg(OP::CMP1MemReg, node, generateX86MemoryReference(s1Reg, resultReg, 0, cg),
        s2ByteReg, cg);
        ...
        switch (node->getOverflowCheckOperation()) {
            ...
            case TR::bsub:
                subAnalyser.integerSubtractAnalyserWithExplicitOperands(node, operand1, operand2, op,
                    OP::UD2, OP::MOV1RegReg, needsEflags);
                break;
            case TR::ssub:
            case TR::isub:
                subAnalyser.integerSubtractAnalyserWithExplicitOperands(node, operand1, operand2, op,
                    OP::UD2, OP::MOV4RegReg, needsEflags);
                break;
            case TR::lsub:
                cg->comp()->target().is32Bit()
                    ? subAnalyser.longSubtractAnalyserWithExplicitOperands(node, operand1, operand2)
                    : subAnalyser.integerSubtractAnalyserWithExplicitOperands(node, operand1, operand2, op,
                          OP::UD2, OP::MOV8RegReg, needsEflags);
                break;
            // mul group
            case TR::imul:
    ...
    TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    ...
    INST_RegMem(OP::MOVQRegMem, node, XMM, generateX86MemoryReference(srcReg, 0, cg), cg);
    INST_MemReg(OP::MOVQMemReg, node, generateX86MemoryReference(dstReg, srcReg, 0, cg), XMM,
        cg);
    INST_RegMem(OP::LEARegMem(), node, srcReg,
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (node->isForwardArrayCopy()) {
        ...
        INST_RegMem(OP::L2RegMem, node, sizeReg, generateX86MemoryReference(srcReg, 0, cg),
            cg);
        INST_MemReg(OP::S2MemReg, node, generateX86MemoryReference(dstReg, 0, cg), sizeReg,
            cg);
    } else // decide direction during runtime
    {
        ...
        INST_RegMem(OP::LEARegMem(), node, dstReg,
    ...
    TR::LabelSymbol *repMovsLabel, TR::LabelSymbol *mainEndLabel)
{
    ...
    // 2 Bytes
    INST_RegMem(OP::L2RegMem, node, tmpReg1, generateX86MemoryReference(srcReg, 0, cg), cg);
    INST_MemReg(OP::S2MemReg, node, generateX86MemoryReference(dstReg, 0, cg), tmpReg1, cg);
    INST_Label(OP::JMP4, node, mainEndLabel, cg);
    ...
    switch (elementSize) {
        ...
        case 2: {
            if (repmovs == OP::REPMOVSD) {
                ...
                INST(OP::REPMOVSD, node, cg);
                INST_Label(OP::JAE1, node, mainEndLabel, cg);
                INST_RegMem(OP::L2RegMem, node, sizeReg,
                    generateX86MemoryReference(srcReg, 0, cg), cg);
                INST_MemReg(OP::S2MemReg, node, generateX86MemoryReference(dstReg, 0, cg),
                    sizeReg, cg);
...
TR::Register *OMR::X86::TreeEvaluator::arraytranslateEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *dependencies
        = REGDEPS((uint8_t)0, noOfDependencies, cg);
    ...
    TR::CodeGenerator *cg)
{
    switch (size) {
        ...
        case 8:
            INST_RegReg(OP::MOVQRegReg8, node, XMMReg, sourceReg, cg);
            INST_RegRegImm(OP::PSHUFDRegRegImm1, node, XMMReg, XMMReg,
                size == 4 ? 0x00 : 0x44, cg);
            break;
        default:
    ...
    TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    ...
    INST_Label(OP::JRCXZ1, node, endLabel, cg);
    INST_Label(OP::label, node, loopLabel, cg);
    INST_MemReg(OP::MOVQMemReg, node,
    ...
    TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *stosDependencies
        = REGDEPS((uint8_t)0, (uint8_t)3, cg);
    ...
    TR::Register *sizeReg, TR::CodeGenerator *cg, TR::LabelSymbol *doneLabel = NULL)
{
    ...
    INST_MemReg(OP::S1MemReg, node, generateX86MemoryReference(addressReg, 0, cg), valueReg,
        cg);
    INST_MemReg(OP::S1MemReg, node,
        generateX86MemoryReference(addressReg, sizeReg, 0, -1, cg), valueReg, cg);
    INST_RegImm(OP::CMPRegImm4(), node, sizeReg, 2, cg);
    if (localDoneLabel)
        doneLabel = generateLabelSymbol(cg);
    INST_Label(OP::JBE4, node, doneLabel, cg);
    INST_MemReg(OP::S1MemReg, node, generateX86MemoryReference(addressReg, 1, cg), valueReg,
        cg);
    if (localDoneLabel)
...
{
    INST_RegMem(OP::LEARegMem(), node, lastWordAddressReg,
        generateX86MemoryReference(addressReg, sizeReg, 0, -4, cg), cg);
    INST_RegImm(OP::ANDRegImms(), node, sizeReg, 8, cg);
    INST_MemReg(OP::S4MemReg, node, generateX86MemoryReference(addressReg, 0, cg), valueReg,
        cg);
    INST_RegImm(OP::SHRRegImm1(), node, sizeReg, 1, cg);
    INST_MemReg(OP::S4MemReg, node, generateX86MemoryReference(lastWordAddressReg, 0, cg),
        valueReg, cg);
    INST_MemReg(OP::S4MemReg, node, generateX86MemoryReference(addressReg, sizeReg, 0, cg),
        valueReg, cg);
    INST_Reg(OP::NEGReg(), node, sizeReg, cg);
    INST_MemReg(OP::S4MemReg, node,
...
{
    INST_MemReg(OP::MOVUPSMemReg, node, generateX86MemoryReference(addressReg, 0, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVUPSMemReg, node,
    ...
    TR::Register *xmmValueReg, TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    INST_MemReg(OP::MOVUPSMemReg, node, generateX86MemoryReference(addressReg, 0, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVUPSMemReg, node, generateX86MemoryReference(addressReg, 16, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVUPSMemReg, node,
    ...
    TR::Register *sizeReg, TR::Register *scratch1Reg, TR::Register *scratch2Reg, TR::CodeGenerator *cg)
{
    ...
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(addressReg, 0, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(addressReg, 16, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(addressReg, 32, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(addressReg, 48, cg),
        xmmValueReg, cg);
    ...
    INST_RegMem(OP::LEARegMem(), node, scratch1Reg,
    ...
    INST_RegMem(OP::LEARegMem(), node, scratch2Reg,
        generateX86MemoryReference(addressReg, sizeReg, 0, -16, cg), cg);
    // Do the stores
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(scratch1Reg, 0, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(scratch1Reg, 16, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVAPSMemReg, node, generateX86MemoryReference(scratch1Reg, 32, cg),
        xmmValueReg, cg);
    INST_MemReg(OP::MOVUPSMemReg, node, generateX86MemoryReference(scratch2Reg, 0, cg),
        xmmValueReg, cg);
}
...
TR::Register *OMR::X86::TreeEvaluator::arraysetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (isShortConstantArrayWithZero) {
        ...
        if (valueNode->getOpCode().isFloat() || valueNode->getOpCode().isDouble()) {
            ...
            INST_RegReg(elementSize == 8 ? OP::MOVQReg8Reg : OP::MOVDReg4Reg,
    ...
    TR::CodeGenerator *cg)
{
    ...
    INST_Mem(OP::LCMPXCHG8BMem, node, generateX86MemoryReference(address, 0, cg), deps, cg);
    if (!returnValue) {
...
static TR::Register *inlineAtomicCompareAndMemoryUpdate(TR::Node *node, bool returnValue, TR::CodeGenerator *cg)
{
    ...
    INST_MemReg(OP::LCMPXCHGMemReg(isNode64Bit), node,
...
TR::Register *OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (SymRef && SymRef->getSymbol()->castToMethodSymbol()->isInlinedByCG()) {
        ...
        if (comp->getSymRefTab()->isNonHelper(SymRef, TR::SymbolReferenceTable::atomicAddSymbol)) {
            ...
            op = node->getChild(1)->getDataType().isInt32() ? OP::LXADD4MemReg
                                                            : OP::LXADD8MemReg;
...
TR::Register *OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (comp->getOption(TR_BreakBBStart)) {
        TR::Machine *machine = cg->machine();
        INST_RegImm(OP::TEST4RegImm4, node, machine->getRealRegister(TR::RealRegister::esp),
            block->getNumber(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
        ...
        if (cg->enableRegisterAssociations()
            && cg->getAppendInstruction()->getOpCodeValue() != OP::assocreg) {
        ...
        if (node->getNumChildren() > 0)
            labelInst = INST_Label(OP::label, node, generateLabelSymbol(cg),
...
TR::Register *OMR::X86::TreeEvaluator::icmpsetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    INST_MemReg(cg->comp()->target().isSMP() ? OP::LCMPXCHGMemReg(nodeIs64Bit)
                                                           : OP::CMPXCHGMemReg(nodeIs64Bit),
...
TR::Register *OMR::X86::TreeEvaluator::vectorBinaryArithmeticEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR_ASSERT_FATAL(nativeOpcode.getMnemonic() != OP::bad,
...
TR::Register *OMR::X86::TreeEvaluator::bitpermuteEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (length->getOpCode().isLoadConst()) {
        ...
        for (uintptr_t x = 0; x < arrayLen; ++x) {
            ...
            if (x > 0)
                INST_RegImm(OP::SHLRegImm1(nodeIs64Bit), node, tmpReg,
...
{
    return TR::TreeEvaluator::compressExpandBitsEvaluator(node, cg, OP::PEXT4RegRegReg,
        OP::PEXT4RegRegMem);
}
...
{
    return TR::TreeEvaluator::compressExpandBitsEvaluator(node, cg, OP::PDEP4RegRegReg,
        OP::PDEP4RegRegMem);
}
        ...
        switch (et) {
            ...
            case TR::Int16: {
                ...
                if (packEncoding != OMR::X86::Legacy && xorEncoding != OMR::X86::Legacy
                    ...
                    INST_RegRegImm(OP::VEXTRACTF128RegRegImm1, node, tmpReg, maskReg, 1, cg,
                        OMR::X86::VEX_L256);
...
TR::Register *OMR::X86::TreeEvaluator::SIMDreductionEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    switch (vl) {
        case TR::VectorLength512:
            ...
            INST_RegRegImm(OP::VEXTRACTF64X4YmmZmmImm1, node, resultVRF, workingReg, 0xFF,
                cg);
            rSrcReg
        ...
        case TR::VectorLength128: {
            ...
            if (dt != TR::Int64 && dt != TR::Double) {
                ...
                if (dt != TR::Int32 && dt != TR::Float) {
                    ...
                    INST_RegRegImm(OP::PSHUFLWRegRegImm1, node, workingReg, resultVRF, 0x1,
                        cg);
...
TR::Register *OMR::X86::TreeEvaluator::vrolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cpu->supportsFeature(OMR_FEATURE_X86_AVX512F) && et != TR::Int16) {
        ...
        OP::Mnemonic gpr2XMMOpcode
            = et.isDouble() || et.isInt64() ? OP::MOVQRegReg8 : OP::MOVDRegReg4;
