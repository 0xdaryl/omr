...
void TR_Debug::printHeader(TR::Logger *log) { log->printf("\n=======>%s\n", signature(_comp->getMethodSymbol())); }

void TR_Debug::printMethodHotness(TR::Logger *log)
{
...
void TR_Debug::printInstrDumpHeader(TR::Logger *log, const char *title)
{
...
//
uint8_t *TR_Debug::printPrefix(TR::Logger *log, TR::Instruction *instr, uint8_t *cursor, uint8_t size)
{
    if (cursor != NULL) {
...
void TR_Debug::printSnippetLabel(TR::Logger *log, TR::LabelSymbol *label, uint8_t *cursor, const char *comment1,
    const char *comment2)
{
    int addressFieldWidth = TR::Compiler->debug.hexAddressFieldWidthInChars();
    int codeByteColumnWidth = TR::Compiler->debug.codeByteColumnWidth();
...
void TR_Debug::print(TR::Logger *log, TR_BitVector *bv)
{
    log->printc('{');
    bool firstOne = true;
...
void TR_Debug::print(TR::Logger *log, TR_SingleBitContainer *sbc) { log->prints(sbc->isEmpty() ? "{}" : "{0}"); }

void TR_Debug::print(TR::Logger *log, TR::BitVector *bv)
{
    log->printc('{');
    bool firstOne = true;
    int32_t num = 0;
...
void TR_Debug::print(TR::Logger *log, TR::SparseBitVector *sparse)
{
    log->printc('{');
    bool firstOne = true;
    int32_t num = 0;
...
int32_t TR_Debug::nodePrintAllFlags(TR::Logger *log, TR::Node *node)
{
    int32_t len = 0;
...
// Prints the SymRef offset if non-zero
void TR_Debug::printSymRefOffset(TR::Logger *log, TR::SymbolReference *symRef, bool omitSpacer)
{
    ...
    if (symRef->getOffset() + displacement) {
        log->printf("%s%+d", omitSpacer ? "" : " ", displacement + symRef->getOffset());
    }
}
            ...
            case TR::Symbol::IsMethod: {
                ...
                switch (methodSym->getMethodKind()) {
                    case TR::MethodSymbol::Virtual:
                        log->prints(" virtual Method");
                        break;
                    case TR::MethodSymbol::Interface:
                        log->prints(" interface Method");
                        break;
                    case TR::MethodSymbol::Static:
                        log->prints(" static Method");
                        break;
                    case TR::MethodSymbol::Special:
                        log->prints(" special Method");
                        break;
                    case TR::MethodSymbol::Helper:
                        log->prints(" helper Method");
                        break;
                    case TR::MethodSymbol::ComputedStatic:
                        log->prints(" computed-static Method");
                        break;
                    case TR::MethodSymbol::ComputedVirtual:
                        log->prints(" computed-virtual Method");
                        break;
                    default:
                        log->prints(" UNKNOWN Method");
                        break;
                }
            } break;
...
void TR_Debug::printSymRefName(TR::Logger *log, TR::SymbolReference *symRef)
{
    TR::Symbol *sym = symRef->getSymbol();
...
void TR_Debug::printSymRefOtherInfo(TR::Logger *log, TR::SymbolReference *symRef)
{
    TR::Symbol *sym = symRef->getSymbol();
...
void TR_Debug::printSymRefLabelSymbol(TR::Logger *log, TR::SymbolReference *symRef)
{
    TR::Symbol *sym = symRef->getSymbol();

    if (sym && sym->getKind() == TR::Symbol::IsLabel) {
        log->prints(getName(sym->castToLabelSymbol()));
    }
}

void TR_Debug::print(TR::Logger *log, TR::SymbolReference *symRef, bool hideHelperMethodInfo, bool verbose)
{
    TR::Symbol *sym = symRef->getSymbol();
    ...
    if (verbose) {
        ...
        if (hideHelperMethodInfo) {
            log->printc(' ');
            printSymRefLabelSymbol(log, symRef);
            log->prints(" [");
            printSymRefOffset(log, symRef, true /* omitSpacer */);
            log->printc(']');
        ...
        // Print base TR::Symbol name
        log->printf(" [");
        log->prints(getName(sym));
        log->printc(']');

        if (sym) {
            log->printf(" (%s", TR::DataType::getName(sym->getDataType()));
...
void TR_Debug::print(TR::Logger *log, TR::LabelSymbol *labelSymbol)
{
...
void TR_Debug::print(TR::Logger *log, TR::SymbolReferenceTable *symRefTab)
{
...
void TR_Debug::printAliasInfo(TR::Logger *log, TR::SymbolReferenceTable *symRefTab)
{
...
void TR_Debug::printAliasInfo(TR::Logger *log, TR::SymbolReference *symRef)
{
...
void TR_Debug::printBlockInfo(TR::Logger *log, TR::Node *node)
{
    if (node) {
...
void TR_Debug::dumpLiveRegisters(TR::Logger *log)
{
    int i = 0;
...
void TR_Debug::setupToDumpTreesAndInstructions(TR::Logger *log, const char *title)
{
    log->printf("\n%s:\n", title);
...
void TR_Debug::dumpSingleTreeWithInstrs(TR::Logger *log, TR::TreeTop *treeTop, TR::Instruction *instr, bool dumpTrees,
...
void TR_Debug::dumpMethodInstrs(TR::Logger *log, const char *title, bool dumpTrees, bool header)
{
...
void TR_Debug::dumpMixedModeDisassembly(TR::Logger *log)
{
...
void TR_Debug::dumpInstructionComments(TR::Logger *log, TR::Instruction *instr, bool needsStartComment)
{
    CS2::HashIndex hashIndex;
...
void TR_Debug::print(TR::Logger *log, TR::Instruction *inst) { print(log, inst, NULL); }
#endif

void TR_Debug::print(TR::Logger *log, TR::Instruction *inst, const char *title)
{
...
void TR_Debug::print(TR::Logger *log, TR::GCRegisterMap *map)
{
#if defined(TR_TARGET_X86)
    if (_comp->target().cpu.isX86()) {
...
void TR_Debug::print(TR::Logger *log, TR::list<TR::Snippet *> &snippetList)
{
...
void TR_Debug::print(TR::Logger *log, TR::Snippet *snippet)
{
#if defined(TR_TARGET_X86)
    if (_comp->target().cpu.isX86()) {
        printx(log, snippet);
...
void TR_Debug::printRegisterMask(TR::Logger *log, TR_RegisterMask mask, TR_RegisterKinds rk)
{
...
void TR_Debug::print(TR::Logger *log, TR::Register *reg, TR_RegisterSizes size)
{
#if defined(TR_TARGET_S390)
...
void TR_Debug::printFullRegInfo(TR::Logger *log, TR::Register *reg)
{
...
void TR_Debug::printGPRegisterStatus(TR::Logger *log, OMR::MachineConnector *machine)
{
#if defined(TR_TARGET_S390)
    if (_comp->target().cpu.isZ()) {
...
void TR_Debug::printFPRegisterStatus(TR::Logger *log, OMR::MachineConnector *machine)
{
#if defined(TR_TARGET_S390)
    if (_comp->target().cpu.isZ()) {
...
void TR_Debug::print(TR::Logger *log, TR::RegisterMappedSymbol *localCursor, bool isSpillTemp)
{
...
void TR_Debug::print(TR::Logger *log, uint8_t *instrStart, uint8_t *instrEnd) {}

void TR_Debug::print(TR::Logger *log, TR::GCStackAtlas *atlas)
{
...
void TR_Debug::print(TR::Logger *log, TR_GCStackMap *map, TR::GCStackAtlas *atlas)
{
...
#ifdef J9_PROJECT_SPECIFIC
void TR_Debug::dump(TR::Logger *log, TR_CHTable *chTable)
{
...
void TR_Debug::dumpGlobalRegisterTable(TR::Logger *log)
{
...
void TR_Debug::startTracingRegisterAssignment(TR::Logger *log, TR_RegisterKinds kindsToAssign)
{
...
void TR_Debug::stopTracingRegisterAssignment(TR::Logger *log)
{
    if (!_comp->getOption(TR_TraceRA))
        return;
...
void TR_Debug::traceRegisterAssignment(TR::Logger *log, const char *format, va_list args)
{
    if (!_comp->getOption(TR_TraceRA))
        return;
...
void TR_Debug::traceRegisterAssignment(TR::Logger *log, TR::Instruction *instr, bool insertedByRA, bool postRA)
{
...
void TR_Debug::traceRegisterAssigned(TR::Logger *log, TR_RegisterAssignmentFlags flags, TR::Register *virtReg,
    TR::Register *realReg)
{
...
void TR_Debug::traceRegisterFreed(TR::Logger *log, TR::Register *virtReg, TR::Register *realReg)
{
    TR_ASSERT(realReg, "Cannot trace assignment of NULL Real Register\n");
    TR_ASSERT(virtReg, "Cannot trace assignment of NULL Virtual Register\n");
...
void TR_Debug::traceRegisterInterference(TR::Logger *log, TR::Register *virtReg, TR::Register *interferingVirtual,
    int32_t distance)
{
...
void TR_Debug::traceRegisterWeight(TR::Logger *log, TR::Register *realReg, uint32_t weight)
{
