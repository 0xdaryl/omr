...
void TR_Debug::addInstructionComment(TR::Instruction *instr, char *comment, ...)
{
    ...
    if (disableInstructionComments || comment == NULL || !_comp->log()->isEnabled())
        return;
...
bool TR_Debug::performTransformationImpl(bool canOmitTransformation, const char *format, ...)
{
    ...
    if ((_comp->getOption(TR_CountOptTransformations) && _comp->getOptions()->getVerboseOptTransformationsRegex())
        || (!disablePerformTransformationLogging && log->isEnabled() && canOmitTransformation
    ...
    //
    if (disablePerformTransformationLogging || !log->isEnabled())
        return true;
...
void TR_Debug::print(TR::Logger *log, TR::SymbolReference *symRef)
{
