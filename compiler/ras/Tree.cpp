...
void TR_Debug::printTopLegend(TR::Logger *log)
{
    log->prints("\n----------------------------------------------------------------------------------------------------"
                "--------------------------------------------------------------------\n");
    log->flush();
...
void TR_Debug::printBottomLegend(TR::Logger *log)
{
    log->prints("\n"
...
// optimization.
//
void TR_Debug::printSymRefTable(TR::Logger *log, bool printFullTable)
{
...
void TR_Debug::printOptimizationHeader(TR::Logger *log, const char *funcName, const char *optName, int32_t optIndex,
    bool mustBeDone)
{
...
void TR_Debug::printLoadConst(TR::Logger *log, TR::Node *node)
{
    int32_t len = 0;
    char const *fmtStr;
...
void TR_Debug::print(TR::Logger *log, TR::CFG *cfg)
{
    int32_t numNodes = 0;
    int32_t index;
    TR::CFGNode *node;
...
void TR_Debug::print(TR::Logger *log, TR_RegionAnalysis *regionAnalysis, uint32_t indentation)
{
...
void TR_Debug::print(TR::Logger *log, TR_Structure *structure, uint32_t indentation)
{
...
void TR_Debug::print(TR::Logger *log, TR_RegionStructure *regionStructure, uint32_t indentation)
{
...
void TR_Debug::printPreds(TR::Logger *log, TR::CFGNode *node)
{
    log->prints("in={");
    int num = 0;
...
void TR_Debug::printSubGraph(TR::Logger *log, TR_RegionStructure *regionStructure, uint32_t indentation)
{
    int offset = 3;
    int num = 0;
...
void TR_Debug::print(TR::Logger *log, TR_InductionVariable *inductionVariable, uint32_t indentation)
{
    int offset = 3;
...
void TR_Debug::printBaseInfo(TR::Logger *log, TR_Structure *structure, uint32_t indentation)
{
    // Do indentation and dump the address of this block
...
void TR_Debug::print(TR::Logger *log, TR_BlockStructure *blockStructure, uint32_t indentation)
{
...
void TR_Debug::print(TR::Logger *log, TR::CFGNode *cfgNode, uint32_t indentation)
{
    if (cfgNode->asBlock())
...
void TR_Debug::print(TR::Logger *log, TR_StructureSubGraphNode *node, uint32_t indentation)
{
    print(log, node->getStructure(), indentation);
}

void TR_Debug::printNodesInEdgeListIterator(TR::Logger *log, TR::CFGEdgeList &li, bool fromNode)
{
    TR::Block *b;
    int num = 0;
...
void TR_Debug::print(TR::Logger *log, TR::Block *block, uint32_t indentation)
{
    // Do indentation and dump the address of this block
...
void TR_Debug::printIRTrees(TR::Logger *log, const char *title, TR::ResolvedMethodSymbol *methodSymbol)
{
    if (!methodSymbol)
...
void TR_Debug::printBlockOrders(TR::Logger *log, const char *title, TR::ResolvedMethodSymbol *methodSymbol)
{
    TR::TreeTop *tt = methodSymbol->getFirstTreeTop();
...
int32_t TR_Debug::print(TR::Logger *log, TR::TreeTop *tt)
{
...
int32_t TR_Debug::print(TR::Logger *log, TR::Node *node, uint32_t indentation, bool printChildren)
{
...
// Return the number of nodes encountered.
//
int32_t TR_Debug::printWithFixedPrefix(TR::Logger *log, TR::Node *node, uint32_t indentation, bool printChildren,
...
void TR_Debug::printDestination(TR::Logger *log, TR::TreeTop *treeTop)
{
    if (treeTop == NULL)
        return;
...
void TR_Debug::printBasicPreNodeInfoAndIndent(TR::Logger *log, TR::Node *node, uint32_t indentation)
{
    uint32_t numSpaces;
...
void TR_Debug::printBasicPostNodeInfo(TR::Logger *log, TR::Node *node, uint32_t indentation)
{
    log->prints("  ");
...
void TR_Debug::printNodeInfo(TR::Logger *log, TR::Node *node)
{
    char const * const globalIndexPrefix = "n";
    ...
    if (node->getOpCode().isNullCheck()) {
        if (node->getNullCheckReference())
            ...
            len += log->prints_len(" on null NullCheckReference ----- INVALID tree!!");
    ...
    if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
        ...
        if (node->getNumRelocations() > 0) {
            if (node->getRelocationType() == TR_AbsoluteAddress)
                len += log->prints_len(" Absolute [");
            else if (node->getRelocationType() == TR_ExternalAbsoluteAddress)
                len += log->prints_len(" External Absolute [");
            else
                len += log->prints_len(" Relative [");
        ...
        if (block->isExtensionOfPreviousBlock())
            len += log->prints_len(" (extension of previous block)");
        if (block->isCatchBlock()) {
            int32_t length;
            const char *classNameChars = block->getExceptionClassNameChars();
            if (classNameChars) {
                length = block->getExceptionClassNameLength();
                len += log->printf(" (catches %.*s)", length, getName(classNameChars, length));
            } else {
                classNameChars = "...";
                length = 3;
                len += log->printf(" (catches %.*s)", length,
                    classNameChars); // passing literal string shouldn't use getName()
            }

            if (block->isOSRCatchBlock()) {
                len += log->prints_len(" (OSR handler)");
            }
        }
        if (block->isSuperCold())
            len += log->prints_len(" (super cold)");
        else if (block->isCold())
            len += log->prints_len(" (cold)");

        if (block->isLoopInvariantBlock())
            len += log->prints_len(" (loop pre-header)");
        ...
        if ((node->getType().isInt64() && _comp->target().is32Bit() && !_comp->cg()->use64BitRegsOn32Bit()))
            len += log->printf(" %s:%s ", getGlobalRegisterName(node->getHighGlobalRegisterNumber()),
                getGlobalRegisterName(node->getLowGlobalRegisterNumber()));
        else
            len += log->printf(" %s ", getGlobalRegisterName(node->getGlobalRegisterNumber()));

        if (node->getOpCode().isLoadReg())
            print(log, node->getRegLoadStoreSymbolReference());
        ...
        if (isParentGlRegDep) {
            ...
            if ((node->getFirstChild()->getType().isInt64() && _comp->target().is32Bit()))
                len += log->printf(" %s:%s ", getGlobalRegisterName(node->getHighGlobalRegisterNumber(), size),
                    getGlobalRegisterName(node->getLowGlobalRegisterNumber(), size));
            else
                len += log->printf(" %s ", getGlobalRegisterName(node->getGlobalRegisterNumber(), size));
        }
    ...
    if (node->getOpCode().isLoadConst()) {
        ...
        if (getCurrentParent() && getCurrentParent()->getOpCodeValue() == TR::newarray
            ...
            len += log->prints_len("   ; array type is ");
...
void TR_Debug::printNodeFlags(TR::Logger *log, TR::Node *node)
{
    if (node->getFlags().getValue()) {
        int32_t len = log->prints_len(" (");
...
#ifdef J9_PROJECT_SPECIFIC
void TR_Debug::printBCDNodeInfo(TR::Logger *log, TR::Node *node)
{
    int32_t len = 0;
        ...
        if (!node->getOpCode().isStore()) {
            ...
            if (node->hasKnownOrAssumedCleanSign() || node->hasKnownOrAssumedPreferredSign()
                ...
                if (node->signStateIsKnown())
                    len += log->prints_len("known(");
                else
                    len += log->prints_len("assumed(");
                if (node->hasKnownOrAssumedCleanSign())
                    len += log->prints_len("clean");
    ...
    if (node->castedToBCD()) {
        len += log->prints_len(" <castedToBCD=true> ");
    }
...
//
void TR_Debug::printVCG(TR::Logger *log, TR::CFG *cfg, const char *sig)
{
    _nodeChecklist.empty();
...
void TR_Debug::printVCG(TR::Logger *log, TR_Structure *structure)
{
    if (structure->asRegion())
        printVCG(log, structure->asRegion());
}

void TR_Debug::printVCG(TR::Logger *log, TR_RegionStructure *regionStructure)
{
    log->prints("graph: {\n");
...
void TR_Debug::printVCG(TR::Logger *log, TR_StructureSubGraphNode *node, bool isEntry)
{
...
void TR_Debug::printVCGEdges(TR::Logger *log, TR_StructureSubGraphNode *node)
{
    for (auto edge = node->getSuccessors().begin(); edge != node->getSuccessors().end(); ++edge) {
...
void TR_Debug::printVCG(TR::Logger *log, TR::Block *block, int32_t vorder, int32_t horder)
{
...
//
void TR_Debug::printVCG(TR::Logger *log, TR::Node *node, uint32_t indentation)
{
    int32_t i;
...
void TR_Debug::print(TR::Logger *log, TR::VPConstraint *info)
{
    if (!info) {
        log->prints("none");
        return;
