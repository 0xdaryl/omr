...
uint8_t *TR::PPCAlignmentNopInstruction::generateBinaryEncoding()
   ...
   if (currentMisalign)
      ...
      trprintf(trace, log, "Expanding alignment nop %p into %u instructions: [ %p ", self(), nopsToAdd, lastNop);
...
void TR::PPCConditionalBranchInstruction::expandIntoFarBranch()
   ...
   trprintf(comp()->getOption(TR_TraceCG), comp()->log(), "Expanding conditional branch instruction %p into a far branch\n", self());
