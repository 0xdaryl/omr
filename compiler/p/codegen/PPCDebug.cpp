...
void
TR_Debug::print(TR::Logger *log, TR::PPCAlignmentNopInstruction *instr)
   {
   printPrefix(log, instr);
...
void
TR_Debug::print(TR::Logger *log, TR::PPCDepInstruction *instr)
   {
   printPrefix(log, instr);
   log->prints(getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCLabelInstruction *instr)
   {
   printPrefix(log, instr);
...
void
TR_Debug::print(TR::Logger *log, TR::PPCDepLabelInstruction *instr)
   {
   print(log, (TR::PPCLabelInstruction *) instr);
   if (instr->getDependencyConditions())
      print(log, instr->getDependencyConditions());
...
void
TR_Debug::print(TR::Logger *log, TR::PPCConditionalBranchInstruction *instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCDepConditionalBranchInstruction *instr)
   {
   print(log, (TR::PPCConditionalBranchInstruction *)instr);
   if (instr->getDependencyConditions())
      print(log, instr->getDependencyConditions());
...
void
TR_Debug::print(TR::Logger *log, TR::PPCAdminInstruction *instr)
   {
   int i;
   printPrefix(log, instr);
   log->printf("%s ", getOpCodeName(&instr->getOpCode()));
   ...
   if (instr->getOpCodeValue()==TR::InstOpCode::fence && instr->getFenceNode()!=NULL)
      {
      log->printf("\t%s[", (instr->getFenceNode()->getRelocationType()==TR_AbsoluteAddress)?"Absolute":"Relative");
...
void
TR_Debug::print(TR::Logger *log, TR::PPCImmInstruction *instr)
   {
   printPrefix(log, instr);
...
void
TR_Debug::print(TR::Logger *log, TR::PPCSrc1Instruction *instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCDepImmSymInstruction *instr)
   {
   ...
   if (name)
      log->printf("%s \t" POINTER_PRINTF_FORMAT "\t\t; Direct Call \"%s\"", getOpCodeName(&instr->getOpCode()), (intptr_t)cursor + distance, name);
   else
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Instruction *instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Src1Instruction *instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1ImmInstruction *instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Src1ImmInstruction *instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
   print(log, instr->getTargetRegister(), TR_WordReg); log->prints(", ");
   ...
   log->printf(", " "%d", (intptr_t)(int32_t)instr->getSourceImmediate());
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Src1Imm2Instruction *instr)
   {
   uint64_t lmask;
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCSrc2Instruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCSrc3Instruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
   print(log, instr->getSource1Register(), TR_WordReg); log->prints(", ");
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Src2Instruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Src2ImmInstruction * instr)
   {
   uint64_t lmask;
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1Src3Instruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
   print(log, instr->getTargetRegister(), TR_WordReg); log->prints(", ");
...
void
TR_Debug::print(TR::Logger *log, TR::PPCMemSrc1Instruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
   ...
   print(log, instr->getMemoryReference()); log->prints(", ");
...
void
TR_Debug::print(TR::Logger *log, TR::PPCMemInstruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCTrg1MemInstruction * instr)
   {
   printPrefix(log, instr);
   log->printf("%s \t", getOpCodeName(&instr->getOpCode()));
...
void
TR_Debug::print(TR::Logger *log, TR::PPCControlFlowInstruction * instr)
   {
   int i;
   printPrefix(log, instr);
...
#ifdef J9_PROJECT_SPECIFIC
void
TR_Debug::print(TR::Logger *log, TR::PPCVirtualGuardNOPInstruction * instr)
   {
   printPrefix(log, instr);
...
void
TR_Debug::print(TR::Logger *log, TR::MemoryReference * mr, bool d_form)
   {
   log->printc('[');
...
void
TR_Debug::printPPCGCRegisterMap(TR::Logger *log, TR::GCRegisterMap *map)
   {
   TR::Machine *machine = _cg->machine();
   ...
   for (int i = 31; i>=0; i--)
      {
      if (map->getMap() & (1 << i))
         log->printf("%s ", getName(machine->getRealRegister((TR::RealRegister::RegNum)(31 - i + TR::RealRegister::FirstGPR))));
      }
...
void
TR_Debug::printp(TR::Logger *log, TR::Snippet *snippet)
   {
   switch (snippet->getKind())
      {
#ifdef J9_PROJECT_SPECIFIC
