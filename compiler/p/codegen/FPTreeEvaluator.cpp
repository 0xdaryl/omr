...
TR::Register *OMR::Power::TreeEvaluator::fbits2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
      ...
      TR::MemoryReference *tempMR = new (comp->trHeapMemory()) TR::MemoryReference(child, 4, cg);
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, floatReg, new (comp->trHeapMemory()) TR::MemoryReference(node, *tempMR, 0, 4, cg));
      generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, target, tempMR);
      tempMR->decNodeReferenceCounts(cg);
      }
   else
      {
      floatReg = comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8) ? cg->gprClobberEvaluate(child) : cg->evaluate(child);
      generateMvFprGprInstructions(cg, node, fpr2gprSp, comp->target().is64Bit(),target, floatReg);
   ...
   if (node->normalizeNanValues())
      ...
      TR::RegisterDependencyConditions *deps = new (comp->trHeapMemory()) TR::RegisterDependencyConditions(0, 2, comp->trMemory());
...
#ifdef J9_PROJECT_SPECIFIC
      if (child->getOpCodeValue() == TR::irlload && comp->target().is64Bit())        // 64-bit only
         {
         ...
         TR::MemoryReference *tempMRLoad1 = new (comp->trHeapMemory()) TR::MemoryReference(child, *tempMR, 0, 4, cg);
         TR::MemoryReference *tempMRLoad2 = new (comp->trHeapMemory()) TR::MemoryReference(child, *tempMR, 4, 4, cg);
...
TR::Register *OMR::Power::TreeEvaluator::dbits2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
      ...
      if (comp->target().is64Bit())
         ...
         TR::MemoryReference  *tempMR2 = new (comp->trHeapMemory()) TR::MemoryReference(node, *tempMR, 0, 8, cg);
         ...
         highMem = new (comp->trHeapMemory()) TR::MemoryReference(node, *tempMR, 0, 4, cg);
         generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, highReg, highMem);
         lowMem = new (comp->trHeapMemory()) TR::MemoryReference(node, *tempMR, 4, 4, cg);
   ...
   if (node->normalizeNanValues())
      ...
      uint16_t numDeps = comp->target().is64Bit() ? 2 : 3;
      TR::RegisterDependencyConditions *deps = new (comp->trHeapMemory()) TR::RegisterDependencyConditions(numDeps, numDeps, comp->trMemory());
   ...
   if (cg->comp()->target().is64Bit())
      ...
      if (offset != PTOC_FULL_INDEX)
         ...
         if (offset<LOWER_IMMED || offset>UPPER_IMMED)
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, trgRegister, new (comp->trHeapMemory()) TR::MemoryReference(srcRegister, LO_VALUE(offset), 4, cg));
            cg->stopUsingRegister(srcRegister);
            }
         else
            {
            generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, trgRegister, new (comp->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 4, cg));
            }
         }
      }
   ...
   if (cg->comp()->target().is64Bit())
      ...
      if (offset != PTOC_FULL_INDEX)
         ...
         if (offset<LOWER_IMMED || offset>UPPER_IMMED)
            ...
            TR::MemoryReference *memRef = new (comp->trHeapMemory()) TR::MemoryReference(srcRegister, LO_VALUE(offset), 8, cg);
            if (splats)
            ...
            {
            TR::MemoryReference *memRef = new (comp->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 8, cg);
            if (splats)
               memRef->forceIndexedForm(node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::floadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   tempMR = new (cg->comp()->trHeapMemory()) TR::MemoryReference(node, 4, cg);
...
TR::Register *OMR::Power::TreeEvaluator::dloadHelper(TR::Node *node, TR::CodeGenerator *cg, TR::Register *tempReg, TR::InstOpCode::Mnemonic opcode)
   ...
   if (cg->comp()->target().is32Bit() && needSync && !cg->is64BitProcessor())
      ...
      TR::RegisterDependencyConditions *dependencies = new (comp->trHeapMemory()) TR::RegisterDependencyConditions(5, 5, comp->trMemory());
      ...
      if (opcode == TR::InstOpCode::lxvdsx || opcode == TR::InstOpCode::lxsdx)
         ...
         if (tempMR->hasDelayedOffset())
            ...
            tempMR = new (comp->trHeapMemory()) TR::MemoryReference(NULL, tmpReg, 16, cg);
            }
         else
            {
            tempMR->forceIndexedForm(node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::vsplatsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (node->getDataType() == TR::VectorInt8)
      ...
      TR::MemoryReference *tempMR  = new (comp->trHeapMemory()) TR::MemoryReference(node, temp, 1, cg);
      ...
      TR::MemoryReference *tempMR  = new (comp->trHeapMemory()) TR::MemoryReference(node, temp, 2, cg);
      TR::Register *srcReg = cg->evaluate(child);
      ...
      if (!disableDirectMove && comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8) && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX))
         {
         ...
         generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, new (comp->trHeapMemory()) TR::MemoryReference(node, localTemp, 4, cg), tempReg);

         TR::MemoryReference *tempMR = new (comp->trHeapMemory()) TR::MemoryReference(node, localTemp, 4, cg);
         tempMR->forceIndexedForm(node, cg);
      ...
      if (!disableDirectMove && comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8) && comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX))
         ...
         TR::MemoryReference *tempMR = new (comp->trHeapMemory()) TR::MemoryReference(node, temp, 8, cg);
         ...
         if (comp->target().is64Bit())
            ...
            {
            TR::MemoryReference *tempMR2 =  new (comp->trHeapMemory()) TR::MemoryReference(node, *tempMR, 4, 4, cg);
...
TR::Register *OMR::Power::TreeEvaluator::vdgetelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::RegisterDependencyConditions *dep = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(0, 4, cg->comp()->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::vdsetelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::RegisterDependencyConditions *dep = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(0, 5, cg->comp()->trMemory());
   dep->addPostCondition(vectorReg, TR::RealRegister::NoReg);
   dep->addPostCondition(idxReg, TR::RealRegister::NoReg);
...
TR::Register *OMR::Power::TreeEvaluator::freturnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::RegisterDependencyConditions *dependencies = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(1, 0, cg->comp()->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::fremEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::Register *copyReg;
   TR::RegisterDependencyConditions *dependencies = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(16, 16, cg->comp()->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::int2dbl(TR::Node * node, TR::Register *srcReg, bool canClobber, TR::CodeGenerator *cg)
   ...
   if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && cg->comp()->target().is64Bit()))
      ...
      {
      TR::RegisterDependencyConditions *dependencies = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(6, 6, cg->comp()->trMemory());
      if (!canClobber)
         {
         tempReg = cg->allocateRegister();
...
TR::Register *OMR::Power::TreeEvaluator::long2dbl(TR::Node *node, TR::CodeGenerator *cg)
{
   ...
   if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && cg->comp()->target().is64Bit()))
      ...
      TR::RegisterDependencyConditions *dependencies = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(8, 8, cg->comp()->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::d2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && cg->comp()->target().is64Bit()))
      ...
      dependencies = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(17, 17, cg->comp()->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::dxfrsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::RegisterDependencyConditions *dep = new (cg->comp()->trHeapMemory()) TR::RegisterDependencyConditions(0, 2, cg->comp()->trMemory());
